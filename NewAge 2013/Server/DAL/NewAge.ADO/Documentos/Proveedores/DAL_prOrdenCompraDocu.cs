    ...
    public class DAL_prOrdenCompraDocu : DAL_Base
    {
        ...
        public List<DTO_prOrdenCompraAprob> DAL_prOrdenCompraDocu_GetPendientesByModulo(DTO_glDocumento doc, string actividadFlujoID, DTO_glActividadPermiso actividadPerm, DTO_seUsuario usuario)
        {
            try
            {
                ...
                mySqlCommand.CommandText =
                    "   select temp.*, " +
                    "    det.ConsecutivoDetaID,det.OrdCompraDocuID,det.OrdCompraDetaID,det.Detalle4ID,det.Documento4ID,det.CodigoBSID,det.Descriptivo,det.inReferenciaID,det.EmpaqueInvID as UnidadInvID,det.CantidadOC " +
                    "    ,solCtrl.PrefijoID PrefijoSolID,solCtrl.DocumentoNro as DocumentoNroSol,det.SolicitudDocuID,det.SolicitudDetaID,det.ValorUni,det.ValorTotML,det.IvaTotML,det.ValorTotME,det.IvaTotME,refer.MarcaInvID, refer.RefProveedor " +
                    "    ,empaque.UnidadInvID as UnidadEmpaque,IsNull(empaque.Cantidad,1) as CantidadEmpaque " +
                    "    from ( select distinct ctrl.EmpresaID,ctrl.NumeroDoc,ctrl.DocumentoID,ctrl.PrefijoID,ctrl.DocumentoNro,ctrl.FechaDoc Fecha,ctrl.Observacion, " +
                    "        pr.ProveedorID,pr.Descriptivo ProveedorNombre,oc.LugarEntrega,oc.MonedaOrden,oc.MonedaPago,docAprueba.UsuarioAprueba,usr.UsuarioID,oc.Observaciones ObservacionOC,oc.Instrucciones, " +
                    ...
                    "        pr.ProveedorID,pr.Descriptivo,oc.LugarEntrega,oc.MonedaOrden,oc.MonedaPago,oc.Observaciones,oc.Instrucciones,docAprueba.UsuarioAprueba,usr.UsuarioID ) temp  " +
                ...
                while (dr.Read())
                {
                    ...
                    dtoDet.PrefDocSol = dr["PrefijoSolID"].ToString().TrimEnd() + " - " + dr["DocumentoNroSol"].ToString();
        ...
        public List<DTO_prOrdenCompraResumen> DAL_prOrdenCompraDocu_GetResumen(int documentID, DTO_seUsuario usuario, ModulesPrefix mod, List<Tuple<string, string>> filtros)
        {
            try
            {
                ...
                "    detOC.CantidadOC CantidadOrdComp,detOC.ValorTotML as ValorTotMLOC,detOC.ValorTotME as ValorTotMEOC,detOC.ValorUni,temp.CantidadSum CantidadOC,ctrlOC.FechaDoc FechaOC,ctrlOC.MonedaID MonedaIDOC, " +
                "    detSol.Documento1ID,detSol.Documento2ID,detSol.Documento3ID,detSol.Documento4ID,detSol.Documento5ID,  " +
                "    detSol.Detalle1ID,detSol.Detalle2ID,detSol.Detalle3ID,detSol.Detalle4ID,detSol.Detalle5ID, IsNull(mat.Margen,0) as Margen, " +
                "    detSol.CantidadDoc1,detSol.CantidadDoc2,detSol.CantidadDoc3,detSol.CantidadDoc4,detSol.CantidadDoc5, oc.ProveedorID,oc.MonedaPago as MonedaPagoOC,oc.MonedaOrden, oc.TasaOrden  " +
                ...
                while (dr.Read())
                {
                    ...
                    if (dtoRes.CantidadSol.Value > Convert.ToDecimal(dr["Margen"]))
                        result.Add(dtoRes);
                }
