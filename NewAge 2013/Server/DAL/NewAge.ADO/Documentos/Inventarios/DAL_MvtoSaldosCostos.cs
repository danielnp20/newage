    ...
    public class DAL_MvtoSaldosCostos : DAL_Base
    {
        ...
        public DTO_inControlSaldosCostos DAL_inControlSaldosCostos_Get(DTO_inControlSaldosCostos filter)
        {
            try
            {
                ...
                query = "Select ctrlCto.*, bodTipo.BodegaTipo, costeo.CosteoTipo,bodUbica.UbicacionID  " +
                        "  From inControlSaldosCostos ctrlCto with(nolock) " +
                        "  inner join inBodega bod with(nolock) on bod.BodegaID = ctrlCto.BodegaID and bod.EmpresaGrupoID =  ctrlCto.eg_inBodega " +
                        ...
                        "  left join inbodegarefe bodUbica with(nolock) on bodUbica.BodegaID = ctrlCto.BodegaID and  bodUbica.inReferenciaID = ctrlCto.inReferenciaID and bodUbica.ActivoInd = 1 " +
                ...
                if (!string.IsNullOrEmpty(filter.CosteoGrupoInvID.Value.ToString()))
                {
                    query += "and ctrlCto.CosteoGrupoInvID = @CosteoGrupoInvID ";
                    mySqlCommand.Parameters.Add("@CosteoGrupoInvID",SqlDbType.Char, UDT_CosteoGrupoInvID.MaxLength);
                    mySqlCommand.Parameters["@CosteoGrupoInvID"].Value = filter.CosteoGrupoInvID.Value;
                    filterInd = true;
                }
        ...
        /// <summary>
        /// Actualizar en tabla inControlSaldosCostos solo consecutivos de COsto y Saldo 
        /// </summary>
        /// <param name="control">filtro</param>
        public void DAL_inControlSaldosCostos_UpdConsecutivos(DTO_inControlSaldosCostos control)
        {
            try
            {
                string egCtrl = StaticMethods.GetGrupoEmpresasControl(base.MySqlConnection, base.MySqlConnectionTx, this.loggerConnectionStr);
                ...
                #region CommandText
                mySqlCommand.CommandText = "    UPDATE inControlSaldosCostos " +
                                           "    SET RegistroSaldo  = @RegistroSaldo  " +
                                           "    ,RegistroCosto  = @RegistroCosto " +
                                           "    ,CosteoGrupoInvID  = @CosteoGrupoInvID " +
                                           "    WHERE EmpresaID = @EmpresaID  and ActivoID = @ActivoID " +
                                           "    and BodegaID = @BodegaID  and EstadoInv = @EstadoInv " +
                                           "    and inReferenciaID = @inReferenciaID  and Parametro1 = @Parametro1 and Parametro2 = @Parametro2 " +
                                           "    and IdentificadorTr = @IdentificadorTr  and Periodo = @Periodo ";
                ...
                mySqlCommand.Parameters.Add("@CosteoGrupoInvID", SqlDbType.Char, UDT_CosteoGrupoInvID.MaxLength);
                mySqlCommand.Parameters.Add("@RegistroSaldo", SqlDbType.Decimal);
                mySqlCommand.Parameters.Add("@RegistroCosto", SqlDbType.Decimal);
                #endregion
                #region Asignacion de valores
                mySqlCommand.Parameters["@EmpresaID"].Value = this.Empresa.ID.Value;
                mySqlCommand.Parameters["@BodegaID"].Value = control.BodegaID.Value;               
                mySqlCommand.Parameters["@inReferenciaID"].Value = control.inReferenciaID.Value;
                mySqlCommand.Parameters["@ActivoID"].Value = control.ActivoID.Value;
                mySqlCommand.Parameters["@EstadoInv"].Value = control.EstadoInv.Value;
                mySqlCommand.Parameters["@Parametro1"].Value = control.Parametro1.Value;
                mySqlCommand.Parameters["@Parametro2"].Value = control.Parametro2.Value;
                mySqlCommand.Parameters["@IdentificadorTr"].Value = control.IdentificadorTr.Value;
                mySqlCommand.Parameters["@Periodo"].Value = control.Periodo.Value;

                mySqlCommand.Parameters["@CosteoGrupoInvID"].Value = control.CosteoGrupoInvID.Value;
                mySqlCommand.Parameters["@RegistroSaldo"].Value = control.RegistroSaldo.Value;
                mySqlCommand.Parameters["@RegistroCosto"].Value = control.RegistroCosto.Value;
                #endregion
                foreach (SqlParameter param in mySqlCommand.Parameters)
                {
                    if (param.Direction.Equals(ParameterDirection.Input))
            ...
            catch (Exception ex)
            {
                ...
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_inControlSaldosCostos_UpdConsecutivos");
                throw exception;
            }
        ...
        public List<DTO_inControlSaldosCostos> DAL_inControlSaldosCostos_GetByParameter(DTO_inControlSaldosCostos ctrlSaldoCosto, bool CostosIFRSInd = false)
        {
            try
            {
                ...
                where = "Select ctrlCto.*, bodTipo.BodegaTipo, costeo.CosteoTipo,bodUbica.UbicacionID " +
                         "  From inControlSaldosCostos ctrlCto with(nolock) " +
                         "  inner join inBodega bod with(nolock) on bod.BodegaID = ctrlCto.BodegaID and bod.EmpresaGrupoID =  ctrlCto.eg_inBodega " +
                         "  inner join inBodegaTipo bodTipo with(nolock) on bodTipo.BodegaTipoID = bod.BodegaTipoID and bodTipo.EmpresaGrupoID =  bod.eg_inBodegaTipo " +
                         "  inner join inCosteoGrupo costeo with(nolock) on costeo.CosteoGrupoInvID = ctrlCto.CosteoGrupoInvID " +
                         "  left join inbodegarefe bodUbica with(nolock) on bodUbica.BodegaID = ctrlCto.BodegaID and  bodUbica.inReferenciaID = ctrlCto.inReferenciaID and bodUbica.ActivoInd = 1 " +
                         "Where ctrlCto.EmpresaID = @EmpresaID ";
