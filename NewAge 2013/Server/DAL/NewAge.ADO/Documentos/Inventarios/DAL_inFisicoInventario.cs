       ...
       /// <summary>
        ...
        public List<DTO_inFisicoInventario> DAL_inFisicoInventario_GetByParameter(DTO_inFisicoInventario fisicoInventario)
        {
            try
            {
                ...
                query = "select inv.*,bodUbica.UbicacionID,refer.RefProveedor,refer.MarcaInvID,marca.Descriptivo as MarcaDesc from inFisicoInventario  inv with(nolock) " +
                        "  left join inbodegarefe bodUbica with(nolock) on bodUbica.BodegaID = inv.BodegaID and  bodUbica.inReferenciaID = inv.inReferenciaID and bodUbica.ActivoInd = 1 " +
                        "  Left join inReferencia as refer  with(nolock) on refer.inReferenciaID = inv.inReferenciaID and refer.EmpresaGrupoID = inv.eg_inReferencia " +
                        "  Left join inMarca as marca  with(nolock) on marca.MarcaInvID = refer.MarcaInvID " +
                        "  where inv.EmpresaID = @EmpresaID ";

                mySqlCommand.Parameters.Add("@EmpresaID", SqlDbType.Char, UDT_EmpresaID.MaxLength);
                mySqlCommand.Parameters["@EmpresaID"].Value = this.Empresa.ID.Value;
                if (!string.IsNullOrEmpty(fisicoInventario.BodegaID.Value))
                {
                    query += "and inv.BodegaID = @BodegaID ";
                ...
                {
                    query += "and inv.inReferenciaID = @inReferenciaID ";
                ...
                {
                    query += "and inv.ActivoID = @ActivoID ";
                ...
                {
                    query += "and inv.EstadoInv = @EstadoInv ";
                ...
                {
                    query += "and inv.Parametro1 = @Parametro1 ";
                ...
                {
                    query += "and inv.Parametro2 = @Parametro2 ";
                ...
                while (dr.Read())
                {
                    ...
                    fisico.UbicacionID = dr["UbicacionID"].ToString();
                    fisico.RefProveedor.Value = dr["RefProveedor"].ToString();
                    fisico.MarcaInvID.Value = dr["MarcaInvID"].ToString();
                    fisico.MarcaDesc.Value = dr["MarcaDesc"].ToString();
                    result.Add(fisico);
                    index++;
                }
                dr.Close();
