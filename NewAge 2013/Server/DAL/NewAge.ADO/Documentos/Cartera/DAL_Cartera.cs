    ...
    public class DAL_Cartera : DAL_Base
    {
        ...
        public List<DTO_QueryGestionCobranza> DAL_Cartera_GestionCobranzaGetActividades(DateTime fechaFin,DTO_glIncumplimientoEtapa etapa, string clienteID)
        {
            try
            {
                ...
                if (etapa.EstadoDeuda.Value == 4)
                    where = " and cli.EstadoCartera in (4,5,6)";
                else
                    where = " and cli.EstadoCartera not in (4,5,6)";
                ...
                mySqlCommand.CommandText =
                        ...
                        " Select *  " +
                        " from  ( " +      
	                        " Select crd.NumeroDoc, cie.EmpresaID,  crd.Libranza,  " +
                        " 		crd.ClienteID, cli.Descriptivo as Nombre,  cli.EstadoCartera, crd.TipoEstado, cie.CobranzaGestionID as CobranzaGestionCierre, " +
                        " 		query1.CuotaID,query1.FechaCuota, dateadd(day, @DiasFin,query1.FechaCuota) as FechaAct,  " +
                        " 		query1.VlrCuota - query1.VlrPagadoCuota as VlrSaldoCuota,ges .EtapaIncumplimiento as EtapaID " +      
                        " 	from ccCierreDiaCartera cie  " +
                        ...
                        " 		Left join  " +
                        " 		(  " +
                        " 		Select pp.numeroDoc,min(CuotaID) as CuotaID,min(FechaCuota)as FechaCuota,min(VlrCuota)as VlrCuota, " +
                        " 		min(VlrPagadoCuota)as VlrPagadoCuota " +
                        " 			from  ccCreditoPlanPagos pp " +
                        " 			Where  pp.VlrPagadoCuota < pp.VlrCuota and pp.FechaCuota <=  @FechaCorte " +
                        " 			group by pp.numeroDoc " +
                        " 		) query1 on query1.NumeroDoc = crd.numeroDoc " +
                        " 	where cie.EmpresaID = @EmpresaID and cast(cie.Fecha as date) = @FechaCorte and ges.EtapaIncumplimiento = @EtapaID " +
                        " 	) query2 where TipoEstado <= 2 Order by FechaAct, Nombre ";
                ...
                #region Asignacion de valores
                mySqlCommand.Parameters["@EmpresaID"].Value = this.Empresa.ID.Value;
                mySqlCommand.Parameters["@FechaCorte"].Value = fechaFin.Date;              
