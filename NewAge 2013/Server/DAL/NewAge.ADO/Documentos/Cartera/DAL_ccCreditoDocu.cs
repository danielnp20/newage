    ...
    public class DAL_ccCreditoDocu : DAL_Base
    {
        ...
        public bool DAL_ccCreditoDocu_Update(DTO_ccCreditoDocu credito)
        {
            try
            {
                ...
                //CobranzaGestion
                if (!string.IsNullOrWhiteSpace(credito.CobranzaGestionCierre.Value))
                    mySqlCommandSel.Parameters["@eg_ccCobranzaGestion"].Value = this.GetMaestraEmpresaGrupoByDocumentID(AppMasters.ccCobranzaGestion, this.Empresa, egCtrl);
                else
        ...
        /// <returns>retorna una lista de DTO_ccCarteraDocu</returns>
        public DTO_ccCreditoDocu DAL_ccCreditoDocu_GetByLibranza(int libranza,string usuarioID)
        {
            try
            {
                DTO_ccCreditoDocu result = null;
                ...
                mySqlCommand.Parameters.Add("@DocPantalla", SqlDbType.Int);
                mySqlCommand.Parameters.Add("@Usuario", SqlDbType.Char, UDT_UsuarioID.MaxLength);

                mySqlCommand.Parameters["@Libranza"].Value = libranza;
                mySqlCommand.Parameters["@EmpresaID"].Value = this.Empresa.ID.Value;
                mySqlCommand.Parameters["@DocPantalla"].Value = AppQueries.QueryLibranza;
                mySqlCommand.Parameters["@Usuario"].Value = usuarioID;
            ...
            try
            {
                ...
                mySqlCommand.CommandText =
                    ...
                    " FROM ccCreditoDocu cred with(nolock)  " +
                        ...
                        " LEFT JOIN ccCreditoPlanPagos pag with (nolock) ON cred.NumeroDoc = pag.NumeroDoc and @FechaCorte > pag.FechaCuota " +
                ...
                if (dr.Read())
                {
                    result = new DTO_ccCreditoDocu(dr);
                    result.Nombre.Value = dr["Nombre"].ToString();
                    if (!string.IsNullOrWhiteSpace(dr["capital"].ToString()))
                        result.VlrCapital.Value = Convert.ToDecimal(dr["capital"]);
                    else
                        result.VlrCapital.Value = 0;
                    if (!string.IsNullOrWhiteSpace(dr["CuotasMora"].ToString()))
                        result.CuotasMora.Value = Convert.ToInt16(dr["CuotasMora"]);
                    else
                        result.CuotasMora.Value = 0;
                }
        ...
        /// <param name="cliente">Cliente</param>
        /// <returns>retorna una lista de DTO_ccCarteraDocu</returns>
        public List<DTO_ccCreditoDocu> DAL_ccCreditoDocu_GetCreditosByClienteAndFecha(string cliente, DateTime fechaCorte,SectorCartera sectorCart,bool onlyWithSaldo, bool useFechaCorte,string usuarioID)
        {
            try
            {
                List<DTO_ccCreditoDocu> result = null;
                     ...
                     mySqlCommand.CommandText =
                        ...
                        "      INNER JOIN ccCliente cli with(nolock) ON cli.ClienteID = cred.ClienteID and cli.EmpresaGrupoID = cred.eg_ccCliente " +
                                 ...
                                 " LEFT JOIN ccCreditoPlanPagos pag with (nolock) ON cred.NumeroDoc = pag.NumeroDoc  " + where +
                                    "  and pag.VlrPagadoCuota < (pag.VlrCuota + pag.VlrPagadoExtras) " +
                               " WHERE cred.EmpresaID = @EmpresaID and cred.ClienteID = @ClienteID and capital.ComponenteCarteraID = '001' ";
                }
                #endregion
        ...
        public List<DTO_ccSolicitudCompraCartera> DAL_ccCreditoDocu_GetCreditosWithRecompra(string clienteID, string userName, string actAprobacionGiro, bool allEmpresas)
        {
            try
            {
                ...
                mySqlCommand.CommandText =
                    ...
                    "   INNER JOIN ccEstadoCuentaHistoria his with(nolock) on his.NumeroDoc = cred.DocEstadoCuenta " +
                    "       AND his.EC_Proposito in (3,4,5) and his.EC_FijadoInd=@Fijado " +
            ...
            DateTime fechaIni, DateTime fechaFin)
        {
            try
            {
                ...
                mySqlCommand.CommandText =
                        ...
                        "    INNER JOIN glActividadEstado act with(nolock) on act.NumeroDoc = cred.NumeroDoc AND act.actividadFlujoID = @ActividadFlujoAprobGiro  " +
                        "    INNER JOIN glActividadPermiso perm with(nolock) on perm.EmpresaGrupoID = ctrl.EmpresaID " +
                        ...
                        "   AND (Cast(ctrl.FechaDoc as Date) between @FechaIni and @FechaFin) " + 
                        "ORDER BY CAST(cred.Libranza AS INTEGER) ";
                #endregion
            ...
            DateTime fechaIni, DateTime fechaFin)
        {
            try
            {
                ...
                mySqlCommand.CommandText =
                        ...
                        "   INNER JOIN glActividadEstado act with(nolock) on act.NumeroDoc = cred.NumeroDoc AND act.actividadFlujoID = @ActividadFlujoID  " +
                        "   INNER JOIN glActividadPermiso perm with(nolock) on perm.EmpresaGrupoID = ctrl.EmpresaID " +
        ...
        public List<DTO_ccCreditoDocu> DAL_ccCreditoDocu_GetCreditosVenta(string actFlujoID, string compradorCarteraID, string oferta, string usuarioID)
        {
            try
            {
                ...
                mySqlCommand.CommandText =
                    ...
                    "   INNER JOIN glActividadEstado act with(nolock) on act.NumeroDoc = cred.NumeroDoc " +
                    "       AND act.actividadFlujoID = @ActividadFlujoID  " +
                    "   INNER JOIN glActividadPermiso perm with(nolock) on perm.EmpresaGrupoID = ctrl.EmpresaID " +
                    "       AND perm.areaFuncionalID = ctrl.areaFuncionalID  AND perm.actividadFlujoID = @ActividadFlujoID  AND perm.UsuarioID = @UsuarioID " +
        ...
        /// <param name="docSolicitud"></param>
        /// <returns></returns>
        public bool DAL_ccCreditoDocu_UpdateSentencia(int numeroDocCred, decimal? vlrCuotaSentencia,DateTime? fechaCuota1Sentencia,Int16? plazoSentencia)
        {
            try
            {
                string egCtrl = StaticMethods.GetGrupoEmpresasControl(base.MySqlConnection, base.MySqlConnectionTx, this.loggerConnectionStr);
                ...
                mySqlCommandSel.CommandText =
                                              "UPDATE ccCreditoDocu SET" +
                                               "  VlrCuotaSentencia = @VlrCuotaSentencia " +
                                               " ,FechaCuota1Sentencia = @FechaCuota1Sentencia " +
                                               " ,PlazoSentencia = @PlazoSentencia " +                                    
                                               " WHERE  NumeroDoc = @NumeroDoc";
                #endregion
                #region Creacion Parametros
                mySqlCommandSel.Parameters.Add("@NumeroDoc", SqlDbType.Int);     
                mySqlCommandSel.Parameters.Add("@VlrCuotaSentencia", SqlDbType.Decimal);             
                mySqlCommandSel.Parameters.Add("@FechaCuota1Sentencia", SqlDbType.SmallDateTime);
                mySqlCommandSel.Parameters.Add("@PlazoSentencia", SqlDbType.SmallInt);
                #endregion
                #region Asignacion Valores
                mySqlCommandSel.Parameters["@NumeroDoc"].Value = numeroDocCred;
                mySqlCommandSel.Parameters["@VlrCuotaSentencia"].Value = vlrCuotaSentencia;
                mySqlCommandSel.Parameters["@FechaCuota1Sentencia"].Value = fechaCuota1Sentencia;
                mySqlCommandSel.Parameters["@PlazoSentencia"].Value = plazoSentencia;
            ...
            catch (Exception ex)
            {
                //Log error
                var exception = new Exception(DictionaryMessages.Err_UpdateData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_ccCreditoDocu_UpdateSentencia");
                return false;
            }
        }
