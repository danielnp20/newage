...
{
    public class DAL_ccCarteraMvtoHist : DAL_Base
    {
        ...
        public DAL_ccCarteraMvtoHist(SqlConnection c, SqlTransaction tx, DTO_glEmpresa empresa, int userId, string loggerConn) : base(c, tx, empresa, userId, loggerConn) { }

        #region CRUD

        /// <summary>
        /// Añade los un registro a ccCarteraMvtoHist
        /// </summary>
        /// <param name="mvto">Movimiento de la caretra</param>
        public void DAL_ccCarteraMvtoHist_Add(DTO_ccCarteraMvto mvto, int numDocProceso)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText =
                    "INSERT INTO ccCarteraMvtoHist (NumCredito,NumeroDoc,NumeroDocProceso,EmpresaID,ComponenteCarteraID,CompradorCarteraID,Tasa,VlrComponente,VlrAbono,eg_ccCarteraComponente,eg_ccCompradorCartera)" +
                    "VALUES (@NumCredito,@NumeroDoc,@NumeroDocProceso,@EmpresaID,@ComponenteCarteraID,@CompradorCarteraID,@Tasa,@VlrComponente,@VlrAbono,@eg_ccCarteraComponente,@eg_ccCompradorCartera)";
                #endregion
            ...
            catch (Exception ex)
            {
                ...
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_ccCarteraMvtoHist_Add");
                throw exception;
            }
        }
        ...
        /// </summary>
        /// <param name="mvto">Movimiento de la caretra</param>
        public void DAL_ccCarteraMvtoHist_Revertir(int numDocCredito, int numDocReversion, int numDocNuevo)
        {
            try
            {
                SqlCommand mySqlCommandSel = base.MySqlConnection.CreateCommand();
                ...
                mySqlCommandSel.CommandText =
                    "insert into ccCarteraMvtoHist(NumCredito, NumeroDoc, EmpresaID, ComponenteCarteraID, CompradorCarteraID, Tasa, VlrComponente, " +
                    ...
                    "	FROM ccCarteraMvtoHist with(nolock) where NumCredito = @NumCredito and NumeroDoc = @NumeroDocOld ";
            ...
            catch (Exception ex)
            {
                //Log error
                var exception = new Exception(DictionaryMessages.Err_AddData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_ccCarteraMvtoHist_Revertir");
                throw exception;
            }
        }
        ...
        /// <summary>
        /// Funcion que trae los movimientos de cartera por credito
        /// </summary>
        /// <param name="numDocCredito">numDocCredito de la libranza</param>
        /// <returns>Lista</returns>
        public List<DTO_ccCarteraMvto> DAL_CarteraMvto_QueryByNumCredito(int? numDocCredito)
        {
            try
            {
                string egCtrl = StaticMethods.GetGrupoEmpresasControl(base.MySqlConnection, base.MySqlConnectionTx, this.loggerConnectionStr);
                List<DTO_ccCarteraMvto> result = new List<DTO_ccCarteraMvto>();

                SqlCommand mySqlCommandSel = base.MySqlConnection.CreateCommand();
                mySqlCommandSel.Transaction = base.MySqlConnectionTx;

                #region Parametros
                mySqlCommandSel.Parameters.Add("@NumCredito", SqlDbType.Char, UDT_DocTerceroID.MaxLength);
                mySqlCommandSel.Parameters["@NumCredito"].Value = numDocCredito;

                #endregion

                mySqlCommandSel.CommandText = " SELECT * FROM ccCarteraMvtoHist where NumCredito = @NumCredito ";

                SqlDataReader dr = mySqlCommandSel.ExecuteReader();
                while (dr.Read())
                {
                    DTO_ccCarteraMvto mvto = new DTO_ccCarteraMvto(dr);
                    result.Add(mvto);
                }
                dr.Close();
            ...
            {
                var exception = new Exception(DictionaryMessages.Err_GettingData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_CarteraMvto_QueryByNumCredito");
                throw exception;
            }
        }
