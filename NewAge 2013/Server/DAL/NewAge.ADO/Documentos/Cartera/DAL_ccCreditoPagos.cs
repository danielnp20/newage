...
namespace NewAge.ADO
{
    public class DAL_ccCreditoPagos : DAL_Base
    {
        ...
        public List<DTO_ccCreditoPagos> DAL_ccCreditoPagos_GetByNumDocCredito(int NumeroDoc)
        {
            try
            {
                ...
                mySqlCommand.CommandText =
                    ...
                    " ctrl.PrefijoID, ctrl.DocumentoNro, ctrl.FechaDoc as FechaPago,Case When ctrl.DocumentoID = 166 Then rec.FechaAplica Else rec.FechaConsignacion End as FechaConsigna, rec.CajaID, rec.BancoCuentaID, pag.* " +
                ...
                while (dr.Read())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dr["FechaConsigna"].ToString()))
                        ...
                        r.FechaConsigna.Value = r.FechaPago.Value;
        ...
        /// <summary>
        /// Elimina los pagos por documento
        /// </summary>
        /// <returns></returns>
        public void DAL_ccCreditoPagos_Delete(int numeroDoc)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText = " Delete from ccCreditoPagos WHERE numeroDoc =  @numeroDoc";
            ...
            {
                var exception = new Exception(DictionaryMessages.Err_GettingData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_ccCreditoPagos_Delete");
                throw exception;
            }
        }
        ...
        public long DAL_ccCreditoPagos_CountByNumDocCredito(int NumeroDoc, DateTime? fecha)
        {
            try
            {
                ...
                {
                    mySqlCommand.CommandText = "SELECT Isnull(sum(Valor),0) FROM ccCreditoPagos with(nolock) WHERE NumeroDoc = @NumeroDoc";
                }
