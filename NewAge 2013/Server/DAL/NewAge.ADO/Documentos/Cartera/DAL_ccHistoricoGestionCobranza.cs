    ...
    public class DAL_ccHistoricoGestionCobranza : DAL_Base
    {
        ...
        public void DAL_ccHistoricoGestionCobranza_Add(DTO_ccHistoricoGestionCobranza historia)
        {
            try
            {
                ...
                mySqlCommandSel.Parameters.Add("@SiniestroEstadoID", SqlDbType.Char, UDT_CodigoGrl5.MaxLength);
                ...
                mySqlCommandSel.Parameters.Add("@FechaControl", SqlDbType.SmallDateTime);
                ...
                mySqlCommandSel.Parameters["@CobranzaEstadoID"].Value = historia.CobranzaEstadoID.Value;
                mySqlCommandSel.Parameters["@SiniestroEstadoID"].Value = historia.SiniestroEstadoID.Value;
                ...
                mySqlCommandSel.Parameters["@FechaControl"].Value = historia.FechaControl.Value;
                mySqlCommandSel.Parameters["@VlrCuota"].Value = historia.VlrCuota.Value;
                mySqlCommandSel.Parameters["@Dato1"].Value = historia.Dato1.Value;
                mySqlCommandSel.Parameters["@eg_ccCobranzaGestion"].Value = this.GetMaestraEmpresaGrupoByDocumentID(AppMasters.ccCobranzaGestion, this.Empresa, egCtrl);                mySqlCommandSel.Parameters["@eg_ccCobranzaGestion"].Value = this.GetMaestraEmpresaGrupoByDocumentID(AppMasters.ccCobranzaGestion, this.Empresa, egCtrl);
        ...
        public void DAL_ccHistoricoGestionCobranza_Update(List<DTO_ccHistoricoGestionCobranza> data)
        {
            try
            {
                ...
                foreach (var g in data)
                {
                    ...
                    mySqlCommandSel.Parameters["@CobranzaEstadoID"].Value = g.CobranzaEstadoID.Value;
                    mySqlCommandSel.Parameters["@SiniestroEstadoID"].Value = g.SiniestroEstadoID.Value;
        ...
        public DataTable DAL_ccHistoricoGestionCobranza_GetExcel(DateTime fechaCorte, string gestionCobrID,  byte? estado, byte? tipoGestion, string centroCosto)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText =
		                ...
		                "        ciu.Descriptivo as Ciudad, dep.Descriptivo as Depto,  " +
                        "        cli.NomEsposa as Nombre, cli.cedEsposa as Documento, ter.Direccion as ResidenciaDir, ter.Tel1 as Telefono,ter.Celular1,ter.CECorporativo as Mail,  " +
                        ...
                        "        left join coTercero ter on cli.CedEsposa = ter.TerceroID   " +
                        "        left join ccCobranzaGestion ges on his.CobranzaGestionID = ges.CobranzaGestionID   " +
		                "        left join glLugarGeografico ciu on cli.ResidenciaCiudad = ciu.LugarGeograficoID and cli.eg_glLugarGeografico = ciu.EmpresaGrupoID " +
		                "        left join glLugarGeografico dep on left(cli.ResidenciaCiudad,2) = dep.LugarGeograficoID and cli.eg_glLugarGeografico = dep.EmpresaGrupoID " +
        ...
        public List<DTO_ccHistoricoGestionCobranza> DAL_ccHistoricoGestionCobranza_GetGestion(DateTime fechaCorte, string gestionCobrID, byte? estado, byte? tipoGestion,string centroCosto)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText =
                        ...
                        "       ges.PlantillaCarta,ges.PlantillaEMail,ges.Referencia,cli.ResidenciaDir as Direccion,lg.Descriptivo as Ciudad,ges.Mensaje,ges.CampID,cli.Celular1 as Telefono  " +
                        "     from ccHistoricoGestionCobranza his " +
                        "      left join ccCreditoDocu crd on his.numerodoc = crd.NumeroDoc " +
                        "      left join glDocumentoControl ctrl on ctrl.NumeroDoc = crd.NumeroDoc   " +
                        ...
                        "       his.NumeroDoc,his.Consecutivo,his.CodConfirmacion,his.FechaConfirmacion,his.FechaControl,his.VlrCuota,his.Dato1, " +
                        "       ges.PlantillaCarta,ges.PlantillaEMail,ges.Referencia,ter.Direccion,lg.Descriptivo as Ciudad,ges.Mensaje,ges.CampID,ter.Celular1 as Telefono  " + 
                        "     from ccHistoricoGestionCobranza his " +
                        "      left join ccCreditoDocu crd on his.numerodoc = crd.NumeroDoc " +
                        "      left join glDocumentoControl ctrl on ctrl.NumeroDoc = crd.NumeroDoc   " +
                        "      left join ccCliente cli on crd.ClienteID = cli.ClienteID and cli.EmpresaGrupoID = crd.eg_ccCliente " +
                ...
                while (dr.Read())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dr["CampID"].ToString()))
                        r.CampID.Value = Convert.ToInt32(dr["CampID"]);
                    else
                        r.CampID.Value = 0;
                    results.Add(r);
                }
        ...
        /// <summary>
          /// Trae una historia de un credito
          /// </summary>
          /// <param name="numDocCredito">doc del credito</param>
          /// <param name="gestionCobrID">gestion actual del credito</param>
          /// <returns></returns>
        public DTO_ccHistoricoGestionCobranza DAL_ccHistoricoGestionCobranza_GetLastByCredito(int numDocCredito, string gestionCobrID)
        {
            try
            {
                DTO_ccHistoricoGestionCobranza result = new DTO_ccHistoricoGestionCobranza();
                ...
                #region CommanText
                mySqlCommandSel.CommandText =
                        " SELECT Top(1) * FROM ccHistoricoGestionCobranza where NumeroDoc = @numDocCredito and CobranzaGestionID = @CobranzaGestionID  " +
            ...
            catch (Exception ex)
            {
                //Log error
                var exception = new Exception(DictionaryMessages.Err_GettingDocument, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_ccHistoricoGestionCobranza_GetLastByCredito");
                return null;
