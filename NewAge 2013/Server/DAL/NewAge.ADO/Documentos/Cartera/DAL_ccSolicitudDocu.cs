...
namespace NewAge.ADO
{
    public class DAL_ccSolicitudDocu : DAL_Base
    {
            ...
            try
            {
                ...
                mySqlCommand.CommandText =
                  ...
                  "	INNER JOIN glDocumentoControl ctrl with(nolock) ON ctrl.NumeroDoc = soli.NumeroDoc " +
                    "WHERE soli.EmpresaID = @EmpresaID and ctrl.DocumentoID = 160 and ctrl.Estado not in(0,4,3) and soli.Libranza = @Libranza and soli.Poliza = @Poliza " +
        ...
        public List<DTO_SolicitudAprobacionCartera> DAL_ccSolicitudDocu_GetForVerificacion(int documentoID, string actFlujoID, string usuarioID)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    if(string.IsNullOrEmpty(dto.ClienteID.Value))
                        dto.ClienteID.Value = dr["ClienteRadica"].ToString();
        ...
        public List<DTO_ccSolicitudDocu> DAL_ccSolicitudDocu_GetByActividad(string actFlujoID, string usuarioID)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText =
                    ...
                    "WHERE ctrl.EmpresaID = @EmpresaID and ctrl.Estado not in(0,4) AND perm.ActividadFlujoID = @ActividadFlujoID --AND doc.ModuloID = @ModuloID";
                #endregion
        ...
        public List<DTO_SolicitudGestion> DAL_ccSolicitudDocu_GetGestionSolicitud(string usuarioID)
        {
            try
            {
                List<DTO_SolicitudGestion> result = new List<DTO_SolicitudGestion>();
                ...
                mySqlCommandSel.CommandText =
                                        "select ctrl.NumeroDoc,Flujo.DocumentoID,ctrl.Fecha, docu.Libranza," +
                                        "   docu.ClienteID,docu.ClienteRadica, clie.Descriptivo as Nombre,docu.ApellidoPri,docu.ApellidoSdo,docu.NombrePri,docu.NombreSdo, " +                                       
                                        ...
                                        "	flujo.Descriptivo as Etapa " +
                                        "   from ccSolicitudDocu Docu with(nolock)" +
                                        "	    inner join glDocumentoControl ctrl with(nolock) on ctrl.NumeroDoc=docu.NumeroDoc " +
                                        "	    left join ccCliente Clie with(nolock) on clie.ClienteID=docu.ClienteID and docu.eg_ccCliente = CLIE.EmpresaGrupoID " +
                                        "	    inner join ccPagaduria pag on pag.PagaduriaID=docu.PagaduriaID AND DOCU.eg_ccPagaduria = pag.EmpresaGrupoID " +
                                        "	    inner join glZona Zona on zona.ZonaID=docu.ZonaID  AND DOCU.eg_glZona = zona.EmpresaGrupoID " +
                                        "	    inner join ccTipoCredito tipcre on docu.TipoCreditoID = tipcre.TipoCreditoID AND DOCU.eg_ccTipoCredito = tipcre.EmpresaGrupoID " +
                                        ...
                                        "	    inner join glActividadEstado Act with(nolock) on docu.NumeroDoc= act.NumeroDoc" +
                                        "	    inner join glActividadPermiso perm with(nolock) ON perm.EmpresaGrupoID = ctrl.EmpresaID " +
                                        ...
                                        "	    inner join glActividadFlujo Flujo with(nolock) on act.ActividadFlujoID=flujo.ActividadFlujoID and act.eg_glActividadFlujo=Flujo.EmpresaGrupoID" +
                                        "	where ctrl.EmpresaID=@empresaID and ctrl.Estado in(1,2) and ctrl.DocumentoID = 160 and Act.CerradoInd=0 and TipCre.TipoCredito in(1,2,4,5) " +
                                        "   order by ctrl.FechaDoc desc";
                ...
                while (dr.Read())
                {
                    DTO_SolicitudGestion datos = new DTO_SolicitudGestion(dr);
                    if (string.IsNullOrWhiteSpace(datos.ClienteID.Value))
                        datos.ClienteID.Value = dr["ClienteRadica"].ToString().TrimEnd();
