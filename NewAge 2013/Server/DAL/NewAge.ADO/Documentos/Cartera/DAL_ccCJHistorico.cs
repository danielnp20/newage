    ...
    public class DAL_ccCJHistorico : DAL_Base
    {
        ...
        public int DAL_ccCJHistorico_Add(DTO_ccCJHistorico historico)
        {
            try
            {
                ...
                mySqlCommandSel.Parameters.Add("@VlrInforme", SqlDbType.Decimal);
                mySqlCommandSel.Parameters.Add("@VlrJuzgado", SqlDbType.Decimal);
                ...
                mySqlCommandSel.Parameters["@Orden"].Value = historico.Orden.Value;
                mySqlCommandSel.Parameters["@VlrInforme"].Value = historico.VlrInforme.Value;
                mySqlCommandSel.Parameters["@VlrJuzgado"].Value = historico.VlrJuzgado.Value;
        ...
        public bool DAL_ccCJHistorico_Update(DTO_ccCJHistorico historico)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText =
                    ...
                    " ,FijadoInd = @FijadoInd " +
                    " ,Orden = @Orden " +
                    " ,VlrInforme = @VlrInforme " +
                    " ,VlrJuzgado = @VlrJuzgado " +
        ...
        /// <summary>
        /// Trae la info de un historico del ultimo abono del credito sin filtros
        /// </summary>
        /// <param name="numDocCredito">Numero doc del crédito</param>
        /// <returns>retorna los 2 ultimos registros del histórico de CJ</returns>
        public Tuple<DTO_ccCJHistorico, DTO_ccCJHistorico> DAL_ccCJHistorico_GetLastAbono(int numDocCredito)
        {
            try
            {
                DTO_ccCJHistorico cjCapital = null;
                ...
                mySqlCommand.CommandText =  "SELECT TOP(1) * " +
                                "FROM ccCJHistorico cj with(nolock) " +
                                "WHERE cj.EmpresaID = @EmpresaID and cj.NumeroDoc = @NumDocCredito and ClaseDeuda = @ClaseDeuda " +
                                "Order by Consecutivo desc";
            ...
            {
                var exception = new Exception(DictionaryMessages.Err_GettingData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_ccCJHistorico_GetLastAbono");
                throw exception;
            }
        }
