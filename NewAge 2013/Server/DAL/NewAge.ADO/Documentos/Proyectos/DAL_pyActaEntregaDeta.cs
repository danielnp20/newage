...
namespace NewAge.ADO
{
    public class DAL_pyActaEntregaDeta : DAL_Base
    {
        ...
        /// <returns></returns>
        public List<DTO_pyActaEntregaDeta> DAL_pyActaEntregaDeta_Get(int? numeroDoc, int? numeroDocProy, int? consTareaCliente, int? consTareaEntrega,bool onlyAprobadas)
        {
            try
            {
                ...
                mySqlCommand.CommandText = "Select tarea.TareaEntregable, tarea.Descripcion, det.* " + 
                                           " from  pyActaEntregaDeta det " +
                                           " Inner join pyProyectoTareaCliente tarea with(nolock) on tarea.Consecutivo = det.ConsTareaCliente " +
                                           " inner join gldocumentocontrol ctrl with(nolock) on ctrl.NumeroDoc = det.NumeroDoc " +  query;
        ...
        public List<DTO_pyActaEntregaDeta> DAL_pyActaEntregaDeta_GetByParameter(DTO_pyActaEntregaDeta filter)
        {
            try
            {
                ...
                query = "Select acta.*, tarea.TareaEntregable,tarea.Descripcion,ctrl.DocumentoNro,ctrl.Fecha,ctrl.Estado,IsNull(ctrl.ConsSaldo,1) as ConsSaldo,IsNull(entr.PorEntrega,0) as PorEntrega " +
                        ...
                        " inner join pyProyectoPlanEntrega entr with(nolock) on entr.Consecutivo = acta.ConsTareaEntrega " +
                        "where NumDocProyecto = @NumDocProyecto " + query;
                ...
                while (dr.Read())
                {
                    DTO_pyActaEntregaDeta act = new DTO_pyActaEntregaDeta(dr);
                    act.DocumentoNro.Value = Convert.ToInt32(dr["DocumentoNro"]);
                    act.Estado.Value = Convert.ToInt16(dr["Estado"]);
                    act.ConsActaProy.Value = Convert.ToInt32(dr["ConsSaldo"]);
                    act.PorEntregaProg.Value = Convert.ToDecimal(dr["PorEntrega"]);
                    results.Add(act);
                    index++;
                }
                dr.Close();
        ...
        /// <param name="consecutivo">id</param>
        /// <returns>retorna idenfiticador</returns>
        public void DAL_pyActaEntregaDeta_DeleteByNumeroDoc(int numeroDocActa)
        {
            try
            {
                string egCtrl = StaticMethods.GetGrupoEmpresasControl(base.MySqlConnection, base.MySqlConnectionTx, this.loggerConnectionStr);
