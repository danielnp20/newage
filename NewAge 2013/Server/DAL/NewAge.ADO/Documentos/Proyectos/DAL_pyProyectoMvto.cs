...
namespace NewAge.ADO
{
    public class DAL_pyProyectoMvto : DAL_Base
    {
        ...
        public List<DTO_pyProyectoMvto> DAL_pyProyectoMvto_GetByParameter(DTO_pyProyectoMvto filter)
        {
            try
            {
                ...
                query = "select mvto.*, deta.RecursoID,deta.FactorID,tarea.TareaCliente,tarea.Descriptivo as TareaDesc,tarea.TareaID,tarea.Cantidad as CantidadTarea,tarea.Consecutivo as ConsecTarea, " +
                        "       rec.RecursoID,rec.Descriptivo as RecursoDesc,rec.TipoRecurso,rec.UnidadInvID,rec.inReferenciaID,rec.CodigoBSID,ref.RefProveedor,ref.MarcaInvID  " +
                ...
                while (dr.Read())
                {
                    ...
                    mvto.ConsecTarea.Value = Convert.ToInt32(dr["ConsecTarea"]);
        ...
        /// <summary>
        ///  Trae informacion de la ultima version de un recurso
        /// </summary>
        /// <param name="recursoID"> Nombre del recursoID</param>
        /// <param name="numDocProy">numero Doc del proyecto</param>
        /// <param name="consecTarea">Consecutivo de la tarea</param>
        /// <returns></returns>
        public int DAL_pyProyectoMvto_GetLastVersionByRecurso(string recursoID, int numDocProy, int? consecTarea)
        {
            try
            {
                int consecMVto = 0;
                ...
                SqlCommand mySqlCommand = base.MySqlConnection.CreateCommand();
                mySqlCommand.Transaction = base.MySqlConnectionTx;
                mySqlCommand.CommandText = " SELECT  mvto.Consecutivo " +
                    " FROM pyProyectoMvto mvto with(nolock)   " +
                    "    inner join pyProyectoDeta d on d.Consecutivo = mvto.ConsecDeta  " +
                    " WHERE d.RecursoID =  @RecursoID  and d.NumeroDoc = @NumeroDoc and d.ConsecTarea = @ConsecTarea" +
                    " Order by mvto.Consecutivo desc ";

                mySqlCommand.Parameters.Add("@RecursoID", SqlDbType.Char, UDT_RecursoID.MaxLength);
                mySqlCommand.Parameters.Add("@NumeroDoc", SqlDbType.Int);
                mySqlCommand.Parameters.Add("@ConsecTarea", SqlDbType.Int);
                mySqlCommand.Parameters["@RecursoID"].Value = recursoID;
                mySqlCommand.Parameters["@NumeroDoc"].Value = numDocProy;
                ...
                if (dr.Read())
                {
                    consecMVto = dr["Consecutivo"] != null ? Convert.ToInt32(dr["Consecutivo"]) : 0;
                }
                dr.Close();
