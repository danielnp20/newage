...
namespace NewAge.ADO
{
    public class DAL_pyProyectoPlanEntrega : DAL_Base
    {
        ...
        /// <summary>
        /// Elimina información de las tareas por tarea entrega
        /// </summary>
        /// <param name="consecutivo">id</param>
        /// <returns>retorna idenfiticador</returns>
        public void DAL_pyProyectoPlanEntrega_DeleteByConsecTarea(int? consecTarea)
        {
            try
            {
                string egCtrl = StaticMethods.GetGrupoEmpresasControl(base.MySqlConnection, base.MySqlConnectionTx, this.loggerConnectionStr);
            ...
            catch (Exception ex)
            {
                //Log error
                var exception = new Exception(DictionaryMessages.Err_AddData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_pyProyectoPlanEntrega_DeleteByConsecTarea");
                throw exception;
            }
        }
