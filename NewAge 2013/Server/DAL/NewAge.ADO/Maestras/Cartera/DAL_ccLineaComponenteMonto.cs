...
{
    public class DAL_ccLineaComponenteMonto : DAL_Base
    {
        /// <summary>
        ...
        public DAL_ccLineaComponenteMonto(SqlConnection c, SqlTransaction tx, DTO_glEmpresa empresa, int userId, string loggerConn) : base(c, tx, empresa, userId, loggerConn) { }

        public DTO_ccLineaComponenteMonto DAL_ccLineaComponenteMonto_GetByMonto(string lineaCreID, string componenteID, int monto)
        {
            try
            {
                DTO_ccLineaComponenteMonto result = null;
                ...
                mySqlCommand.Parameters["@EmpresaGrupoID"].Value = this.GetMaestraEmpresaGrupoByDocumentID(AppMasters.ccLineaComponenteMonto, this.Empresa, egCtrl);
                ...
                mySqlCommand.CommandText =
                    "select top(1) * from ccLineaComponenteMonto with(nolock) " +
                    "where LineaCreditoID = @LineaCreditoID and ComponenteCarteraID = @ComponenteCarteraID and Monto >= @Monto " +
                ...
                if(dr.Read())
                {
                    result = new DTO_ccLineaComponenteMonto();
            ...
            catch (Exception ex)
            {
                ...
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "DAL_ccLineaComponenteMonto_GetByMonto");
                throw exception;
            }
        }
    }
