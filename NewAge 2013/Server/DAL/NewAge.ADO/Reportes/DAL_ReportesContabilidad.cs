    ...
    public class DAL_ReportesContabilidad : DAL_Base
    {
        ...
        public List<DTO_ReportBalanceComparativo> DAL_ReportesContabilidad_BalanceComparativo(string libroFUNC, string libroAux, DateTime fechaFin, DateTime fechaIni, int año)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText =
                    ...
                    "            and DATEPART(MONTH, PeriodoID)=@fechaInic and DATEPART(YEAR, PeriodoID) = @año and DATEPART(DAY, PeriodoID) = @Dia " +
                ...
                mySqlCommandSel.Parameters.Add("@año", SqlDbType.Int);
                mySqlCommandSel.Parameters.Add("@Dia", SqlDbType.Int);
                ...
                mySqlCommandSel.Parameters["@año"].Value = año;
                mySqlCommandSel.Parameters["@Dia"].Value = fechaFin.Day;
        ...
        public List<DTO_ReportInventariosBalance> DAL_ReportesContabilidad_InventariosBalance(int mesIni, int mesFin, string libro, string cuentaIni, string cuentaFin, int _año)
        {
            try
            {
                ...
                //                "       		AND day(bal.PeriodoID)		=	1     " + // Se coloca filtro para que no tome el cierre
                ...
                mySqlCommandSel.CommandText =
                          ...
                          "                        		TerceroID, "+
                          "                        		TerceroDesc, "+
                          "                        		@Libro			as BalanceTipoID,"+
                          "                        		sum(DebitoML_Cuenta) DebitoML_Cuenta, "+
                          "                        		sum(CreditoML_Cuenta) CreditoML_Cuenta,"+
                          "                        		sum(InicialML_Cuenta) InicialML_Cuenta,"+
                          "                        		sum(InicialML_Cuenta + DebitoML_Cuenta - CreditoML_Cuenta) FinalML_Cuenta,"+
                          "                        		sum(DebitoML_Tercero) DebitoML_Tercero,"+
                          "                        		sum(CreditoML_Tercero) CreditoML_Tercero,"+   
                          "                        		sum(InicialML_Tercero) InicialML_Tercero,"+
                          "                        		sum(InicialML_Tercero + DebitoML_Tercero - CreditoML_Tercero) FinalML_Tercero,"+
                          "                        		cta.TerceroInd "+
                          "                        FROM"+ 
                          "                        	("+
                          "                        	SELECT	cuentaID,"+
                          "                        			NULL as TerceroID,"+
                          "                        			NULL as TerceroDesc,"+
                          "                        			SUM((DbOrigenLocML + DbOrigenExtML)) DebitoML_Cuenta,"+
                          "                        			SUM((CrOrigenLocML + CrOrigenExtML))*(-1) CreditoML_Cuenta,"+
                          "                        			CASE When Month(PeriodoID) = @MonthIni Then SUM((DbSaldoIniLocML + DbSaldoIniExtML + CrSaldoIniLocML + CrSaldoIniExtML)) Else 0 End  as InicialML_Cuenta,"+
                          "                        			0 as DebitoML_Tercero,"+
                          "                        			0 as CreditoML_Tercero, "+
                          "                        			0 as InicialML_Tercero"+
                          "                        	FROM	coBalance"+
                          "                        	WHERE	EmpresaID				=  @EmpresaID"+
                          "                        			AND Month(PeriodoID)	>= @MonthIni"+ 
                          "                        			AND Month(PeriodoID)	<= @MonthFin"+    
                          ...
                          "                        			and day(PeriodoID)		=  1"+
                          "                        			AND BalanceTipoID		= @Libro"+
                          "                        			AND CuentaID = (CASE WHEN @CuentaIni != '' THEN @CuentaIni ELSE CuentaID END)"+
                          "                        			AND CuentaID = (CASE WHEN @CuentaFin != '' THEN @CuentaFin ELSE CuentaID END)"+
                          "                        	GROUP BY PeriodoID,	CuentaID, eg_coPlanCuenta"+
                          "                        	UNION"+
                          "                        	SELECT	cuentaID,"+
                          "                        			ctaSaldo.TerceroID,"+
                          "                        			ter.Descriptivo as TerceroDesc,"+
                          "                        			0 as DebitoML_Cuenta,"+
                          "                        			0 as CreditoML_Cuenta,"+
                          "                        			0 asInicialML_Cuenta,"+
                          "                        			SUM((ctaSaldo.DbOrigenLocML + ctaSaldo.DbOrigenExtML)) DebitoML_Tercero,"+
                          "                        			SUM((ctaSaldo.CrOrigenLocML + ctaSaldo.CrOrigenExtML))*(-1) CreditoML_Tercero,"+
                          "                        				CASE When Month(ctaSaldo.PeriodoID) = @MonthIni Then SUM((ctaSaldo.DbSaldoIniLocML + ctaSaldo.DbSaldoIniExtML + ctaSaldo.CrSaldoIniLocML + ctaSaldo.CrSaldoIniExtML)) Else 0 End  as InicialML_Tercero"+
                          "                        	FROM	coCuentaSaldo ctaSaldo"+
                          ...
                          "                        				ter.Descriptivo"+
                          "                        	) Sdo"+
                          "                        	LEFT JOIN coPlanCuenta cta WITH(NOLOCK) on  cta.cuentaID = sdo.CuentaID "+
                          "                        WHERE	cta.EmpresaGrupoID = @EmpresaID and"+
                          "                        		(TerceroID IS NULL"+
                          "                        		OR	(DebitoML_Tercero)	!= 0      "+
                          "                        		OR	(CreditoML_Tercero) != 0 "+
                          "                        		OR	(InicialML_Tercero) != 0)"+
                          "                        GROUP BY sdo.cuentaID, cta.Descriptivo, TerceroID, TerceroDesc, TerceroInd"+
                          "                        ORDER BY sdo.cuentaID, CAST(TerceroID AS BIGINT)";
        ...
        /// <summary>
        /// Obtiene un datatable con la info de Contabilidad segun filtros
        /// </summary>
            ...
            try
            {
                ...
                if (documentoID == AppReports.coSaldos)
                {
                    if (tipoReporte==4)
                    {
                        ...
                        mySqlCommandSel.CommandText =
                           " Select sdo.CuentaID, sdo.NomCuenta as CuentaDesc, sdo.Saldoini as SaldoInicial,"+
                           " sdo.Movim, sdo.SaldoFIN as SaldoFinal," +
                           "  (case when ter.TerceroDocTipoID = 'CC'  then 13" +
                           "      when ter.TerceroDocTipoID = 'NIT' then 31" +
                           "      else 43 end) as TipoIdentificacion," +
                           "    sdo.TerceroID," +
                           "   (case when ter.DigitoVerif is null then '' else ter.DigitoVerif end) as DigitoVerif," +
                           "   (case when ter.TerceroDocTipoID = 'CC' then (case when ApellidoPri is null then '' else ApellidoPri end) else '' end) as PrimerApellido," +
                           "   (case when ter.TerceroDocTipoID = 'CC' then (case when ApellidoSdo is null then '' else ApellidoSdo end) else '' end) as SegundoApellido," +
                           "   (case when ter.TerceroDocTipoID = 'CC' then (case when NombrePri is null   then '' else NombrePri   end) else '' end) as PrimerNombre," +
                           "   (case when ter.TerceroDocTipoID = 'CC' then (case when NombreSdo is null   then '' else NombreSdo   end) else '' end) as SegundoNombre," +
                           "   (case when ter.TerceroDocTipoID <> 'CC' then Descriptivo else '' end) as RazonSocial," +
                           "   Ter.Direccion," +
                           "   left(ter.LugarGeograficoID,2) as Depto," +
                           "   Right(rtrim(ter.LugarGeograficoID),3) as Municipio," +
                           "   169 as Pais," +
                           "   (case when ter.ceCorporativo is null then '' else ter.ceCorporativo end) as Correo," +
                           "   (case when ter.celular1 is null   then '' else ter.celular1 end) as celular1 , " +
                           "   (case when ter.Telefono1 is null  then '' else ter.Telefono1 end) as Telefono1" +
                           " from " +
                           "  (" +
                           "  Select sdo.Cuentaid, puc.Descriptivo as NomCuenta," +
                           "    (case when puc.TerceroSaldosInd = 0 then @NitEmpresa else sdo.TerceroID end) as TerceroID  , " +
                           "    sum((case when sdo.PeriodoID= @FechaIni then sdo.dbSaldoIniLocML+sdo.crSaldoIniLocML else 0 end)) as SaldoINI," +
                           "    sum(sdo.DbOrigenLocML+sdo.crOrigenLocML) as Movim," +
                           "    sum((case when sdo.PeriodoID= @FechaFin then sdo.dbSaldoIniLocML+sdo.crSaldoIniLocML+sdo.DbOrigenLocML+sdo.crOrigenLocML else 0 end)) as SaldoFIN" +
                           "  from coCuentaSaldo sdo with (nolock)" +
                           "   left join coPlanCuenta puc with (nolock) on sdo.CuentaID = puc.CuentaID and sdo.eg_coPlanCuenta = puc.EmpresaGrupoID" +
                           "  where sdo.PeriodoID >= @FechaIni and sdo.PeriodoID <= @FechaFin  and sdo.EmpresaID = @EmpresaID" +
                           "  group by sdo.Cuentaid, puc.Descriptivo, sdo.TerceroID,puc.TerceroSaldosInd, puc.Naturaleza" +
                            " ) Sdo" +
                           "  left join coTercero ter on sdo.TerceroID = ter.TerceroID" +
                           " where (sdo.Movim <> 0 or sdo.SaldoFIN <> 0)and ter.EmpresaGrupoID = @EmpresaID" +
                           " order by sdo.Cuentaid, sdo.TerceroID";
                        // whereAll 
                        ...
                        mySqlCommandSel.Parameters.Add("@libro", SqlDbType.Char);
                        mySqlCommandSel.Parameters.Add("@EmpresaID", SqlDbType.Char, UDT_EmpresaID.MaxLength);
                        mySqlCommandSel.Parameters.Add("@NitEmpresa", SqlDbType.Char, UDT_EmpresaID.MaxLength);
                        ...
                        mySqlCommandSel.Parameters["@libro"].Value = balanceTipo;
                        mySqlCommandSel.Parameters["@EmpresaID"].Value = this.Empresa.ID.Value;
                        mySqlCommandSel.Parameters["@NitEmpresa"].Value = terceroID;
                        mySqlCommandSel.Parameters["@cuentaIni"].Value = cuentaID;
                        mySqlCommandSel.Parameters["@CuentaFin"].Value = otroFilter.ToString();
                ...
                #region Medios Magneticos
                if (documentoID == AppReports.coMediosMagneticos)
                {
                    if (tipoReporte == 1)
                    {
                        #region Filtros
                        ...
                        mySqlCommandSel.CommandText =
                           ...
                           "  from coCuentaSaldo sdo with (nolock)" +
                           "   left join coPlanCuenta puc with (nolock) on sdo.CuentaID = puc.CuentaID and sdo.eg_coPlanCuenta = puc.EmpresaGrupoID" +
                           "  where sdo.PeriodoID >= @FechaIni and sdo.PeriodoID <= @FechaFin  and sdo.EmpresaID = @EmpresaID and sdo.BalanceTipoID=@libro " +
                           "  group by sdo.Cuentaid, puc.Descriptivo, sdo.TerceroID,puc.TerceroSaldosInd, puc.Naturaleza" +
                            " ) Sdo" +
                           "  left join coTercero ter on sdo.TerceroID = ter.TerceroID" +
                           " where (sdo.Movim <> 0 or sdo.SaldoFIN <> 0)and ter.EmpresaGrupoID = @EmpresaID" +
                    ...
                    else if (tipoReporte == 2)
                    {
                        ...
                        mySqlCommandSel.CommandText =
                           " Select (case when ter.TerceroDocTipoID = 'CC'  then 13" +
                           "      when ter.TerceroDocTipoID = 'NIT' then 31" +
                           "      else 43 end) as TipoIdentificacion," +
                           " 	sal.Tercero as TerceroID," +
                           "   (case when ter.DigitoVerif is null then '' else ter.DigitoVerif end) as DigitoVerif," +
                           "   (case when ter.TerceroDocTipoID = 'CC' then (case when ApellidoPri is null then '' else ApellidoPri end) else '' end) as PrimerApellido," +
                           "   (case when ter.TerceroDocTipoID = 'CC' then (case when ApellidoSdo is null then '' else ApellidoSdo end) else '' end) as SegundoApellido," +
                           "   (case when ter.TerceroDocTipoID = 'CC' then (case when NombrePri is null   then '' else NombrePri   end) else '' end) as PrimerNombre," +
                           ...
                           "   (case when ter.celular1 is null   then '' else ter.celular1 end) as celular1 , " +
                           "   (case when ter.Telefono1 is null  then '' else ter.Telefono1 end) as Telefono1, " +
                           "   sal.SaldoINI as SaldoInicial, sal.Debitos as DebitoML, sal.Creditos as CreditoML, sal.SaldoFIN as SaldoFinal" +
                           " from coTercero ter" +
                           " inner join " +
                           " (" +
                           "     Select TerceroID, CAST(TerceroID as bigint) as Tercero," +
                           "     sum((case when PeriodoID= @FechaIni  then dbSaldoIniLocML+crSaldoIniLocML else 0 end)) as SaldoINI," +
                           "     sum(DbOrigenLocML) as Debitos," +
                           "     SUM(crOrigenLocML) as Creditos," +
                           "     sum((case when PeriodoID= @FechaFin then dbSaldoIniLocML+crSaldoIniLocML+DbOrigenLocML+crOrigenLocML else 0 end)) as SaldoFIN" +
                           "   from coCuentaSaldo" +
                           "   where year(PeriodoID) = YEAR(@FechaFin) and EmpresaID = @EmpresaID and BalanceTipoID=@libro" +
                           "   group by TerceroID" +
                           "   ) sal on ter.TerceroID = sal.TerceroID" +
                           " where ter.EmpresaGrupoID = @EmpresaID" +
                           " order by SAL.TerceroID";
                           // whereAll 
...
//--DECLARE @BalanceTipo		varchar(2)	='01'
                    ...
                    //   "    				and ( b.BalanceTipoID = @BalanceTipo ) " +
                    //   "    				and c.tipo = 2" +
                    //   "    				and  ((@Proyecto is null) or (b.ProyectoID=@Proyecto)) " +
                    ...
                    "    set  @PerFin			= (case when @MesFin<=12 then @MesFin else 12 end) * 100 + (case when @MesFin<=12 then 1 else 2 end)" +
                     ...
                     "  ("+
                     "  	select	"+
                     "  		CuentaID,CuentaDesc,LEN(CuentaID) LongitudCta,"+
                     "  		titulo,Moneda,Codigo1,Codigo1Desc,Codigo2,Codigo2Desc,"+
                     "  		cast(Signo * round(sum(Case when Mes = 1  then Movimiento else 0 end),0)as Bigint) as Mov01,"+
                     "  		cast(Signo * round(sum(Case when Mes = 2  then Movimiento else 0 end),0)as Bigint)  as Mov02,"+
                     "  		cast(Signo * round(sum(Case when Mes = 3  then Movimiento else 0 end),0)as Bigint)  as Mov03,"+
                     "  		cast(Signo * round(sum(Case when Mes = 4  then Movimiento else 0 end),0)as Bigint)  as Mov04,"+
                     "  		cast(Signo * round(sum(Case when Mes = 5  then Movimiento else 0 end),0)as Bigint)  as Mov05,"+
                     "  		cast(Signo * round(sum(Case when Mes = 6  then Movimiento else 0 end),0)as Bigint)  as Mov06,"+
                     "  		cast(Signo * round(sum(Case when Mes = 7  then Movimiento else 0 end),0)as Bigint)  as Mov07,"+
                     "  		cast(Signo * round(sum(Case when Mes = 8  then Movimiento else 0 end),0)as Bigint)  as Mov08,"+
                     "  		cast(Signo * round(sum(Case when Mes = 9  then Movimiento else 0 end),0)as Bigint)  as Mov09,"+
                     "  		cast(Signo * round(sum(Case when Mes = 10 then Movimiento else 0 end),0)as Bigint)  as Mov10,"+
                     "  		cast(Signo * round(sum(Case when Mes = 11 then Movimiento else 0 end),0)as Bigint)  as Mov11,"+
                     "  		cast(Signo * round(sum(Case when Mes = 12 then Movimiento else 0 end),0)as Bigint)  as Mov12,"+
                     "  		cast(Signo * round(sum(Movimiento),0)as Bigint)   as Movimiento"+
                     "  	from "+
                     "  		("+
                     "  		SELECT  (CASE WHEN LEN(b.CuentaID)>6 THEN '1' ELSE '0' END) AS NEGRITA,"+
                     "      		(case when @Moneda = 'Local'/*1*/ then 'BALANCE - PESOS' else 'BALANCE DOLARES' end) as Titulo,"+
                     ...
                     "  ("+
                     "  	select "+
                     "  		CuentaID,CuentaDesc,@CuentaLength LongitudCta,"+
                     "  		titulo,Moneda,Codigo1,Codigo1Desc,Codigo2,Codigo2Desc,"+
                     "  		sum(Mov01) as Mov01,"+
                     "  		sum(Mov02) as Mov02,"+
                     "  		sum(Mov03) as Mov03,"+
                     ...
                     "  	select	"+
                     "  		'99' as CuentaID,"+
                     "  		'Total' as CuentaDesc,LEN(CuentaID) LongitudCta,"+
                     "  		titulo,Moneda,Codigo1,Codigo1Desc,Codigo2,Codigo2Desc,"+
                     "  		cast(Signo * round(sum(Case when Mes = 1  then Movimiento else 0 end),0)as Bigint) as Mov01,"+
                     "  		cast(Signo * round(sum(Case when Mes = 2  then Movimiento else 0 end),0)as Bigint)  as Mov02,"+
                     "  		cast(Signo * round(sum(Case when Mes = 3  then Movimiento else 0 end),0)as Bigint)  as Mov03,"+
                     ...
                     "  	WHERE Mov01 <>0 or Mov02 <>0 or Mov03 <>0 or Mov04 <>0 or Mov05 <>0 or Mov06 <>0 or"+
                     "  		Mov07 <>0 or Mov08 <>0 or Mov09 <>0 or Mov10 <>0 or Mov11 <>0 or Mov12 <>0 "+
                     "  	group by CuentaID, CuentaDesc, Titulo, Moneda,"+
                     "      		Codigo1, Codigo1Desc,Codigo2, Codigo2Desc"+
                     "  ) Final3";
                ...
                sda.SelectCommand.Parameters.Add("@Proyecto", SqlDbType.Char, UDT_ProyectoID.MaxLength);
                sda.SelectCommand.Parameters.Add("@CentroCto", SqlDbType.Char, UDT_CentroCostoID.MaxLength);
                ...
                sda.SelectCommand.Parameters["@Proyecto"].Value = proyecto;
                sda.SelectCommand.Parameters["@CentroCto"].Value = centroCto;
                ...
                if (!string.IsNullOrEmpty(QueryBalance))
                {
