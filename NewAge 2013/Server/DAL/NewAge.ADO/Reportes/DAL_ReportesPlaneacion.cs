...
namespace NewAge.ADO
{
    ...
    public class DAL_ReportesPlaneacion : DAL_Base
    {
        ...
        /// <summary>
        /// Consulta para revisar la Ejecucion Presupuestal
        /// </summary>
        /// <returns>Lista de la ejecucion presupuestal</returns>
        public List<DTO_QueryEjecucionPresupuestal> DAL_ReportesPlaneacion_EjecucionPresupuestal(DateTime fechaCorte)
        {
            //// Cambio de Select para la Consulta
            try
            {
                ...
                " EXECUTE Proyectos_EjecucionPresupuestal   @EmpresaID ";
                ...
                while (dr.Read())
                {
                    ...
                    bool nuevo = true;
                    DTO_QueryEjecucionPresupuestal dto = new DTO_QueryEjecucionPresupuestal(dr);
                    List<DTO_QueryEjecucionPresupuestal> list = result.Where(x => ((DTO_QueryEjecucionPresupuestal)x).Proyecto.Value.TrimEnd().Equals(codProyecto)).ToList();
                    if (list.Count > 0)
                    {
                        dto = list.First();
                        nuevo = false;
                    ...
                    {
                        dto = new DTO_QueryEjecucionPresupuestal(dr);
                    }

                    DTO_QueryEjecucionPresupuestalDetalle dtoDet = new DTO_QueryEjecucionPresupuestalDetalle(dr);
        ...
        public List<DTO_QueryIndicadores> DAL_ReportesPlaneacion_Indicadores(DateTime fechaCorte)
        {
            try
            {

                ///Pendiente cambiar procedimiento almacenado OSCAR
                List<DTO_QueryIndicadores> result = new List<DTO_QueryIndicadores>();

                //SqlCommand mySqlCommand = base.MySqlConnection.CreateCommand();
                //mySqlCommand.Transaction = base.MySqlConnectionTx;
                //mySqlCommand.Parameters.Add("@Empresa", SqlDbType.Char, UDT_EmpresaGrupoID.MaxLength);
                //mySqlCommand.Parameters.Add("@Periodo", SqlDbType.DateTime);
                //mySqlCommand.CommandText =
                //" EXECUTE Contabilidad_Indicadores  @Empresa,@Periodo";
                //mySqlCommand.Parameters["@Empresa"].Value = this.Empresa.ID.Value;
                //mySqlCommand.Parameters["@Periodo"].Value = fechaCorte;
                //SqlDataReader dr = mySqlCommand.ExecuteReader();

                SqlCommand mySqlCommandSel = new SqlCommand("Contabilidad_Indicadores", base.MySqlConnection.CreateCommand().Connection);
                mySqlCommandSel.Transaction = base.MySqlConnectionTx;
                mySqlCommandSel.CommandType = CommandType.StoredProcedure;
                mySqlCommandSel.Parameters.Add("@Empresa", SqlDbType.Char, UDT_EmpresaGrupoID.MaxLength);
                mySqlCommandSel.Parameters.Add("@Periodo", SqlDbType.DateTime);
                mySqlCommandSel.Parameters["@Empresa"].Value = this.Empresa.ID.Value;
                mySqlCommandSel.Parameters["@Periodo"].Value = fechaCorte;
                SqlDataReader dr = mySqlCommandSel.ExecuteReader();


                while (dr.Read())
                {
                    var indicador = new DTO_QueryIndicadores(dr);
