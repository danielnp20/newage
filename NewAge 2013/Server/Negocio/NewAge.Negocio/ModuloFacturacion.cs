    ...
    public class ModuloFacturacion : ModuloBase
    {
        ...
        private List<DTO_faFacturacionFooter> faFacturaFooter_Load(DTO_glDocumentoControl docCtrl, DTO_faFacturaDocu header, List<DTO_glMovimientoDeta> movDet)
        {
            try
            {
                ...
                DTO_SolicitudTrabajo proyecto = this._moduloProy.SolicitudProyecto_Load(AppDocuments.Proyecto, string.Empty, null, null, string.Empty, docCtrl.ProyectoID.Value, false, false, false, false, false);
                #endregion
                string terceroPorDefecto = this.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_TerceroXDefecto);
                string tipoFacturaCtaCobro = this.GetControlValueByCompany(ModulesPrefix.fa, AppControl.fa_TipoFacturaCtaCobro);
                foreach (DTO_glMovimientoDeta mvto in movDet)
                {
                    ...
                    filtro.ValorFiltro = mvto.ConceptoCargoID.Value;
                    filtro.OperadorFiltro = OperadorFiltro.Igual;
                    filtro.OperadorSentencia = "and";
                    filtros.Add(filtro);
                    ...
                    _porcIVA = proyecto != null && proyecto.HeaderProyecto.APUIncluyeAIUInd.Value.Value ? 0 : _porcIVA;
                    ...
                    footerItem.ValorIVA = docCtrl.MonedaID.Value == monedaLocal ? (mvto.Valor2LOC.Value == 0? vlrIva : mvto.Valor2LOC.Value.Value) : mvto.Valor2EXT.Value.Value;
                    footerItem.ValorIVA = proyecto != null && proyecto.HeaderProyecto.APUIncluyeAIUInd.Value.Value ? 0 : footerItem.ValorIVA;
                ...
                if(proyecto != null && proyecto.HeaderProyecto.APUIncluyeAIUInd.Value.Value && !header.FacturaTipoID.Value.Equals(tipoFacturaCtaCobro))
                {
                    header.PorcAdministracion = proyecto.HeaderProyecto.PorEmpresaADM.Value.Value;
                    header.PorcImprevistos = proyecto.HeaderProyecto.PorEmpresaIMP.Value.Value;
                    header.PorcUtilidad = proyecto.HeaderProyecto.PorEmpresaUTI.Value.Value;
                    header.Administracion.Value = Math.Round(footer.Sum(x => x.ValorBruto) * (header.PorcAdministracion / 100), 0);
                    header.Imprevistos.Value = Math.Round(footer.Sum(x => x.ValorBruto) * (header.PorcImprevistos / 100), 0);
                    header.Utilidad.Value = Math.Round(footer.Sum(x => x.ValorBruto) * (header.PorcUtilidad / 100), 0);
                    _porcIVAUtilidad = proyecto.HeaderProyecto.PorIVA.Value.HasValue? proyecto.HeaderProyecto.PorIVA.Value.Value : _porcIVAUtilidad;
                    header.Iva.Value = footer.Count > 0 ? Math.Round(header.Utilidad.Value.Value * (_porcIVAUtilidad / 100),0) : 0;
                }
        ...
        private List<DTO_ComprobanteFooter> faComprobanteFooter_Load(DTO_glDocumentoControl ctrl, DTO_faFacturaDocu header, List<DTO_faFacturacionFooter> footer)
        {
            try
            {
                ...
                string terceroPorDefecto = this.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_TerceroXDefecto);
                string tipoFacturaCtaCobro = this.GetControlValueByCompany(ModulesPrefix.fa, AppControl.fa_TipoFacturaCtaCobro);
                DTO_SolicitudTrabajo proyecto = this._moduloProy.SolicitudProyecto_Load(AppDocuments.Proyecto, string.Empty, null, null, string.Empty, ctrl.ProyectoID.Value, false, false, false, false, false);
                ...
                foreach (DTO_faFacturacionFooter footerItem in footer.FindAll(x=>x.ValorBruto > 0))
                {
                    ...
                    if (proyecto != null && proyecto.HeaderProyecto.APUIncluyeAIUInd.Value.Value)
                    {
                        if (cacheCuentaServ.ContainsKey(_concepto.CuentaAIU.Value))
                            _planCuentaServ = cacheCuentaServ[_concepto.CuentaAIU.Value];
                        else
                        {
                            _planCuentaServ = (DTO_coPlanCuenta)GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.coPlanCuenta, _concepto.CuentaAIU.Value, true, false);
                            cacheCuentaServ.Add(_concepto.CuentaAIU.Value, _planCuentaServ);
                        }
                    } 
                    else
                    {
                    ...
                    DTO_coPlanCuenta cuentaVlrBruto = (DTO_coPlanCuenta)GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.coPlanCuenta, _planCuentaServ.ID.Value, true, false);
                    DTO_ComprobanteFooter compFooterValorBruto = new DTO_ComprobanteFooter();
                    compFooterValorBruto.Index = compFooter.Count;
                    compFooterValorBruto.CuentaID.Value = _planCuentaServ.ID.Value;
                ...
                if (proyecto != null && proyecto.HeaderProyecto.APUIncluyeAIUInd.Value.Value && !header.FacturaTipoID.Value.Equals(tipoFacturaCtaCobro))
                {
                    #region Crea registro de Administracion
                    if (header.Administracion.Value.HasValue &&  header.Administracion.Value != 0)
                    {
                        string ctaAdmin = this.GetControlValueByCompany(ModulesPrefix.fa, AppControl.fa_CuentaAdminAIU);
                        string conceptoxDef = this.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_ConceptoCargoXDefecto);
                        DTO_coPlanCuenta cta = (DTO_coPlanCuenta)this.GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.coPlanCuenta, ctaAdmin, true, false);
                        if (cta != null)
                        {
                            DTO_ComprobanteFooter compFooterImpItem = new DTO_ComprobanteFooter();
                            compFooterImpItem.CentroCostoID.Value = ctrl.CentroCostoID.Value;
                            ...
                            compFooterImpItem.LugarGeograficoID.Value = ctrl.LugarGeograficoID.Value;
                            compFooterImpItem.DocumentoCOM.Value = ctrl.DocumentoNro.Value.ToString();
                            compFooterImpItem.CuentaID.Value = ctaAdmin;

                            compFooterImpItem.Descriptivo.Value = "Admin / Imprev AIU";
                            compFooterImpItem.ConceptoSaldoID.Value = cta.ConceptoSaldoID.Value;
                            compFooterImpItem.ConceptoCargoID.Value = conceptoxDef;
                            compFooterImpItem.DatoAdd2.Value = "Admin: " + proyecto.HeaderProyecto.PorEmpresaADM.ToString() + "% - " + "Imprev: " + proyecto.HeaderProyecto.PorEmpresaIMP.ToString() + "%";
                            compFooterImpItem.vlrBaseML.Value = 0;
                            compFooterImpItem.vlrBaseME.Value = 0;

                            if (monedaLocal == ctrl.MonedaID.Value)
                            {
                                compFooterImpItem.vlrMdaLoc.Value = header.Administracion.Value * (-1);
                                compFooterImpItem.vlrMdaExt.Value = (ctrl.TasaCambioDOCU.Value == 0) ? 0 : Math.Round(compFooterImpItem.vlrMdaLoc.Value.Value / ctrl.TasaCambioDOCU.Value.Value, 2);
                                compFooterImpItem.vlrMdaOtr.Value = compFooterImpItem.vlrMdaLoc.Value;
                            }
                            else
                            {
                                compFooterImpItem.vlrMdaExt.Value = header.Administracion.Value * (-1);
                                compFooterImpItem.vlrMdaLoc.Value = compFooterImpItem.vlrMdaExt.Value * ctrl.TasaCambioDOCU.Value;
                                compFooterImpItem.vlrMdaOtr.Value = compFooterImpItem.vlrMdaExt.Value;
                            }
                            compFooter.Add(compFooterImpItem);
                            _valorTotal += compFooterImpItem.vlrMdaLoc.Value.Value;
                        }
                    }

                    #endregion
                    #region Crea registro de Imprevistos
                    if (header.Imprevistos.Value.HasValue && header.Imprevistos.Value != 0)
                    {
                        string ctaImprevistos = this.GetControlValueByCompany(ModulesPrefix.fa, AppControl.fa_CuentaImprevAIU);
                        string conceptoxDef = this.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_ConceptoCargoXDefecto);
                        DTO_coPlanCuenta cta = (DTO_coPlanCuenta)this.GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.coPlanCuenta, ctaImprevistos, true, false);
                        if (cta != null)
                        {
                            DTO_ComprobanteFooter compFooterImpItem = new DTO_ComprobanteFooter();
                            compFooterImpItem.CentroCostoID.Value = ctrl.CentroCostoID.Value;
                            ...
                            compFooterImpItem.LugarGeograficoID.Value = ctrl.LugarGeograficoID.Value;
                            compFooterImpItem.DocumentoCOM.Value = ctrl.DocumentoNro.Value.ToString();
                            compFooterImpItem.CuentaID.Value = ctaImprevistos;

                            compFooterImpItem.Descriptivo.Value = "Imprevistos AIU";
                            compFooterImpItem.ConceptoSaldoID.Value = cta.ConceptoSaldoID.Value;
                            compFooterImpItem.ConceptoCargoID.Value = conceptoxDef;
                            compFooterImpItem.DatoAdd2.Value = "Imprevistos: " + proyecto.HeaderProyecto.PorEmpresaIMP.ToString() + "%";
                            compFooterImpItem.vlrBaseML.Value = 0;
                            compFooterImpItem.vlrBaseME.Value = 0;

                            if (monedaLocal == ctrl.MonedaID.Value)
                            {
                                compFooterImpItem.vlrMdaLoc.Value = header.Imprevistos.Value * (-1);
                                compFooterImpItem.vlrMdaExt.Value = (ctrl.TasaCambioDOCU.Value == 0) ? 0 : Math.Round(compFooterImpItem.vlrMdaLoc.Value.Value / ctrl.TasaCambioDOCU.Value.Value, 2);
                                compFooterImpItem.vlrMdaOtr.Value = compFooterImpItem.vlrMdaLoc.Value;
                            }
                            else
                            {
                                compFooterImpItem.vlrMdaExt.Value = header.Imprevistos.Value * (-1);
                                compFooterImpItem.vlrMdaLoc.Value = compFooterImpItem.vlrMdaExt.Value * ctrl.TasaCambioDOCU.Value;
                                compFooterImpItem.vlrMdaOtr.Value = compFooterImpItem.vlrMdaExt.Value;
                            }
                            compFooter.Add(compFooterImpItem);
                    ...
                    #endregion
                    #region Crea registro de Utilidad
                    if (header.Utilidad.Value.HasValue && header.Utilidad.Value != 0)
                    {
                        string ctaUtilidad = this.GetControlValueByCompany(ModulesPrefix.fa, AppControl.fa_CuentaUtilidadAIU);
                        string conceptoxDef = this.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_ConceptoCargoXDefecto);
                        DTO_coPlanCuenta cta = (DTO_coPlanCuenta)this.GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.coPlanCuenta, ctaUtilidad, true, false);
                        if (cta != null)
                        {
                            DTO_ComprobanteFooter compFooterImpItem = new DTO_ComprobanteFooter();
                            compFooterImpItem.CentroCostoID.Value = ctrl.CentroCostoID.Value;
                            ...
                            compFooterImpItem.LugarGeograficoID.Value = ctrl.LugarGeograficoID.Value;
                            compFooterImpItem.DocumentoCOM.Value = ctrl.DocumentoNro.Value.ToString();
                            compFooterImpItem.CuentaID.Value = ctaUtilidad;

                            compFooterImpItem.Descriptivo.Value = "Utilidad AIU";
                            compFooterImpItem.ConceptoSaldoID.Value = cta.ConceptoSaldoID.Value;
                            compFooterImpItem.ConceptoCargoID.Value = conceptoxDef;
                            compFooterImpItem.DatoAdd2.Value = "Utilidad: " + proyecto.HeaderProyecto.PorEmpresaUTI.ToString() + "%";
                            compFooterImpItem.vlrBaseML.Value = 0;
                            compFooterImpItem.vlrBaseME.Value = 0;

                            if (monedaLocal == ctrl.MonedaID.Value)
                            {
                                compFooterImpItem.vlrMdaLoc.Value = header.Utilidad.Value * (-1);
                                compFooterImpItem.vlrMdaExt.Value = (ctrl.TasaCambioDOCU.Value == 0) ? 0 : Math.Round(compFooterImpItem.vlrMdaLoc.Value.Value / ctrl.TasaCambioDOCU.Value.Value, 2);
                                compFooterImpItem.vlrMdaOtr.Value = compFooterImpItem.vlrMdaLoc.Value;
                            }
                            else
                            {
                                compFooterImpItem.vlrMdaExt.Value = header.Utilidad.Value * (-1);
                                compFooterImpItem.vlrMdaLoc.Value = compFooterImpItem.vlrMdaExt.Value * ctrl.TasaCambioDOCU.Value;
                                compFooterImpItem.vlrMdaOtr.Value = compFooterImpItem.vlrMdaExt.Value;
                            }
                            compFooter.Add(compFooterImpItem);
                    ...
                    #endregion
                    #region Crea registro de IVA Utilidad
                    if (header.Utilidad.Value.HasValue && header.Utilidad.Value != 0)
                    {
                        string ctaIVAUtilidad = this.GetControlValueByCompany(ModulesPrefix.fa, AppControl.fa_CuentaIVAUtilidadAIU);
                        string conceptoxDef = this.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_ConceptoCargoXDefecto);
                        DTO_coPlanCuenta cta = (DTO_coPlanCuenta)this.GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.coPlanCuenta, ctaIVAUtilidad, true, false);
                        if (cta != null)
                        {
                            DTO_ComprobanteFooter compFooterImpItem = new DTO_ComprobanteFooter();
                            compFooterImpItem.CentroCostoID.Value = ctrl.CentroCostoID.Value;
                            ...
                            compFooterImpItem.LugarGeograficoID.Value = ctrl.LugarGeograficoID.Value;
                            compFooterImpItem.DocumentoCOM.Value = ctrl.DocumentoNro.Value.ToString();
                            compFooterImpItem.CuentaID.Value = ctaIVAUtilidad;

                            compFooterImpItem.Descriptivo.Value = "IVA Utilidad AIU";
                            compFooterImpItem.ConceptoSaldoID.Value = cta.ConceptoSaldoID.Value;
                            compFooterImpItem.ConceptoCargoID.Value = conceptoxDef;
                            //compFooterImpItem.DatoAdd2.Value = "Porcentaje Utilidad: " + proyecto.HeaderProyecto.PorEmpresaUTI.ToString() + "%";

                            if (monedaLocal == ctrl.MonedaID.Value)
                            {
                                compFooterImpItem.vlrBaseML.Value = header.Utilidad.Value * (-1);
                                compFooterImpItem.vlrBaseME.Value = (ctrl.TasaCambioDOCU.Value == 0) ? 0 : Math.Round(header.Utilidad.Value.Value / ctrl.TasaCambioDOCU.Value.Value, 2);
                                compFooterImpItem.vlrMdaLoc.Value = header.Iva.Value * (-1);
                                compFooterImpItem.vlrMdaExt.Value = (ctrl.TasaCambioDOCU.Value == 0) ? 0 : Math.Round(compFooterImpItem.vlrMdaLoc.Value.Value / ctrl.TasaCambioDOCU.Value.Value, 2);
                                compFooterImpItem.vlrMdaOtr.Value = compFooterImpItem.vlrMdaLoc.Value;
                            }
                            else
                            {
                                compFooterImpItem.vlrBaseME.Value = header.Utilidad.Value * (-1);
                                compFooterImpItem.vlrBaseML.Value = compFooterImpItem.vlrBaseME.Value * ctrl.TasaCambioDOCU.Value;
                                compFooterImpItem.vlrMdaExt.Value = header.Iva.Value * (-1);
                                compFooterImpItem.vlrMdaLoc.Value = compFooterImpItem.vlrMdaExt.Value * ctrl.TasaCambioDOCU.Value;
                                compFooterImpItem.vlrMdaOtr.Value = compFooterImpItem.vlrMdaExt.Value;
                            }
                            compFooter.Add(compFooterImpItem);
                                                  ...
                                                  int compNro, string obs, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                {
                    result.Result = ResultValue.NOK;
                    result.ResultMessage = "Numero del comprobante no puede ser 0, verifique";
                    numeroDoc = 0;
                    return result;
                }
                result = this._moduloContabilidad.Comprobante_Aprobar(documentID, actividadFlujoID, ModulesPrefix.fa, numeroDoc, true, periodo, compID,
                    compNro, string.Empty, true, false, true, true);
                if (result.Result == ResultValue.NOK)
                    return result;
                #endregion
                ...
                if (cacheBodega.Count > 0)
                {
                    ...
                    foreach (DTO_MvtoInventarios item in listBodegas)
                    {
                        ...
                        {
                            result = (DTO_TxResult)resultInventario;
                            if (result.Result == ResultValue.NOK)
                                return result;
                        }
        ...
        public DTO_SerializedObject FacturaVenta_Guardar(int documentID, DTO_glDocumentoControl ctrl, DTO_faFacturaDocu header, List<DTO_faFacturacionFooter> footer, bool update, out int numeroDoc, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx, bool ContabilizaInd = true)
        {
            ...
            try
            {
                ...
                int documentoNro = 0;
                int compNro = 0;
                DateTime periodoInicial = ctrl.PeriodoDoc.Value.Value;
                string monedaLocal = this.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_MonedaLocal);
                ...
                else
                {
                    ...
                    periodoInicial = docCtrl.PeriodoDoc.Value.Value;
                ...
                if (ContabilizaInd)
                {
                    ...
                    if (periodoInicial != ctrl.PeriodoDoc.Value)//Valida si cambia el periodo del comprobante
                    {
                        this._moduloContabilidad.BorrarAuxiliar_Pre(ctrl.PeriodoDoc.Value.Value, string.Empty, 0, ctrl.NumeroDoc.Value.Value);
                        result  = this._moduloContabilidad.AgregarAuxiliar_Pre(documentID, true, comp, true);
                    }  
                    else
                    {
        ...
        /// <returns>Retorna el resultado de la operacion</returns>
        public DTO_TxResult Facturacion_Revertir(int documentID, int numeroDoc, int? consecutivoPos, ref List<DTO_glDocumentoControl> ctrls,
            ref List<DTO_coComprobante> coComps, bool insideAnotherTx)
        {
            ...
            try
            {
                this._moduloGlobal = (ModuloGlobal)base.GetInstance(typeof(ModuloGlobal), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._moduloContabilidad = (ModuloContabilidad)this.GetInstance(typeof(ModuloContabilidad), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._moduloProy = (ModuloProyectos)this.GetInstance(typeof(ModuloProyectos), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_actasEntrega = (DAL_pyActaEntregaDeta)this.GetInstance(typeof(DAL_pyActaEntregaDeta), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                ...
                {
                    DTO_pyActaEntregaDeta filter = new DTO_pyActaEntregaDeta();
                    filter.NumDocFactura.Value = numeroDoc;
                    filter.NumDocProyecto.Value = proyecto.DocCtrl.NumeroDoc.Value;
                    List<DTO_pyActaEntregaDeta> actasFactura = this._dal_actasEntrega.DAL_pyActaEntregaDeta_GetByParameter(filter);
                    foreach (DTO_pyActaEntregaDeta acta in actasFactura)
                    {
        ...
        public List<DTO_FacturacionTotales> ReportesFacturacion_CxCPorEdadesDetalladas(DateTime fechaCorte, string tercero, bool isDetallada)
        {
            try
            {
                ...
                foreach (var item in distinct)
                {
                    ...
                    {
                        total += (sum.No_Vencidas.Value.Value+sum.Treinta.Value.Value + sum.Sesenta.Value.Value + sum.Noventa.Value.Value + sum.COchenta.Value.Value + sum.MasCOchenta.Value.Value);
        ...
        /// <param name="isDetallada">Revisa si la consulta es detalla o resumida (true) detallada (false) resumida</param>
        /// <returns></returns>
        public List<DTO_ReportLibroVentas> ReportesFacturacion_LibroVentas(DateTime periodo, int diaFinal, string cliente, string prefijo, string NroFactura)
    //    public DTO_FacturacionTotales ReportesFacturacion_LibroVentas(DateTime periodo, int diaFinal, string cliente, string prefijo, string NroFactura)
        {
            try
            {
                this._dal_ReportesFacturacion = (DAL_ReportesFacturacion)this.GetInstance(typeof(DAL_ReportesFacturacion), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                List<DTO_ReportLibroVentas> result = new List<DTO_ReportLibroVentas>();
                DTO_ReportLibroVentas libros = new DTO_ReportLibroVentas();
                libros.Detalle = new List<DTO_ReportLibroVentas>();

                libros.Detalle = this._dal_ReportesFacturacion.DAL_ReportesFacturacion_LibroVentas(periodo, diaFinal, cliente, prefijo, NroFactura);
...

        {
            try
            {
                ...
                this._dal_ReportesFacturacion= (DAL_ReportesFacturacion)this.GetInstance(typeof(DAL_ReportesFacturacion), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                result = this._dal_ReportesFacturacion.DAL_Reportes_CC_CxCToExcel(documentoID, tipoReporte, fechaIni, fechaFin, tercero,isDetallada);
