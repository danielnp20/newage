    ...
    public class ModuloCarteraFin : ModuloBase
    {
            ...
            finally
            {
                ...
                if (result.Result == ResultValue.OK)
                {
                    if (!insideAnotherTx)
                    {
                        ...
                        if (ctrl != null && coComp != null && ctrl.NumeroDoc.Value != null && ctrl.NumeroDoc.Value != 0)
                        {
        ...
        public DTO_TxResult Proceso_AmortizacionMensual(int documentID, DateTime periodo, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                object res = null;
                #endregion                
                #region Valida si ya se realizo la contabilizacion del periodo actual
                DTO_glDocumentoControl ctrlFilter = new DTO_glDocumentoControl();
                ctrlFilter.DocumentoID.Value = AppDocuments.AcuerdoPago;
                ctrlFilter.DocumentoTercero.Value = "AMORTIZADER";
                ctrlFilter.PeriodoDoc.Value = periodo;
                List<DTO_glDocumentoControl> docs = this._moduloGlobal.glDocumentoControl_GetByParameter(ctrlFilter);
                if (docs.Count > 0)
                {
                    result.Result = ResultValue.NOK;
                    result.ResultMessage = "Ya se realizó la contabilización para el periodo actual";
                    result.Details.Add(resultGLDC);
                    return result;
                }
                #endregion
                ...
                ctrlAmortizDerPagos.DocumentoTercero.Value = "AMORTIZADER";
                ...
                ctrlAmortizDereRecomp.DocumentoTercero.Value = "AMORTIZADER";
        ...
        /// </summary>
        public DTO_TxResult Proceso_CarteraAdministracion(int documentID, DateTime periodo, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            if (!insideAnotherTx)
                base._mySqlConnectionTx = base._mySqlConnection.BeginTransaction();
            ...
            DTO_coComprobante coCompCartAdmon = null;
            DTO_glDocumentoControl ctrlCartAdmon = null;
            try
            {
                #region Variables
                ...
                DateTime fechaDoc = new DateTime(periodo.Year, periodo.Month, DateTime.DaysInMonth(periodo.Year, periodo.Month));
                DTO_TxResultDetail resultGLDC = new DTO_TxResultDetail();
                DTO_Comprobante compCartAdmon = new DTO_Comprobante();
                DTO_Comprobante compAmortizDerRecompra = new DTO_Comprobante();
                object res = null;
                #endregion
                #region Valida si ya se realizo la contabilizacion del periodo actual
                DTO_glDocumentoControl ctrlFilter = new DTO_glDocumentoControl();
                ctrlFilter.DocumentoID.Value = AppDocuments.AcuerdoPago;
                ctrlFilter.DocumentoTercero.Value = "CARTERAADM";
                ctrlFilter.PeriodoDoc.Value = periodo;
                List<DTO_glDocumentoControl> docs = this._moduloGlobal.glDocumentoControl_GetByParameter(ctrlFilter);
                if (docs.Count > 0)
                {
                ...
                #endregion
                #region Crea el documento de Cart Admon (glDocumentoControl)
                ctrlCartAdmon = new DTO_glDocumentoControl();
                ctrlCartAdmon.DocumentoNro.Value = 0;
                ctrlCartAdmon.DocumentoID.Value = AppDocuments.AcuerdoPago;
                ctrlCartAdmon.DocumentoTipo.Value = (int)DocumentoTipo.DocInterno;
                ctrlCartAdmon.ComprobanteID.Value = string.Empty;
                ctrlCartAdmon.Fecha.Value = DateTime.Now;
                ctrlCartAdmon.FechaDoc.Value = fechaDoc;
                ctrlCartAdmon.PeriodoDoc.Value = periodo;
                ctrlCartAdmon.PeriodoUltMov.Value = periodo;
                ctrlCartAdmon.AreaFuncionalID.Value = af;
                ctrlCartAdmon.PrefijoID.Value = prefijoDef;
                ctrlCartAdmon.ProyectoID.Value = proyDef;
                ctrlCartAdmon.CentroCostoID.Value = ctoCostoDef;
                ctrlCartAdmon.LugarGeograficoID.Value = lugGeoDef;
                ctrlCartAdmon.LineaPresupuestoID.Value = linPresDef;
                ctrlCartAdmon.TerceroID.Value = terceroDef;
                ctrlCartAdmon.MonedaID.Value = mdaLoc;
                ctrlCartAdmon.TasaCambioCONT.Value = 0;
                ctrlCartAdmon.TasaCambioDOCU.Value = 0;
                ctrlCartAdmon.DocumentoTercero.Value = "CARTERAADM";
                ctrlCartAdmon.Descripcion.Value = "CARTERA EN ADMINISTRACION PERIODO: " + periodo.ToString(FormatString.ControlDate);
                ctrlCartAdmon.Estado.Value = (byte)EstadoDocControl.Aprobado;
                ctrlCartAdmon.seUsuarioID.Value = this.UserId;
                ctrlCartAdmon.Valor.Value = 0;
                ctrlCartAdmon.Iva.Value = 0;

                resultGLDC = this._moduloGlobal.glDocumentoControl_Add(AppDocuments.AcuerdoPago, ctrlCartAdmon, true);
                if (resultGLDC.Message != ResultValue.OK.ToString())
                {
                    result.Result = ResultValue.NOK;
                    result.ResultMessage = "NOK";
                ...
                ctrlCartAdmon.NumeroDoc.Value = Convert.ToInt32(resultGLDC.Key);
                #endregion
                #region Crear Comprobante Cart Admon
                res = this._dal_CarteraFin.DAL_CarteraFin_GetComprobanteCarteraAdmon(ctrlCartAdmon.NumeroDoc.Value.Value);
                if (res.GetType() == typeof(DTO_TxResult))
                {
                    result = (DTO_TxResult)res;
                    return result;
                    ...
                    compCartAdmon = (DTO_Comprobante)res;

                compCartAdmon.coDocumentoID = ctrlCartAdmon.DocumentoID.Value.Value.ToString();
                compCartAdmon.PrefijoID = ctrlCartAdmon.PrefijoID.Value.ToString();
                compCartAdmon.DocumentoNro = ctrlCartAdmon.DocumentoNro.Value.Value;
                compCartAdmon.CuentaID = ctrlCartAdmon.CuentaID.Value;
                compCartAdmon.TerceroID = ctrlCartAdmon.TerceroID.Value;
                compCartAdmon.ProyectoID = ctrlCartAdmon.ProyectoID.Value;
                compCartAdmon.CentroCostoID = ctrlCartAdmon.CentroCostoID.Value;
                compCartAdmon.LineaPresupuestoID = ctrlCartAdmon.LineaPresupuestoID.Value;
                compCartAdmon.LugarGeograficoID = ctrlCartAdmon.LugarGeograficoID.Value;
                #endregion
                #region Guardar comprobante Cart Admon
                result = this._moduloContabilidad.ContabilizarComprobante(documentID, compCartAdmon, compCartAdmon.Header.PeriodoID.Value.Value, ModulesPrefix.cc, 0, false);
                if (result.Result == ResultValue.NOK)
                    return result;
                coCompCartAdmon = (DTO_coComprobante)this.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.coComprobante, compCartAdmon.Header.ComprobanteID.Value, true, false);
            ...
            finally
            {
                ...
                if (result.Result == ResultValue.OK)
                {
                    if (!insideAnotherTx)
                    {
                        ...
                        if (ctrlCartAdmon.NumeroDoc.Value != null && ctrlCartAdmon.NumeroDoc.Value != 0)
                        {
                            ctrlCartAdmon.ComprobanteID.Value = coCompCartAdmon.ID.Value;
                            ctrlCartAdmon.DocumentoNro.Value = this.GenerarDocumentoNro(AppDocuments.AcuerdoPago, ctrlCartAdmon.PrefijoID.Value);
                            ctrlCartAdmon.ComprobanteIDNro.Value = this.GenerarComprobanteNro(coCompCartAdmon, ctrlCartAdmon.PrefijoID.Value, ctrlCartAdmon.PeriodoDoc.Value.Value, ctrlCartAdmon.DocumentoNro.Value.Value);
                            this._moduloGlobal.ActualizaConsecutivos(ctrlCartAdmon, true, true, false);
                            this._moduloContabilidad.ActualizaComprobanteNro(ctrlCartAdmon.NumeroDoc.Value.Value, ctrlCartAdmon.ComprobanteIDNro.Value.Value, false);
                        }                        
            ...
            bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                if (!numDocSolicitud.HasValue)
                {
                    ...
                    ctrl.DocumentoNro.Value = 0;
                    ctrl.DocumentoID.Value = AppDocuments.SolicitudLibranza;
                    ctrl.NumeroDoc.Value = 0;
                    ctrl.PeriodoDoc.Value = periodo;
                    ctrl.PeriodoUltMov.Value = periodo;
                    ctrl.Fecha.Value = DateTime.Now;
                    ctrl.ComprobanteID.Value = string.Empty;
                    ...
                    poliza.NumDocSolicitud.Value = numDocNew;
                    #endregion
        ...
        public DTO_TxResult CobroJuridicoRenovacionPoliza_Add(int documentID, DTO_ccPolizaEstado poliza, bool insideAnotherTx)
        {
            ...
            {
                result.Result = ResultValue.NOK;
                result.ResultMessage = Mentor_Exception.LogException_Local(this.loggerConnectionStr, ex, this.UserId.ToString(), "CobroJuridicoRenovacionPoliza_Add");
                return result;
            }
            finally
            {
        ...
        /// /// <returns>Retorna un objeto TxResult si se presenta un error, de lo contrario devuelve un objeto de tipo DTO_PlanDePagos</returns>
        private DTO_SerializedObject CrearPlanPagosCredito(int numeroDocCred,string lineaCredID, int valorSolicitado, int vlrGiro, int valorFuturo, int plazo, int edad,
            DateTime fechaLiquida, DateTime fechaCuota1, decimal porcInteres, List<DTO_Cuota> cuotasExtras, Dictionary<string, decimal> compsNuevoValor, string tipoCredito, bool excluyeCompInvisibleInd)
        {
            ...
            try
            {
                ...
                decimal sumExtra = cuotasExtras.Sum(x => x.ValorCuota* x.NumCuota);
                ...
                #endregion
                #region Calcula Componentes especiales

                //Calcula Comp Pagaduria en base a las cuotas extra
                if (cacheCarteraComponentes.Any(x => x.Value.FuenteLiquida.Value == (byte)FuenteLiquida.Pagaduria))
                {
                    this._dal_ccSolicitudDocu = (DAL_ccSolicitudDocu)base.GetInstance(typeof(DAL_ccSolicitudDocu), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    string compPagaduria = cacheCarteraComponentes.Values.Where(x => x.FuenteLiquida.Value == (byte)FuenteLiquida.Pagaduria).First().ID.Value;
                    DTO_ccSolicitudDocu doc = this._dal_ccSolicitudDocu.DAL_ccSolicitudDocu_GetByNumeroDoc(numeroDocCred);
                    if (doc != null && !string.IsNullOrEmpty(doc.PagaduriaID.Value))
                    {
                        DTO_ccPagaduria pagad = (DTO_ccPagaduria)this.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccPagaduria, doc.PagaduriaID.Value, true, false);
                        decimal porcPag = pagad.IncorporacionPorc.Value.HasValue ? pagad.IncorporacionPorc.Value.Value : 0;
                        decimal vlrComponentePag = valorFuturo * (porcPag / 100);
                        DTO_ccSolicitudComponentes compPag = componentesAll.Find(x => x.ComponenteCarteraID.Value == compPagaduria);
                        compPag.CuotaValor.Value = Math.Round(vlrComponentePag/ plazo,0);
                        compPag.TotalValor.Value = compPag.CuotaValor.Value * plazo;
                    }
                }

                //Calcula Comp de Ajuste en base a las cuotas extra
                if (cacheCarteraComponentes.Any(x => x.Value.TipoLiquida.Value == (byte)TipoLiquidacionCartera.AjustaVlrNominal))
                {
                    this._dal_ccSolicitudDocu = (DAL_ccSolicitudDocu)base.GetInstance(typeof(DAL_ccSolicitudDocu), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    decimal dif = valorFuturo -(sumTotal + componentesAll.Where(x=>!x.CompInvisibleInd.Value.Value).Sum(x => x.TotalValor.Value.Value));
                    string compAjuste = cacheCarteraComponentes.Values.Where(x => x.TipoLiquida.Value == (byte)TipoLiquidacionCartera.AjustaVlrNominal).First().ID.Value;
                    DTO_ccSolicitudComponentes compPag = componentesAll.Find(x => x.ComponenteCarteraID.Value == compAjuste);
                    if (dif < 0)
                    {
                        result.Result = ResultValue.NOK;
                        result.ResultMessage = "El valor nominal liquidado es mayor al valor futuro";
                        return result;
                    }                   
                    compPag.CuotaValor.Value = Math.Round(dif / plazo,0);
                    compPag.TotalValor.Value = compPag.CuotaValor.Value * plazo;
                }
                ...
                if (vlrCuotaReal != 0)
                {
                    for (i = 1; i <= plazo; ++i)
                    {
                        ...
                        {
                            //Asigna la info del seguro
                            decimal tasa = dict_TasasComponentes.ContainsKey(compSeguro)? dict_TasasComponentes[compSeguro] : 0;
                            DTO_ccSolicitudComponentes comp = dict_TasasComponentes.ContainsKey(compSeguro) ? componentes.Where(x => x.ComponenteCarteraID.Value == compSeguro).First() : null;
                            if (comp != null)
                            {
                                if (dict_TasasComponentes.ContainsKey(compSeguro) && !dict_ComponentesFijos[compSeguro])
                                {
                                    cuota.Seguro = Convert.ToInt32(tasa * saldoParcial / 100);
                            ...
                            if (dict_ComponentesFijos.ContainsKey(compSeguro) && dict_ComponentesFijos[compSeguro])
                            ...
                            if(plazo != 1)
                                ...
                                dict_TotalComponentesSaldo.Add(compInteres, cuota.Intereses);
        ...
        private DTO_TxResult RechazarSolicitud(int documentID, string actFlujoID, DTO_ccSolicitudDocu sol, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                //Estado documento
                //this._moduloGlobal.glDocumentoControl_ChangeDocumentStatus(documentID, sol.NumeroDoc.Value.Value, EstadoDocControl.Anulado, sol.Observacion.Value, true);
            ...
            List<DTO_Cuota> cuotasExtras, Dictionary<string, decimal> compsNuevoValor, int numDocCredito, string tipoCredito, bool excluyeCompInvisibleInd)
        {
            ...
            try
            {
                ...
                if (liquidaAll)
                {
                    object credito = this.CrearPlanPagosCredito(numDocCredito, lineaCredID, valorCredito, vlrGiro, valorFuturo, plazoCredito, edad, fechaLiquida, fechaCuota1, interesCredito, cuotasExtras, compsNuevoValor,tipoCredito, excluyeCompInvisibleInd);
                ...
                if (valorPoliza != 0)
                {
                    ...
                    planPagosFinal.VlrPoliza = planPagosPoliza.VlrPoliza;
                ...
                foreach (DTO_ccSolicitudDocu sol in solicitudes)
                {
                    ...
                    if (sol.Aprobado.Value.Value)
                    {
                        ...
                        try
                        {
                            ...
                            if (sol.TipoCredito.Value == (byte)TipoCredito.RestructuracionSinCambio)
                            {
                                this._moduloCartera = (ModuloCartera)this.GetInstance(typeof(ModuloCartera), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                                result = this._moduloCartera.LiquidacionCreditoReestructura_Aprobar(documentID, actFlujoID, sol, false);
                            }
                            else
                            {
        ...
        /// <returns></returns>
        public DTO_TxResult RecaudosMasivosFin_Validar(int documentID, DateTime fechaNomina, string pagaduriaID, ref List<DTO_ccIncorporacionDeta> data, Dictionary<Tuple<int, int>, int> batchProgress)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            try
            {
                ...
                string compInteres = this.GetControlValueByCompany(ModulesPrefix.cc, AppControl.cc_ComponenteInteresCorriente);
                string compSeguro = this.GetControlValueByCompany(ModulesPrefix.cc, AppControl.cc_ComponenteSeguroVida);
                int cantCtasAnticipadas = Convert.ToInt32(this.GetControlValueByCompany(ModulesPrefix.cc, AppControl.cc_CantCuotasPermRecAnticipada));
                ...
                foreach (string cli in clientes)
                {
                    ...
                    foreach (DTO_ccIncorporacionDeta detaxLib in data.FindAll(x => x.ClienteID.Value == cli && x.Libranza.Value != null))
                    {
                        ...
                        if (cred != null)
                        {
                            if (!cacheCreditoClientes.ContainsKey(cli))
                    ...
                    if (data.Count(x => x.ClienteID.Value == cli && x.Libranza.Value == null) > 0)
                    {
                        ...
                        if (creditosCli.Count > 0)
                        {
                            foreach (DTO_ccCreditoDocu cred in creditosCli)
                            {
                                ...
                                //if(!cacheSaldoCredito.Keys.Any(x=>x == cred.Libranza.Value.Value))
                                ...
                                foreach (DTO_ccCreditoPlanPagos pp in saldo.PlanPagos)
                                {
                                    ...
                                    pagos.indexCuota = indexCuotaxCred;//Permite contar cuantas cuotas tiene cada credito en el recaudo
                                    listPagos.Add(pagos);
                                    indexCuotaxCred++;
                                    #endregion
                                }
                            ...
                            foreach (DTO_RecaudoPagos pag in listPagos)
                            {
                                ...
                                newDeta.indexCuota = pag.indexCuota;//Permite contar cuantas cuotas tiene cada credito en el recaudo
                                vlrCuotaRestante -= pag.VlrPago.Value.Value;
                                newDeta.FechaCuota1.Value = pag.FechaCuota.Value;
                        ...
                        else //Sino valida si tiene creditos ya cancelados
                        {
                            if (dtoCliente.EstadoCartera.Value == (byte)EstadoDeuda.Juridico || dtoCliente.EstadoCartera.Value == (byte)EstadoDeuda.AcuerdoPago ||
                                dtoCliente.EstadoCartera.Value == (byte)EstadoDeuda.AcuerdoPagoIncumplido)
                                deta.EstadoClienteInvalid.Value = true;
                            List<DTO_ccCreditoDocu> cred = this._moduloCartera.GetCreditoByCliente(deta.ClienteID.Value);
                            cred = cred.OrderByDescending(x => x.NumeroDoc.Value).ToList();
                            if (cred.Count > 0)
                            {
                                deta.CanceladoInd.Value = cred.First().CanceladoInd.Value;
                                deta.EstadoCuentaInd.Value = cred.First().DocEstadoCuenta.Value != null ? true : false;
                                incorporacionFinal.Add(deta);
                            }
                            else
                                deta.Libranza.Value = null;
                        }
                ...
                #region Valida si ya existen nominas realizadas en la fecha indicada(Pagaduria)
                if (!string.IsNullOrEmpty(pagaduriaID))
                {
                    foreach (DTO_ccIncorporacionDeta d in data)
                    {
                        DTO_ccCreditoDocu cred = this._moduloCartera.GetCreditoByLibranza(d.Libranza.Value.HasValue? d.Libranza.Value.Value : 0);
                        bool exist = this._dal_ccNominaDeta.DAL_ccNominaDeta_ExistNomina(pagaduriaID, fechaNomina, (cred != null? cred.NumeroDoc.Value: null));
                        DTO_TxResultDetail rd = new DTO_TxResultDetail();
                        rd.DetailsFields = new List<DTO_TxResultDetailFields>();
                        rd.line = d.PosicionImportacion.Value.Value;
                        d.IndInconsistencia.Value = false; //Valida si tiene inconsistencias
                        validRow = true;
                        if (exist && string.IsNullOrEmpty(d.Codeudor.Value))
                        {
                            validRow = false;
                            rd.Message = "Cliente: " + d.ClienteID.Value + ", Obligación: " + d.Libranza.Value.Value.ToString() + ". Ya existe recaudo nómina para la fecha indicada, retirelo e intente nuevamente";
                        }
                        //Asigna las inconsistenciass
                        if (!string.IsNullOrWhiteSpace(rd.Message))
                        {
                            result.Details.Add(rd);
                            d.IndInconsistencia.Value = true;
                            d.NominaExist.Value = true;
                        }
                    }
                ...
                for (i = 1; i <= data.Count; ++i)
                {
                    ...
                    if (dto.Libranza.Value != null)
                    {
                        ...
                        else 
                        {
                            ...
                            foreach (DTO_ccCreditoPlanPagos pp in saldo.PlanPagos)
                            {
                                ...
                                {
                                    if (indexCuota <= cantCtasAnticipadas) //Abona solo xxxxx anticipidas se definan
                                    {
                                    ...
                                    //else if (!string.IsNullOrEmpty(pagaduriaID) && dto.ValorCuota.Value <= saldo.SaldosComponentes.FindAll(x => x.CuotaID.Value == pp.CuotaID.Value).Sum(x => x.CuotaSaldo.Value))
                                    //    credito.VlrSaldoOtros.Value = data.FindAll(x => x.Libranza.Value == dto.Libranza.Value.Value && x.FechaCuota1.Value > fechaNomina).Sum(y => y.ValorCuota.Value);

                                }                               
                            }
                            //Valida que el valor del recaudo no sea mayor a xxxxxx cuotas
                            if (data.FindAll(x=>x.Libranza.Value == dto.Libranza.Value.Value).Sum(y=>y.ValorCuota.Value) > (credito.VlrSaldoOtros.Value + credito.VlrSaldoVencido.Value) && dto.FechaCuota1.Value > fechaNomina && !lastCuotaCredito)                           
                            {
                                if (!string.IsNullOrEmpty(pagaduriaID))
                                {     
                                    if (dto.indexCuota == 0 || dto.indexCuota > cantCtasAnticipadas)//Valida si esta recaudando maximo xxxxx cuotas por credito( es 0 cuando Importa la libranza)
                                    {
                                        validRow = false;
                                        rd.Message = "Cliente: " + dto.ClienteID.Value + ", Obligación: " + dto.Libranza.Value.Value.ToString() + ". Paga " + (cantCtasAnticipadas == 0? "1" : cantCtasAnticipadas.ToString()) + " o más cuotas anticipadas ";
                                    }
                                }
                                else
                                {
                                ...
                                else if (creditos.Count > 1)
                                {
                                    ...
                                    foreach (DTO_ccCreditoDocu cred in creditos)
                                    {
                                        ...
                                        foreach (DTO_ccCreditoPlanPagos pp in saldo.PlanPagos)
                                        {
                                            ...
                                            {
                                                if (indexCuota <= cantCtasAnticipadas) //Abona solo xxxxxx cuotas anticipidas
                                                {
                                    ...
                                    //Valida que el valor del recaudo no sea mayor a xxxx cuotas
                                    if (data.FindAll(x => x.Libranza.Value == dto.Libranza.Value.Value).Sum(y => y.ValorCuota.Value) > (vlrSaldoCuotas + vlrSaldoVencido) && dto.FechaCuota1.Value > fechaNomina && !lastCuotaCredito)
                                    {
                                        if (!string.IsNullOrEmpty(pagaduriaID))
                                        {
                                            if (dto.indexCuota == 0 || dto.indexCuota > cantCtasAnticipadas)//Valida si esta recaudando maximo xxxxx cuotas por credito( es 0 cuando Importa la libranza)
                                        ...
                                        {
                                            validRow = false;
                                            rd.Message = rd.Message = "Obligación: " + creditos.Last().Libranza.Value.ToString() + ". Paga " + (cantCtasAnticipadas == 0 ? "1" : cantCtasAnticipadas.ToString()) + " o más cuotas anticipadas ";
                                            if (result.Details.Exists(x => x.Message.Equals(rd.Message)))
                                            {
                                                validRow = true;
                                                rd.Message = string.Empty;
                    ...
                    #endregion
                    #region Valida que la pagaduria corresponda con el de el credito
                    if (string.IsNullOrWhiteSpace(rd.Message) && !string.IsNullOrEmpty(pagaduriaID))
                    {
                        if (credito != null && credito.PagaduriaID.Value != pagaduriaID && string.IsNullOrEmpty(dto.Codeudor.Value))
                        {
                            validRow = false;
                            rd.Message = DictionaryMessages.Err_Cc_InvalidPagaduriaLibranza;
                        }
                    }
                    #endregion
                    ...
                    if (!string.IsNullOrWhiteSpace(rd.Message))
                    {
                        ...
                        dto.MensajeError.Value = rd.Message;
                    }
            ...
            {
                result.Result = ResultValue.NOK;
                result.ResultMessage = Mentor_Exception.LogException_Local(this.loggerConnectionStr, ex, this.UserId.ToString(), "RecaudosMasivosFin_Validar");
                return result;
            }
        }
        ...
        /// <param name="data">Información a migrar</param>
        /// <returns>Retorna el resultado de la operacion</returns>
        public DTO_TxResult RecaudosMasivosFin_Procesar(int documentID, DateTime periodo, List<DTO_ccIncorporacionDeta> data,string pagaduria, bool isAnotherTx, 
            Dictionary<Tuple<int, int>, int> batchProgress)
        {
            ...
            try
            {
                ...
                string pag  = string.IsNullOrEmpty(pagaduria)?  this.GetControlValueByCompany(ModulesPrefix.cc, AppControl.cc_PagaduriaXDefecto) : pagaduria;
                result = this._dal_CarteraFin.DAL_CarteraFin_RecaudosMasivos_Inconsistencias(pag, periodo, data);
                return result;
            }
            catch (Exception ex)
            {
