...
namespace NewAge.Negocio
{
    public class ModuloProyectos : ModuloBase
    {
        ...
        private DAL_pyProyectoLocacion _dal_pyProyectoLocacion = null;
        private DAL_pyProyectoIngDetalleTarea _dal_pyProyectoIngTareas = null;
        private DAL_pyProyectoIngDetalleDeta _dal_pyProyectoIngDeta = null;
        private DAL_pyProyectoEntregasxMes _dal_pyProyectoEntregaxMes = null;
        private DAL_pyProyectoModificaFechas _dal_pyProyectoModificaFechas = null;
        #endregion
        #region Modulos
                    ...
                    if (_docControl.DocumentoID.Value == AppDocuments.PreProyecto)
                    {
                        ...
                        foreach (DTO_pyPreProyectoTarea tarea in transaccion.Detalle)
                        {
                            ...
                            if(exist)
                            {
                                ...
                                tarea.CentroCostoID.Value = centroCto;
                                tarea.TrabajoID.Value = trabajoXDef;                               
                                try { this._dal_pyPreProyectoTarea.DAL_pyPreProyectoTarea_Upd(tarea); }
                                catch (Exception)
                                {
                                    result.Result = ResultValue.NOK;
                                    result.ResultMessage = "Error al guardar las tareas, verifique que sean correctas y no estén duplicadas (Ítem-Tarea-Descripción)";
                                    return result;
                                }
                                foreach (DTO_pyPreProyectoDeta det in tarea.Detalle)
                                {
                            ...
                            else
                            {
                                ...
                                tarea.CentroCostoID.Value = centroCto;
                                tarea.TareaCliente.Value = string.IsNullOrEmpty(tarea.TareaCliente.Value) ? tarea.TareaID.Value : tarea.TareaCliente.Value;
                                try { this._dal_pyPreProyectoTarea.DAL_pyPreProyectoTarea_Add(tarea); }
                                catch (Exception)
                                {
                                    result.Result = ResultValue.NOK;
                                    result.ResultMessage = "Error al guardar las tareas, verifique que sean correctas y no estén duplicadas (Ítem-Tarea-Descripción)";
                    ...
                    else if (_docControl.DocumentoID.Value == AppDocuments.Proyecto)
                    {
                        ...
                        foreach (DTO_pyProyectoTarea tarea in transaccion.DetalleProyecto)
                        {
                            ...
                            if(exist)
                            {
                                ...
                                try { this._dal_pyProyectoTarea.DAL_pyProyectoTarea_Upd(tarea); }
                                catch (Exception)
                                {
                                    result.Result = ResultValue.NOK;
                                    result.ResultMessage = "Error al guardar las tareas, verifique que sean correctas y no estén duplicadas (Ítem-Tarea-Descripción)";
                            ...
                            else
                            {
                                ...
                                tarea.CentroCostoID.Value = centroCto;
                                tarea.TareaCliente.Value = string.IsNullOrEmpty(tarea.TareaCliente.Value) ? tarea.TareaID.Value : tarea.TareaCliente.Value;
                                try{ this._dal_pyProyectoTarea.DAL_pyProyectoTarea_Add(tarea); }
                                catch (Exception)
                                {
                                    result.Result = ResultValue.NOK;
                                    result.ResultMessage = "Error al guardar las tareas, verifique que sean correctas y no estén duplicadas (Ítem-Tarea-Descripción)";
        ...
        public DTO_SerializedObject SolicitudProyecto_AprobarProy(int documentID, DTO_SolicitudTrabajo transaccion, DateTime fechaInicio,Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                if (transaccion.DocCtrl.DocumentoID.Value == AppDocuments.PreProyecto)
                {
                    ...
                    proyectoDocu.PorcRteGarantia.Value = transaccion.Header.PorcRteGarantia.Value;
                    proyectoDocu.RteGarantiaIvaInd.Value = transaccion.Header.RteGarantiaIvaInd.Value;
                    proyectoDocu.ValorAnticipoInicial.Value = transaccion.Header.ValorAnticipoInicial.Value;
                    proyectoDocu.MesesDesviacion.Value = 0;
                    proyectoDocu.MesesDuracion.Value = 0;
                ...
                else //Si es el Proyecto Final lo aprueba
                {
                    ...
                    if(transaccion.DocCtrl.Estado.Value == (byte)EstadoDocControl.SinAprobar && (!transaccion.HeaderProyecto.MesesDuracion.Value.HasValue || transaccion.HeaderProyecto.MesesDuracion.Value == 0))
                    {
                        result.Result = ResultValue.NOK;
                        result.ResultMessage = DictionaryMessages.Err_Py_ProyectoMesesDiracionInvalid;
                        return result;
                    }
                            ...
                            foreach (DTO_pyProyectoDeta det in tarea.Detalle)
                            {
                                ...
                                if (existDeta)
                                {
                                    ...
                                    if (movsByDeta.Count > 0)
                                    {
                                        ...
                                        ////valida si hay menos cantidad requerida y si ya se pidio a proveedores para invalidar la operacion
                                        //else if (cantExist > cantNueva && movsByDeta.Sum(x => x.CantidadPROV.Value) > cantNueva)
                                        //{
                                        //    result.ResultMessage = "La cantidad a retirar del recurso " + det.RecursoID.Value + " de la tarea " + tarea.TareaID + "-" + tarea.Descriptivo.Value + ", " + "no es válida porque ya tiene solicitud de proveedores. " +
                                        //                            "(Cant Prov: " + movsByDeta.Sum(x => x.CantidadPROV.Value).Value.ToString("n2") + " - " + " Cant Retiro: " + cantNueva.ToString("n2") + ")";
                                        //    result.Result = ResultValue.NOK;
                                        //    if (result.Result == ResultValue.NOK)
                                        //        return result;
                                        //}
        ...
        public DTO_SerializedObject CompraRecursos_ApproveSolicitudOC(int documentID, List<DTO_pyProyectoMvto> listMvtos, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                this._moduloGlobal = (ModuloGlobal)this.GetInstance(typeof(ModuloGlobal), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                DateTime periodoProv = Convert.ToDateTime(this.GetControlValueByCompany(ModulesPrefix.pr, AppControl.pr_Periodo));
                ...
                {   
                    int numeroDoc = 0;
                    footerSol = new List<DTO_prSolicitudFooter>();
                    headerSol = new DTO_prSolicitudDocu();
                    ...
                    if (mvtosByNumDoc.Count > 0)
                    {
                        ...
                        ctrlSol.PeriodoDoc.Value = periodoProv;
                        ctrlSol.PeriodoUltMov.Value = periodoProv;
                        ctrlSol.FechaDoc.Value = DateTime.Now.Month == periodoProv.Month ? DateTime.Now.Date : new DateTime(periodoProv.Year,periodoProv.Month,DateTime.DaysInMonth(periodoProv.Year, periodoProv.Month));
                        ...
                        else if (numeroDoc != 0)
                        {
                            ...
                            {
                                foreach (DTO_pyProyectoMvto mvtoHij in mvtoPadre.DetalleTareas.FindAll(x=>x.NumeroDoc.Value == numDoc))
                                {
                            ...
                            {
                                this._moduloGlobal.glDocumentoControl_ChangeDocumentStatus(documentID, numeroDoc, EstadoDocControl.Aprobado, string.Empty, true);                               List<string> act = this._moduloGlobal.glActividadFlujo_GetActividadesByDocumentID(AppDocuments.SolicitudAprob);
                                if (act.Count > 0)
        ...
        public List<DTO_pyProyectoMvto> pyProyectoMvto_GetParameter(DTO_pyProyectoMvto filter)
        {
            ...
            {
                var exception = new Exception(DictionaryMessages.Err_GettingData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "pyProyectoMvto_GetParameter");
                throw exception;
            }
        }
        ...
        public DTO_pyProyectoMvto pyProyectoMvto_GetByConsecutivo(int? consecMvto)
        {
            ...
            {
                this._dal_pyProyectoMvto = (DAL_pyProyectoMvto)this.GetInstance(typeof(DAL_pyProyectoMvto), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                return this._dal_pyProyectoMvto.DAL_pyProyectoMvto_GetByConsecutivo(consecMvto);
            }
            catch (Exception ex)
            {
                var exception = new Exception(DictionaryMessages.Err_GettingData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "pyProyectoMvto_GetByConsecutivo");
                throw exception;
            }
        }
        #endregion
        ...
        /// <returns></returns>
        public DTO_SerializedObject ActaTrabajo_Add(int documentoID, DTO_glDocumentoControl docCtrl, List<DTO_pyActaTrabajoDeta> actasList, DTO_coProyecto proy, bool update, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            try
            {
                ...
                else
                {
                    ...
                    this._dal_pyActaTrabajoDet.DAL_pyActaTrabajoDeta_DeleteByNumeroDoc(numDoc);
                    foreach (DTO_pyActaTrabajoDeta acta in actasList.FindAll(x => x.CantidadREC.Value > 0))
                    {
                        acta.NumeroDoc.Value = numDoc;
                        this._dal_pyActaTrabajoDet.DAL_pyActaTrabajoDeta_Add(acta);
                ...
                proy.CtrlVersion.Value = proy.CtrlVersion.Value++;
                this._dal_MasterHierarchy.DocumentID = AppMasters.coProyecto;
                result = this._dal_MasterHierarchy.DAL_MasterSimple_Update(proy, true);
                if (result.Result == ResultValue.NOK)
                    return result;
        ...
        public DTO_SerializedObject ActaTrabajo_ApproveRecibidoBS(int documentID, DTO_SolicitudTrabajo proyecto, DTO_glDocumentoControl ctrlActa, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                foreach (string prov in proveedorDistinct)
                {
                    ...
                    ctrlRec.Iva.Value = 0;
                    DTO_prProveedor proveedor = (DTO_prProveedor)this.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.prProveedor, prov.Split('-').First(), true, false);
                    ctrlRec.TerceroID.Value = proveedor != null? proveedor.TerceroID.Value : string.Empty;
                    #endregion
                    #region Asigna el Header(prRecibidoDocu)
                    DTO_coProyecto proy = (DTO_coProyecto)this.GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.coProyecto, proyecto.DocCtrl.ProyectoID.Value, true, false);
                    headerRec.ProveedorID.Value = prov.Split('-').First();
                    ...
                    foreach (DTO_pyActaTrabajoDeta acta in actasByProv)
                    {
                        ...
                        resumen.OrdCompraDocuID.Value = acta.NumDocOrdCompra.Value;
        ...
        /// <returns>Objeto con todos los datos</returns>
        public List<DTO_pyActaTrabajoDeta> pyActaTrabajoDeta_GetByParameter(DTO_pyActaTrabajoDeta filter)
        {
            try
            {
                this._dal_pyActaTrabajoDet = (DAL_pyActaTrabajoDeta)this.GetInstance(typeof(DAL_pyActaTrabajoDeta), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                return this._dal_pyActaTrabajoDet.DAL_pyActaTrabajoDeta_GetByParameter(filter);
            }
            catch (Exception ex)
            {
                var exception = new Exception(DictionaryMessages.Err_GettingData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "pyActaTrabajoDeta_GetByParameter");
                throw exception;
            }
        }
        #endregion
        ...
        public List<DTO_QueryComiteTecnico> pyProyectoDocu_GetAllProyectos(DateTime fechaCorte, bool loadTareas, bool loadCompras, bool loadFinanciero)
        {
            try
            {
                ...
                if (loadTareas)
                {
                    this._dal_pyProyectoMvto = (DAL_pyProyectoMvto)this.GetInstance(typeof(DAL_pyProyectoMvto), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    this._dal_pyActaTrabajoDet = (DAL_pyActaTrabajoDeta)this.GetInstance(typeof(DAL_pyActaTrabajoDeta), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    result = this._dal_pyProyectoDocu.DAL_pyProyectoDocu_GetAllProyectosByTareas(fechaCorte);
                    foreach (DTO_QueryComiteTecnico q in result)
                    {
                        q.FechaInicio.Value = q.Detalle.Min(x => x.FechaInicio.Value);
                        q.FechaFin.Value = q.Detalle.Max(x => x.FechaFin.Value);

                        //Carga los movimientos que existen del Presupuesto
                        DTO_pyProyectoMvto filter = new DTO_pyProyectoMvto();
                        filter.NumeroDoc.Value = q.NumeroDoc.Value;
                        q.Movimientos = this.pyProyectoMvto_GetParameter(filter);     
                    }
                }
                if(loadCompras)
                {
                    result = this._dal_pyProyectoDocu.DAL_pyProyectoDocu_GetAllProyectosByCompras(fechaCorte);
                }
                if(loadFinanciero)
                {
        ...
        /// <returns></returns>
        public DTO_TxResult EntregasCliente_Add(int documentoID, int numDocProy, List<DTO_pyProyectoTareaCliente> tareasCliente, List<DTO_pyProyectoTarea> tareasProy, List<DTO_pyProyectoTarea> tareasAdicProy,List<int> entregasDelete, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            try
            {
                ...
                foreach (DTO_pyProyectoTareaCliente tarea in tareasCliente)
                {
                    ...
                    if (exist)
                    {
                        ...
                        #endregion
                        foreach (DTO_pyProyectoPlanEntrega entrega in tarea.Detalle.FindAll(x=>x.FechaEntrega.Value.HasValue))
                        {
                            //Valida si existe
                ...
                foreach (DTO_pyProyectoTareaCliente entregable in result)
                {
                    ...
                    entregable.Detalle.ForEach(x =>
                    {
                       ...
                       index++;
                        ...
                        acta.PorEntregaProg.Value = x.PorEntrega.Value;
                        ...
                        acta.Estado.Value = acta.NumeroDoc.Value.HasValue ? listActaDetaExist.Find(y => y.NumeroDoc.Value == acta.NumeroDoc.Value).Estado.Value : 0;
                        ...
                        acta.PorPendiente.Value = acta.PorEntregaProg.Value - acta.PorEntregado.Value;
                        acta.PorAEntregar.Value = 0;
                        acta.Cantidad.Value = x.Cantidad.Value;
                        acta.CantProgramada.Value = x.Cantidad.Value;//Cantidad real planeada
                        ...
                        if (!string.IsNullOrEmpty(x.ValorFactura.Value.ToString()))
                            acta.ValorFactura.Value = x.ValorFactura.Value;
                        else
                            acta.ValorFactura.Value = 0;
                        ...
                        if(acta.Cantidad.Value > 0)
                            entregable.DetalleActas.Add(acta);

                    });
                    entregable.ValorFactura.Value = entregable.Detalle.Sum(x => x.ValorFactura.Value);
                    var rows = this._dal_pyActaEntregaDeta.DAL_pyActaEntregaDeta_Get(null, numDocProy, entregable.Consecutivo.Value, null,true);
                    ...
                    entregable.PorEntregado.Value = rows.Sum(x => x.PorEntregado.Value);
        ...
        /// </summary>
        /// <param name="documentoID">documento ID</param>
        /// <param name="docCtrlActa">doc control</param>
        /// <param name="listEntregables">lista de actas</param>
        /// <returns>resultado</returns>
        public DTO_SerializedObject ActaEntrega_Add(int documentoID, DTO_glDocumentoControl docCtrlActa, List<DTO_pyProyectoTareaCliente> listEntregables, DTO_coProyecto proy, bool update, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            try
            {
                ...
                if (!update)
                {
                    docCtrlActa.NumeroDoc.Value = 0;
                    resultGLDC = this._moduloGlobal.glDocumentoControl_Add(AppDocuments.ActaEntrega, docCtrlActa, true);
                    if (resultGLDC.Message != ResultValue.OK.ToString())
                    {
                        result.Result = ResultValue.NOK;
                        result.ResultMessage = DictionaryMessages.Err_AddDocument;
                    ...
                    numDoc = Convert.ToInt32(resultGLDC.Key);
                    docCtrlActa.NumeroDoc.Value = numDoc;

                    #region Guarda en pyActaEntregaDeta
                    ...
                    foreach (DTO_pyProyectoTareaCliente tarea in listEntregables)
                    {
                        foreach (DTO_pyActaEntregaDeta acta in tarea.DetalleActas.FindAll(x=>x.PorAEntregar.Value != 0))
                        {
                            ...
                            acta.Cantidad.Value = acta.CantAEntregar.Value;
                            acta.PorEntregado.Value = acta.PorAEntregar.Value;
                ...
                {
                    resultGLDC = new DTO_TxResultDetail();
                    resultGLDC = this._moduloGlobal.glDocumentoControl_Update(docCtrlActa, true, true);
                    if (resultGLDC.Message != ResultValue.OK.ToString())
                    {
                        result.Result = ResultValue.NOK;
                    ...
                    #region Actualiza en pyActaEntregaDeta
                    numDoc = docCtrlActa.NumeroDoc.Value.Value;
                    this._dal_pyActaEntregaDeta.DAL_pyActaEntregaDeta_DeleteByNumeroDoc(numDoc);
                    foreach (DTO_pyProyectoTareaCliente tarea in listEntregables)
                    {
                        foreach (DTO_pyActaEntregaDeta acta in tarea.DetalleActas.FindAll(x => x.PorAEntregar.Value != 0))
                        {
                ...
                if (result.Result == ResultValue.OK)
                {
                    ...
                    alarma = this.GetFirstMailInfo(numDoc, true);
                    alarma.NumeroDoc = numDoc.ToString();
                    alarma.PrefijoID = docCtrlActa.PrefijoID.Value.TrimEnd();
                    alarma.Consecutivo = docCtrlActa.DocumentoNro.Value.ToString();
                    return alarma;
                    #endregion
                }
            ...
            finally
            {
                if (result.Result == ResultValue.OK)
                {
                    if (!insideAnotherTx)
                    {
                        ...
                        if (docCtrlActa.DocumentoNro.Value == 0)
                        {
                            docCtrlActa.DocumentoNro.Value = Convert.ToInt32(this.GenerarDocumentoNro(documentoID, docCtrlActa.PrefijoID.Value));
                            this._moduloGlobal.ActualizaConsecutivos(docCtrlActa, true, false, false);
                        }
                        result.ExtraField = docCtrlActa.DocumentoNro.Value.ToString();
                        alarma.Consecutivo = docCtrlActa.DocumentoNro.Value.ToString();
                    }
                }
                else if (base._mySqlConnectionTx != null)
        ...
        /// <returns>resultado</returns>
        public DTO_SerializedObject ActaEntrega_Aprobar(int documentoID, DTO_glDocumentoControl docCtrl,DTO_coProyecto proy, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            try
            {
                ...
                DTO_TxResultDetail resultGLDC = new DTO_TxResultDetail();
                //docCtrl.ConsSaldo.Value = docCtrl.ConsSaldo.Value + 1;
                //resultGLDC = this._moduloGlobal.glDocumentoControl_Update(docCtrl, true,true);
                //if (resultGLDC.Message != ResultValue.OK.ToString())
                //{
                //    result.Result = ResultValue.NOK;
                //    result.ResultMessage = DictionaryMessages.Err_UpdateDocument;
                ...
                this._moduloGlobal.glDocumentoControl_ChangeDocumentStatus(documentoID, docCtrl.NumeroDoc.Value.Value, EstadoDocControl.Aprobado, "", true);
        ...
        public DTO_SerializedObject ActaEntrega_ApprovePreFactura(int documentID,DTO_glDocumentoControl ctrlProy,DTO_pyProyectoDocu docuProy, List<DTO_pyProyectoTareaCliente> listEntregables,List<DTO_faFacturacionFooter> listDetalleFact,Dictionary<Tuple<int, int>, int> batchProgress)
        {
            ...
            try
            {
                ...
                if (result.Result == ResultValue.OK)
                {
                    ...
                    header.Retencion10.Value = docuProy.VlrAnticipoFactVenta.Value;
                    header.ValorAnticipo = docuProy.VlrAnticipoFactVenta.Value;
                    header.DatoAdd5.Value = docuProy.VlrPorcAnticipo.ToString()+"%";
                    header.FacturaFijaInd.Value = false;
                    header.RteGarantiaIvaInd.Value = docuProy.RteGarantiaIvaInd.Value;
                    header.PorcRteGarantia.Value = docuProy.PorcRteGarantia.Value;
                    #endregion
                    #region Valida si el proyecto es AIU
                    if (docuProy != null && docuProy.APUIncluyeAIUInd.Value.Value)
                    {
                        header.PorcAdministracion = docuProy.PorEmpresaADM.Value.Value;
                        header.PorcImprevistos = docuProy.PorEmpresaIMP.Value.Value;
                        header.PorcUtilidad = docuProy.PorEmpresaUTI.Value.Value;

                        header.Administracion.Value = Math.Round(listDetalleFact.Sum(x => x.ValorBruto) * (header.PorcAdministracion / 100), 0);
                        header.Imprevistos.Value = Math.Round(listDetalleFact.Sum(x => x.ValorBruto) * (header.PorcImprevistos / 100), 0);
                        header.Utilidad.Value = Math.Round(listDetalleFact.Sum(x => x.ValorBruto) * (header.PorcUtilidad / 100), 0);
                        header.Iva.Value = docuProy.PorIVA.Value.HasValue ? Math.Round(header.Utilidad.Value.Value * (docuProy.PorIVA.Value.Value / 100), 0) : 0;
                        header.Valor.Value = header.Valor.Value + header.Iva.Value;
                    }
                    else
                    {
                        foreach (DTO_faFacturacionFooter d in listDetalleFact)
                        {
                            d.PorcIVA = docuProy.PorIVA.Value.HasValue ? docuProy.PorIVA.Value.Value : 0;
                            d.ValorIVA = Math.Round((d.PorcIVA / 100) * d.ValorBruto, 0);
                            d.ValorTotal = d.ValorBruto + d.ValorIVA;
                            d.ValorNeto = d.ValorTotal;
                        }
                        header.Iva.Value = listDetalleFact.Sum(x => x.ValorIVA);
                        header.Valor.Value = header.Valor.Value + header.Iva.Value;
                    }

                    //Valida si tiene retegarantia para calcularla
                    if (header.PorcRteGarantia.Value > 0)
                    {
                        if (header.RteGarantiaIvaInd.Value.HasValue && header.RteGarantiaIvaInd.Value.Value)
                            header.ValorRteGarantia = Math.Round(listDetalleFact.Where(x => x.Movimiento.ImprimeInd.Value.Value).Sum(x => x.ValorBruto + x.ValorIVA) * (header.PorcRteGarantia.Value.Value / 100), 0);
                        else
                            header.ValorRteGarantia = Math.Round(listDetalleFact.Where(x => x.Movimiento.ImprimeInd.Value.Value).Sum(x => x.ValorBruto) * (header.PorcRteGarantia.Value.Value / 100), 0);
                        header.Retencion4.Value = header.ValorRteGarantia;
                    }
                    #endregion
        ...
        public List<DTO_pyProyectoModificaFechas> pyProyectoModificaFechas_GetByNumeroDoc(int numeroDoc, string tarea)
        {
            try
            {
                this._dal_pyProyectoModificaFechas = (DAL_pyProyectoModificaFechas)this.GetInstance(typeof(DAL_pyProyectoModificaFechas), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                List<DTO_pyProyectoModificaFechas> tareas = this._dal_pyProyectoModificaFechas.DAL_pyProyectoModificaFechas_GetByNumeroDoc(numeroDoc, tarea).ToList();
                if (numeroDoc != null)
                {
            ...
            {
                var exception = new Exception(DictionaryMessages.Err_GettingData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "pyProyectoModificaFechas_Get");
                throw exception;
            }
        }
        ...
        {
            this._dal_pyProyectoModificaFechas = (DAL_pyProyectoModificaFechas)base.GetInstance(typeof(DAL_pyProyectoModificaFechas), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
            return this._dal_pyProyectoModificaFechas.DAL_pyProyectoModificaFechas_Load(numeroDoc);
        }

        public DTO_TxResult pyProyectoModificaFechas_Add(DTO_pyProyectoModificaFechas datos, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            {
                this._dal_pyProyectoModificaFechas = (DAL_pyProyectoModificaFechas)base.GetInstance(typeof(DAL_pyProyectoModificaFechas), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyProyectoModificaFechas.DAL_pyProyectoModificaFechas_Add(datos);
        ...
        public DTO_TxResult pyProyectoModificaFechas_Upd(DTO_pyProyectoModificaFechas datos, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            {
                this._dal_pyProyectoModificaFechas = (DAL_pyProyectoModificaFechas)base.GetInstance(typeof(DAL_pyProyectoModificaFechas), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyProyectoModificaFechas.DAL_pyProyectoModificaFechas_Upd(datos);
            ...
            catch (Exception ex)
            {
                result.Result = ResultValue.NOK;
                result.ResultMessage = ex.Message;
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, ex, this.UserId.ToString(), "SolicitudProyecto_Upd");
                return result;
            }
            finally
            {
        ...
        /// <returns>Retorna el resultado de la operacion</returns>
        public DTO_TxResult MigracionLocacionEntregas(int documentID,int numDocProy, List<DTO_pyProyectoLocacion> listLocaciones, List<DTO_pyProyectoIngDetalleTarea> listLocTareas, List<DTO_pyProyectoIngDetalleDeta> listLocRecursos, List<DTO_pyProyectoEntregasxMes> listEntregas, Dictionary<Tuple<int, int>, int> batchProgress)
        {
            base._mySqlConnectionTx = base._mySqlConnection.BeginTransaction();
            ...
            try
            {
                ...
                this._moduloGlobal = (ModuloGlobal)base.GetInstance(typeof(ModuloGlobal), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_MasterSimple = (DAL_MasterSimple)base.GetInstance(typeof(DAL_MasterSimple), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyProyectoLocacion = (DAL_pyProyectoLocacion)base.GetInstance(typeof(DAL_pyProyectoLocacion), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyProyectoIngTareas = (DAL_pyProyectoIngDetalleTarea)base.GetInstance(typeof(DAL_pyProyectoIngDetalleTarea), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyProyectoIngDeta = (DAL_pyProyectoIngDetalleDeta)base.GetInstance(typeof(DAL_pyProyectoIngDetalleDeta), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyProyectoEntregaxMes = (DAL_pyProyectoEntregasxMes)base.GetInstance(typeof(DAL_pyProyectoEntregasxMes), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyProyectoTarea = (DAL_pyProyectoTarea)base.GetInstance(typeof(DAL_pyProyectoTarea), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyProyectoDeta = (DAL_pyProyectoDeta)base.GetInstance(typeof(DAL_pyProyectoDeta), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_pyActaEntregaDeta = (DAL_pyActaEntregaDeta)base.GetInstance(typeof(DAL_pyActaEntregaDeta), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);

                int diasTerminacionTrabajos = !string.IsNullOrEmpty(this.GetControlValueByCompany(ModulesPrefix.py, AppControl.py_DiasTerminaTrabajoEntrega))? Convert.ToInt32(this.GetControlValueByCompany(ModulesPrefix.py, AppControl.py_DiasTerminaTrabajoEntrega)) :0;
                List<DTO_pyProyectoTarea> listTareasProy = this._dal_pyProyectoTarea.DAL_pyProyectoTarea_Get(numDocProy,string.Empty);

                int i = 0;
                #region Valida las locaciones
                if (listLocaciones != null && listLocaciones.Count > 0)
                {
                    List<DTO_pyProyectoLocacion> locacionExist = this._dal_pyProyectoLocacion.DAL_pyProyectoLocacion_GetByNumDocProy(numDocProy);
                    foreach (DTO_pyProyectoLocacion loc in locacionExist)
                    {
                        bool exist = this._dal_pyProyectoIngTareas.DAL_pyProyectoIngDetalleTarea_ExistByLocacion(loc.LocacionID.Value);
                        if (!exist)
                            this._dal_pyProyectoLocacion.DAL_pyProyectoLocacion_Delete(loc.Consecutivo.Value.Value);
                    }

                    foreach (DTO_pyProyectoLocacion loc in listLocaciones)
                    {
                        int percent = ((i + 1) * 100) / listLocaciones.Count;
                        batchProgress[tupProgress] = percent;

                        DTO_pyProyectoLocacion exist = this._dal_pyProyectoLocacion.DAL_pyProyectoLocacion_GetByLocacion(numDocProy, loc.LocacionID.Value);
                        if (exist == null)
                        {
                            #region Crea la nueva locacion
                            exist = new DTO_pyProyectoLocacion();
                            exist.NumeroDoc.Value = numDocProy;
                            exist.LocacionID.Value = loc.LocacionID.Value;
                            this._dal_pyProyectoLocacion.DAL_pyProyectoLocacion_Add(exist);
                            #endregion                            
                        }                      
                        i++;
                    }
                ...
                #region Valida las tareas
                if (listLocTareas != null && listLocTareas.Count > 0)
                {
                    this._dal_pyProyectoIngTareas.DAL_pyProyectoIngDetalleTarea_DeleteByNumeroDoc(numDocProy);
                    foreach (DTO_pyProyectoIngDetalleTarea loc in listLocTareas)
                    {
                        int percent = ((i + 1) * 100) / listLocTareas.Count;
                        batchProgress[tupProgress] = percent;
                        DTO_pyProyectoIngDetalleTarea exist = this._dal_pyProyectoIngTareas.DAL_pyProyectoIngDetalleTarea_GetByTarea(numDocProy, loc.ConsecTarea.Value, loc.LocacionID.Value);
                        if (exist == null)
                        {
                            #region Crea  nueva locacion de tarea
                            exist = new DTO_pyProyectoIngDetalleTarea();
                            exist.NumeroDoc.Value = numDocProy;
                            exist.LocacionID.Value = loc.LocacionID.Value;
                            exist.ConsecTarea.Value = loc.ConsecTarea.Value;
                            exist.Cantidad.Value = loc.Cantidad.Value;
                            this._dal_pyProyectoIngTareas.DAL_pyProyectoIngDetalleTarea_Add(exist);
                            #endregion                            
                        }
                        else
                        {
                            #region Actualiza tarea
                            exist.Cantidad.Value = loc.Cantidad.Value;
                            this._dal_pyProyectoIngTareas.DAL_pyProyectoIngDetalleTarea_Upd(exist);
                            #endregion
                        }
                        i++;
                        //Actualiza la tarea
                        DTO_pyProyectoTarea tareaProy = listTareasProy.Find(x => x.Consecutivo.Value == loc.ConsecTarea.Value);
                        tareaProy.Cantidad.Value = listLocTareas.FindAll(x => x.ConsecTarea.Value == loc.ConsecTarea.Value).Sum(y=>y.Cantidad.Value);
                        tareaProy.CostoTotalML.Value = tareaProy.Cantidad.Value * (tareaProy.CostoTotalUnitML.Value.HasValue? tareaProy.CostoTotalUnitML.Value : 0);
                        tareaProy.CostoLocalCLI.Value = tareaProy.Cantidad.Value * (tareaProy.CostoLocalUnitCLI.Value.HasValue ? tareaProy.CostoLocalUnitCLI.Value : 0);
                        this._dal_pyProyectoTarea.DAL_pyProyectoTarea_Upd(tareaProy);
                    }
                }
                #endregion                                

                #region Valida los APUs
                if (listLocRecursos != null && listLocRecursos.Count > 0)
                {
                    //Trae tareas locacion existentes
                    List<DTO_pyProyectoIngDetalleTarea> listLocTareasExist = this._dal_pyProyectoIngTareas.DAL_pyProyectoIngDetalleTarea_GetByParameter(numDocProy, null, string.Empty, null);

                    //Obtiene y elimina las locaciones que ya no existen en la ing detalle
                    List<string> locacionExist = this._dal_pyProyectoIngDeta.DAL_pyProyectoIngDetalleDeta_GetByLocacionInvalid(numDocProy);
                    foreach (var loc in locacionExist)
                        this._dal_pyProyectoIngDeta.DAL_pyProyectoIngDetalleDeta_DeleteByLocacion(loc);

                    //Obtiene las locaciones nuevas y las elimina la ing detalle para ingresar nuevamente
                    List<string> locacionesNuevas = listLocRecursos.Select(x => x.LocacionID.Value).Distinct().ToList();
                    foreach (var loc in locacionesNuevas)
                        this._dal_pyProyectoIngDeta.DAL_pyProyectoIngDetalleDeta_DeleteByLocacion(loc);

                    foreach (DTO_pyProyectoIngDetalleDeta loc in listLocRecursos)
                    {
                        int percent = ((i + 1) * 100) / listLocRecursos.Count;
                        batchProgress[tupProgress] = percent;
                        DTO_pyProyectoIngDetalleDeta exist = this._dal_pyProyectoIngDeta.DAL_pyProyectoIngDetalleDeta_GetByRecurso(numDocProy, loc.ConsecTarea.Value, loc.LocacionID.Value,loc.RecursoID.Value);
                        if (exist == null)
                        {
                            #region Crea nueva locacion de recurso
                            exist = new DTO_pyProyectoIngDetalleDeta();
                            exist.NumeroDoc.Value = numDocProy;
                            exist.LocacionID.Value = loc.LocacionID.Value;
                            exist.ConsecTarea.Value = loc.ConsecTarea.Value;
                            exist.RecursoID.Value = loc.RecursoID.Value;
                            exist.Cantidad.Value = loc.Cantidad.Value;
                            this._dal_pyProyectoIngDeta.DAL_pyProyectoIngDetalleDeta_Add(exist);
                            #endregion                            
                        }
                        else
                        {
                            #region Actualiza locacion recurso
                            exist.Cantidad.Value = loc.Cantidad.Value;
                            this._dal_pyProyectoIngDeta.DAL_pyProyectoIngDetalleDeta_Upd(exist);
                            #endregion
                        }
                        //Actualiza el recurso
                        DTO_pyProyectoDeta deta = this._dal_pyProyectoDeta.DAL_pyProyectoDeta_GetByRecursoTarea(loc.ConsecTarea.Value.Value,loc.RecursoID.Value);
                        if (deta != null)
                        {
                            decimal? cantTarea = listLocTareasExist.FindAll(x => x.ConsecTarea.Value == loc.ConsecTarea.Value).Sum(y => y.Cantidad.Value);
                            decimal? cantRecurso = listLocRecursos.FindAll(x => x.ConsecTarea.Value == loc.ConsecTarea.Value && x.RecursoID.Value == loc.RecursoID.Value).Sum(y => y.Cantidad.Value);
                            deta.FactorID.Value = cantTarea != 0 ? Math.Round((decimal)(cantRecurso /cantTarea), 8) : deta.FactorID.Value;
                            this._dal_pyProyectoDeta.DAL_pyProyectoDeta_Upd(deta);
                        }
                        i++;
                    }
                }
                ...
                #region Valida entregas de tareas
                if (false && listEntregas != null && listEntregas.Count > 0)
                {
                    this._dal_pyProyectoEntregaxMes.DAL_pyProyectoEntregasxMes_Delete(numDocProy);
                    foreach (DTO_pyProyectoEntregasxMes loc in listEntregas)
                    {
                        int percent = ((i + 1) * 100) / listEntregas.Count;
                        batchProgress[tupProgress] = percent;
                        #region Crea  nueva entrega de tarea
                        DTO_pyProyectoEntregasxMes exist = new DTO_pyProyectoEntregasxMes();
                        exist.NumeroDoc.Value = numDocProy;
                        exist.Fecha.Value = loc.Fecha.Value;
                        exist.ConsecTarea.Value = loc.ConsecTarea.Value;
                        exist.Cantidad.Value = loc.Cantidad.Value;
                        this._dal_pyProyectoEntregaxMes.DAL_pyProyectoEntregasxMes_Add(exist);
                        #endregion                            
                        i++;
                        DTO_pyTarea dtoTar = (DTO_pyTarea)this.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.pyTarea, loc.TareaID.Value, true, false);
                        DTO_pyProyectoTarea tareaProy = listTareasProy.Find(x => x.Consecutivo.Value == loc.ConsecTarea.Value);
                        tareaProy.FechaFin.Value = listEntregas.Where(x => x.ConsecTarea.Value == loc.ConsecTarea.Value).Max(y => y.Fecha.Value);
                        //tareaProy.FechaInicio.Value = listEntregas.Where(x => x.ConsecTarea.Value == loc.ConsecTarea.Value).Min(y => y.Fecha.Value);
                        tareaProy.FechaInicio.Value = tareaProy.FechaFin.Value.Value.AddDays(diasTerminacionTrabajos*-1);
                        if(dtoTar != null && dtoTar.DiasInicioTrabajo.Value.HasValue)
                            tareaProy.FechaInicio.Value = tareaProy.FechaInicio.Value.Value.AddDays(dtoTar.DiasInicioTrabajo.Value.Value * -1);
                        tareaProy.DiasInicioTrabajo.Value = (int)(tareaProy.FechaFin.Value.Value - tareaProy.FechaInicio.Value.Value).TotalDays;
                        this._dal_pyProyectoTarea.DAL_pyProyectoTarea_Upd(tareaProy);
                    }

                    //Crea entregables proyecto
                    List<DTO_pyProyectoTareaCliente> listEntregablesExist = this.pyProyectoTareaCliente_GetByNumeroDoc(numDocProy, string.Empty, string.Empty);
                    List<DTO_pyProyectoTareaCliente> listEntregables = new List<DTO_pyProyectoTareaCliente>();
                    DTO_pyActaEntregaDeta filter = new DTO_pyActaEntregaDeta();
                    filter.NumDocProyecto.Value = numDocProy;
                    List<DTO_pyActaEntregaDeta> actasEntregaExist = this._dal_pyActaEntregaDeta.DAL_pyActaEntregaDeta_GetByParameter(filter);
                    if(actasEntregaExist.Count > 0)
                    {
                        result.Result = ResultValue.NOK;
                        result.ResultMessage = "No es posible crear Plan de entregas porque ya existe actas de entrega realizadas en el proyecto";
                        return result;
                    }
                    foreach (DTO_pyProyectoEntregasxMes entregaxMes in listEntregas)
                    {
                        DTO_pyProyectoTareaCliente entr = new DTO_pyProyectoTareaCliente();
                        DTO_pyProyectoTarea tarProy = listTareasProy.Find(x => x.Consecutivo.Value == entregaxMes.ConsecTarea.Value);
                        DTO_pyTarea tarea = (DTO_pyTarea)this.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.pyTarea, tarProy.TareaID.Value, true, false);
                        if (tarea != null && tarea.TipoTarea.Value == 1)
                        {
                            entr.NumeroDoc.Value = numDocProy;
                            entr.Descripcion.Value = tarProy.Descriptivo.Value;
                            entr.TareaEntregable.Value = string.IsNullOrEmpty(tarProy.TareaEntregable.Value) ? (listEntregables.Count + 1).ToString() : tarProy.TareaEntregable.Value;
                            ...
                            entr.MonedaFactura.Value = "COP";
                            if (listEntregablesExist.Exists(x => x.TareaEntregable.Value == entr.TareaEntregable.Value))
                            {
                                entr.Consecutivo.Value = listEntregablesExist.Find(x => x.Descripcion.Value == entr.Descripcion.Value && x.TareaEntregable.Value == entr.TareaEntregable.Value).Consecutivo.Value;
                                entr.Detalle = listEntregablesExist.Find(x => x.Descripcion.Value == entr.Descripcion.Value && x.TareaEntregable.Value == entr.TareaEntregable.Value).Detalle;
                                entr.DetalleActas = listEntregablesExist.Find(x => x.Descripcion.Value == entr.Descripcion.Value && x.TareaEntregable.Value == entr.TareaEntregable.Value).DetalleActas;
                                entr.DetalleTareas = listEntregablesExist.Find(x => x.Descripcion.Value == entr.Descripcion.Value && x.TareaEntregable.Value == entr.TareaEntregable.Value).DetalleTareas;
                                entr.Observaciones.Value = listEntregablesExist.Find(x => x.Descripcion.Value == entr.Descripcion.Value && x.TareaEntregable.Value == entr.TareaEntregable.Value).Observaciones.Value;
                                entr.FechaInicio.Value = listEntregablesExist.Find(x => x.Descripcion.Value == entr.Descripcion.Value && x.TareaEntregable.Value == entr.TareaEntregable.Value).FechaInicio.Value;
                                entr.FechaFinal.Value = listEntregablesExist.Find(x => x.Descripcion.Value == entr.Descripcion.Value && x.TareaEntregable.Value == entr.TareaEntregable.Value).FechaFinal.Value;
                                entr.ValorFactura.Value = listEntregablesExist.Find(x => x.Descripcion.Value == entr.Descripcion.Value && x.TareaEntregable.Value == entr.TareaEntregable.Value).ValorFactura.Value;
                            }
                            else
                                entr.Consecutivo.Value = this._dal_pyProyTareaCliente.DAL_pyProyectoTareaCliente_Add(entr);
                            
                            listEntregables.Add(entr);

                            //Borra los planes de entrega existentes
                            this._dal_pyProyPlanEntrega.DAL_pyProyectoPlanEntrega_DeleteByConsecTarea(entr.Consecutivo.Value);
                    ...
                    result = this.EntregasCliente_Add(AppDocuments.Entregables, numDocProy, listEntregables, listTareasProy, null, null, true);
            ...
            catch (Exception ex)
            {
                batchProgress[tupProgress] = 100;
                result.Result = ResultValue.NOK;
                result.ResultMessage = Mentor_Exception.LogException_Local(this.loggerConnectionStr, ex, this.UserId.ToString(), "MigracionLocacionEntregas");
                return result;
            }
            finally
            {
        ...
        public DTO_TxResult LocacionEntregasGetByProyecto(int documentID, int numDocProy, ref List<DTO_pyProyectoLocacion> listLocaciones, ref List<DTO_pyProyectoIngDetalleTarea> listLocTareas, ref List<DTO_pyProyectoIngDetalleDeta> listLocRecursos, ref List<DTO_pyProyectoEntregasxMes> listEntregas)
        {
            ...
            try
            {
                ...
                listLocaciones = this._dal_pyProyectoLocacion.DAL_pyProyectoLocacion_GetByParameter(numDocProy,null,string.Empty);
                listLocTareas = this._dal_pyProyectoIngTareas.DAL_pyProyectoIngDetalleTarea_GetByParameter(numDocProy,null, string.Empty,null);
                listLocRecursos = this._dal_pyProyectoIngDeta.DAL_pyProyectoIngDetalleDeta_GetByParameter(numDocProy,null, string.Empty, null, string.Empty);
                listEntregas = this._dal_pyProyectoEntregaxMes.DAL_pyProyectoEntregasxMes_GetByParameter(numDocProy,null, null, null);
        ...
        /// <returns>Retorna el resultado de la operacion</returns>
        public DTO_TxResult Proyectos_Revertir(int documentID, int numeroDoc, int? consecutivoPos, ref List<DTO_glDocumentoControl> ctrls, ref List<DTO_coComprobante> coComps, bool insideAnotherTx)
        {
            if (!insideAnotherTx)
                base._mySqlConnectionTx = base._mySqlConnection.BeginTransaction();
            ...
            try
            {
                ...
                List<DTO_pyProyectoMvto> mvtos = this._dal_pyProyectoMvto.DAL_pyProyectoMvto_GetByParameter(filter);
                DTO_glDocumentoControl ctrlSelect = this._moduloGlobal.glDocumentoControl_GetByID(numeroDoc);

                if (mvtos.Any(x=>x.CantidadPROV.Value > 0) || mvtos.Any(x => x.CantidadINV.Value > 0))
                {
                    result.Result = ResultValue.NOK;
                    DTO_TxResultDetailFields rdF = new DTO_TxResultDetailFields();
                    rdF.Field = "Solicitudes";
                    rdF.Message = "El proyecto no se puede revertir o anular porque tiene movimientos en proveedores o inventarios";
                ...
                else
                {
                    this._dal_pyProyectoDeta = (DAL_pyProyectoDeta)base.GetInstance(typeof(DAL_pyProyectoDeta), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    this._dal_pyProyectoTarea = (DAL_pyProyectoTarea)base.GetInstance(typeof(DAL_pyProyectoTarea), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    this._dal_pyProyectoDocu = (DAL_pyProyectoDocu)base.GetInstance(typeof(DAL_pyProyectoDocu), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    this._dal_pyProyectoDetaHist = (DAL_pyProyectoDetaHistoria)base.GetInstance(typeof(DAL_pyProyectoDetaHistoria), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    this._dal_pyProyectoTareaHist = (DAL_pyProyectoTareaHistoria)base.GetInstance(typeof(DAL_pyProyectoTareaHistoria), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);

                    DTO_SolicitudTrabajo proyecto = this.SolicitudProyecto_Load(AppDocuments.Proyecto,string.Empty,null,numeroDoc,string.Empty,string.Empty,false,false,false,false,false);
                    this._moduloGlobal.glDocumentoControl_ChangeDocumentStatus(AppDocuments.PreProyecto, proyecto.HeaderProyecto.DocSolicitud.Value.Value, EstadoDocControl.SinAprobar, string.Empty, true);
                    foreach (DTO_pyProyectoTarea tarea in proyecto.DetalleProyecto)
                    {
                        foreach (DTO_pyProyectoDeta det in tarea.Detalle)
                        {
                            this._dal_pyProyectoDeta.DAL_pyProyectoDeta_DeleteByConsecutivo(det.Consecutivo.Value);
                        }
                        this._dal_pyProyectoTarea.DAL_pyProyectoTarea_DeleteByConsecutivo(tarea.Consecutivo.Value.Value);
                    }
                    this._dal_pyProyectoDetaHist.DAL_pyProyectoDetaHistoria_DeleteByNumeroDoc(numeroDoc);
                    this._dal_pyProyectoTareaHist.DAL_pyProyectoTareaHistoria_DeleteByNumeroDoc(numeroDoc);
                    this._dal_pyProyectoDocu.DAL_pyProyectoDocu_DeleteByNumeroDoc(numeroDoc);
                }
                ...
                ctrlSelect.ProyectoID.Value = null;
                this._moduloGlobal.glDocumentoControl_Update(ctrlSelect, false, true);
