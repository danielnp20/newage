...
namespace NewAge.Negocio
{
    public class ModuloGlobal : ModuloBase
    {
        ...
        public DTO_TxResultDetail ccCliente_AddFromSource(int documentoID, object data, bool insideAnotherTx)
        {
            ...
            try
            {
                if (documentoID == AppDocuments.RegistroSolicitud || documentoID == AppDocuments.DigitacionCreditoFinanciera || documentoID == AppDocuments.VerificacionPreliminar)
                {
                    ...
                    DTO_DigitaSolicitudCredito datos = (DTO_DigitaSolicitudCredito)data;
                    ...
                    foreach (DTO_ccSolicitudDatosPersonales persona in datos.DatosPersonales)
                    {
                        ...
                        if (coTercero == null)
                        {
                            ...
                            coTercero.LugarGeograficoID.Value = persona.CiudadExpDoc.Value;
                            ...
                            if (lugar == null)
                            {
                                ...
                                rdF.Message = "Ciudad Residencia no es válida, verifique";
                                result.Message = ResultValue.NOK.ToString();
                                result.DetailsFields.Add(rdF);
                                return result;                          
                            }
                            coTercero.Pais.Value = lugar.PaisID.Value;
                            DTO_glLugarGeografico lugarExp = (DTO_glLugarGeografico)this.GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.glLugarGeografico, persona.CiudadExpDoc.Value, true, false);
                            if (lugarExp == null)
                            {
                                result.DetailsFields = new List<DTO_TxResultDetailFields>();
                                DTO_TxResultDetailFields rdF = new DTO_TxResultDetailFields();
                                rdF.Field = "CiudadExpDoc";
                                rdF.Message = "Ciudad Expedición no es válida, verifique";
                                result.Message = ResultValue.NOK.ToString();
                                result.DetailsFields.Add(rdF);
                                return result;
                            }
                            ...
                            if (persona.TipoPersona.Value == 1) // Si es deudor
                            {
                                ...
                                cli.CiudadExpDoc.Value = persona.CiudadExpDoc.Value;
        ...
        internal List<DTO_glDocumentoControl> glDocumentoControl_GetByMigracionNomina(int documentID, DateTime periodo, string pagaduria)
        {
            this._dal_glDocumentoControl = (DAL_glDocumentoControl)base.GetInstance(typeof(DAL_glDocumentoControl), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
            return _dal_glDocumentoControl.DAL_glDocumentoControl_GetByMigracionNomina(documentID, periodo, pagaduria);
        }
        ...
        /// <returns>Retorna el resultado de la operacion</returns>
        public DTO_TxResultDetail glDocumentoControl_Add(int documentID, DTO_glDocumentoControl docCtrl, bool insideAnotherTx)
        {
            if (!insideAnotherTx)
                base._mySqlConnectionTx = base._mySqlConnection.BeginTransaction();
        ...
        public DTO_TxResult glActividadEstado_AddOrUpd(int documentID,DTO_InfoTarea actividad,bool update, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                {                        
                    ...
                    actEstado.Valor.Value = actividad.Valor.Value;
                    actEstado.seUsuarioID.Value = this.UserId;
        ...
        public string glActividadEstado_GetActFlujoByNumeroDoc(int numeroDoc, bool onlyAbiertas)
        {
            this._dal_glActividadEstado = (DAL_glActividadEstado)this.GetInstance(typeof(DAL_glActividadEstado), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
            return this._dal_glActividadEstado.DAL_glActividadEstado_GetActFlujoByNumeroDoc(numeroDoc,onlyAbiertas);
        }
        #endregion
        ...
        /// </summary>
        public DTO_TxResult glGarantiaControl_Add(int documentID, List<DTO_glGarantiaControl> garantias,List<int> itemsDelete, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            Tuple<int, int> tupProgress = new Tuple<int, int>(this.UserId, documentID);
            ...
            try
            {
                ...
                foreach (int del in itemsDelete)
                    this._dal_garantiaControl.DAL_glGarantiaControl_Delete(del);
       ...
       /// <returns></returns>
        public DTO_TxResult glIncumpleCambioEstado_Update(int documentID, List<DTO_GestionCobranza> gestionList, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                foreach (var item in gestionList)
                {
                    ...
                    incump.Valor1.Value = item.Valor.Value;
        ...
        public List<DTO_glMovimientoDeta> glMovimientoDeta_GetByParameter(DTO_glMovimientoDeta filter, bool isPre,bool onlyInventario = false)
        {
            ...
            List<DTO_glMovimientoDeta> result = this._dal_glMovimientoDeta.DAL_glMovimientoDeta_GetByParameter(filter,isPre, onlyInventario);
            try
            {               
                ...
                if (filter.FechaIni.Value.HasValue)
                    result = result.FindAll(x => x.Fecha.Value.Value.Date >= filter.FechaIni.Value.Value.Date).ToList();
                if (filter.FechaFin.Value.HasValue)
                    result = result.FindAll(x => x.Fecha.Value.Value.Date <= filter.FechaFin.Value.Value.Date).ToList();
            }
            catch (Exception ex)
            {
                ;
        ...
        public DataTable glMovimientoDeta_GetByParameterExcel(DTO_glMovimientoDeta filter, bool isPre, bool onlyInventario  =false)
        {
            ...
            result = this._dal_glMovimientoDeta.DAL_glMovimientoDeta_GetByParameterExcel(filter, isPre, onlyInventario);
            return result;
        }
