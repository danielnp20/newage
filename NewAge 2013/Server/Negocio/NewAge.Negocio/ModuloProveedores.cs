...
namespace NewAge.Negocio
{
    public class ModuloProveedores : ModuloBase
    {
        ...
        private DAL_prComprasModificaFechas _dal_prComprasModificaFechas = null;
        ...
        public DTO_SerializedObject CierreDetalle_Guardar(int documentID, DTO_glDocumentoControl ctrl, List<DTO_prDetalleDocu> footer, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                foreach (DTO_prDetalleDocu det in footer)
                {
                    ...
                    {
                        //Valida si cierra el item o habilita la solicitud nuevamente(borra item OC)
                        if (det.SolicitarInd.Value.HasValue && det.SolicitarInd.Value.Value)
                            this._dal_prDetalleDocu.DAL_prDetalleDocu_Delete(det.OrdCompraDetaID.Value.Value);
                        else
                        {
                            det.ConsecutivoDetaID.Value = this._dal_prDetalleDocu.DAL_prDetalleDocu_Add(det);
                    ...
                    {
                        #region Actualiza movimientos de Proyectos si existen (pyProyectoMvto)
                ...
                #region Solicita nuevos items (Comentarios)
                if (false && documentID == AppDocuments.CierreDetalleOrdenComp && footer.Any(x => x.SolicitarInd.Value == true))
                {
                    ////Crea solicitudes nuevas
                    ...
                    //foreach (DTO_prDetalleDocu detaOld in footer.FindAll(x => x.SolicitarInd.Value == true))
                    //{
                    //    DTO_prSolicitudFooter det = new DTO_prSolicitudFooter();
                    //    det.DetalleDocu = this._dal_prDetalleDocu.DAL_prDetalleDocu_GetByID(detaOld.SolicitudDetaID.Value.Value);
                    //    det.DetalleDocu.ConsecutivoDetaID.Value = null;
                    //    det.DetalleDocu.NumeroDoc.Value = null;
                    //    det.DetalleDocu.SolicitudDocuID.Value = null;
                    //    det.DetalleDocu.SolicitudDetaID.Value = null;
                    //    det.DetalleDocu.DatoAdd3.Value = "";
                    //    det.DetalleDocu.CantidadSol.Value = detaOld.CantidadCierre.Value;

                    //    //Asigna Cargos de la Solicitud
                    //    List<DTO_prSolicitudCargos> cargos = this._dal_prSolicitudCargos.DAL_prSolicitudCargos_GetByID(detaOld.SolicitudDetaID.Value.Value);
                    //    foreach (var d in cargos)
                    //    {
                    //        d.ConsecutivoDetaID.Value = null;
                    //        d.NumeroDoc.Value = null;
                    //    }
                    //    det.SolicitudCargos = cargos;
                    //    footerSol.Add(det);
                    //}
                    ...
                    //ctrlSol.PeriodoDoc.Value = ctrl.PeriodoDoc.Value;
                    //ctrlSol.PeriodoUltMov.Value = ctrl.PeriodoDoc.Value;
                    //ctrlSol.FechaDoc.Value = ctrl.FechaDoc.Value;
                    //ctrlSol.AreaFuncionalID.Value = this.GetAreaFuncionalByUser();
                    //ctrlSol.PrefijoID.Value = ctrl.PrefijoID.Value;
                    //ctrlSol.MonedaID.Value = ctrl.MonedaID.Value;
                    //ctrlSol.TasaCambioCONT.Value = 0;
                    //ctrlSol.TasaCambioDOCU.Value = 0;
                    //ctrlSol.Estado.Value = (byte)EstadoDocControl.Aprobado;
                    //ctrlSol.ProyectoID.Value = ctrl.ProyectoID.Value;
                    //ctrlSol.CentroCostoID.Value = ctrl.CentroCostoID.Value;
                    //ctrlSol.LugarGeograficoID.Value = ctrl.LugarGeograficoID.Value;
                    ...
                    //ctrlSol.Descripcion.Value = "Creacion Solicitud OC x Cierre Det";
                    //ctrlSol.Observacion.Value = "Solicitud Orden de Compra x Cierre";
                    //ctrlSol.Valor.Value = 0;
                    //ctrlSol.Iva.Value = 0;
                    //#endregion
                    //#region Asigna el Header(prSolicitudDocu)
                    //string proyAdmin = this.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_ProyectoXDefecto);
                    //DTO_coProyecto proy = (DTO_coProyecto)this.GetMasterDTO(AppMasters.MasterType.Hierarchy, AppMasters.coProyecto, proyAdmin, true, false);
                    //headerSol.LugarEntrega.Value = proy != null ? proy.LocFisicaID.Value : string.Empty;
                    //headerSol.FechaEntrega.Value = ctrlSol.FechaDoc.Value;
                    ...
                    //headerSol.Ano.Value = ctrlSol.FechaDoc.Value.Value.Year;
                    //#endregion
                    //#region Crea Solicitud
                    //int numeroDoc = 0;
                    //var resultModOProveedor = this.Solicitud_Guardar(AppDocuments.Solicitud, ctrlSol, headerSol, null, footerSol, false, out numeroDoc, batchProgress, true);
        ...
        public List<DTO_prComprasModificaFechas> prComprasModificaFechas_GetByNumeroDoc(int numeroDoc,bool aprobadoDocInd)
        {
            try
            {
                this._dal_prComprasModificaFechas= (DAL_prComprasModificaFechas)this.GetInstance(typeof(DAL_prComprasModificaFechas), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                List<DTO_prComprasModificaFechas> tareas = this._dal_prComprasModificaFechas.DAL_prComprasModificaFechas_GetByNumeroDoc(numeroDoc, aprobadoDocInd).ToList();
                if (numeroDoc != null)
                {
            ...
            {
                var exception = new Exception(DictionaryMessages.Err_GettingData, ex);
                Mentor_Exception.LogException_Local(this.loggerConnectionStr, exception, this.UserId.ToString(), "prComprasModificaFechas_Get");
                throw exception;
            }
        }
        public DTO_prComprasModificaFechas prComprasModificaFechas_Load(int numeroDoc)
        {
            this._dal_prComprasModificaFechas = (DAL_prComprasModificaFechas)base.GetInstance(typeof(DAL_prComprasModificaFechas), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
            return this._dal_prComprasModificaFechas.DAL_prComprasModificaFechas_Load(numeroDoc);
        }
        ...
        public DTO_TxResult prComprasModificaFechas_Add(DTO_prComprasModificaFechas datos, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            ...
            {
                this._dal_prComprasModificaFechas = (DAL_prComprasModificaFechas)base.GetInstance(typeof(DAL_prComprasModificaFechas), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_prComprasModificaFechas.DAL_prComprasModificaFechas_Add(datos);
        ...
        public DTO_TxResult prComprasModificaFechas_Upd(DTO_prComprasModificaFechas datos, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            {
                this._dal_prComprasModificaFechas = (DAL_prComprasModificaFechas)base.GetInstance(typeof(DAL_prComprasModificaFechas), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_prComprasModificaFechas.DAL_prComprasModificaFechas_Upd(datos);
                    ...
                    foreach (DTO_prSolicitudFooter itemFooter in footer)
                    {
                        ...
                        if (itemFooter.DetalleDocu.ConsecTarea.HasValue)
                        {
                            ...
                            {                             
                                ...
                                itemFooter.DetalleDocu.Documento4ID.Value = tarea.NumeroDoc.Value;//Guarda el numeroDoc del proyecto 
                                itemFooter.DetalleDocu.DatoAdd4.Value = "("  + tarea.TareaCliente.Value + ")";//Guarda el item de la tarea 
                                itemFooter.DetalleDocu.CantidadDoc4.Value = itemFooter.DetalleDocu.CantidadSol.Value;//Guarda la cantidad solicitada 
                                //Obtiene el conecutivo del mvto del recurso del proyecto
                                int consMvtoProy = itemFooter.DetalleDocu.Documento4ID.Value.HasValue? this._dal_pyProyectoMvto.DAL_pyProyectoMvto_GetLastVersionByRecurso(itemFooter.DetalleDocu.inReferenciaID.Value, itemFooter.DetalleDocu.Documento4ID.Value.Value, itemFooter.DetalleDocu.Detalle5ID.Value) : 0;
                                itemFooter.DetalleDocu.Detalle4ID.Value = consMvtoProy != 0? consMvtoProy : itemFooter.DetalleDocu.Detalle4ID.Value;//Guarda el consecutivo del mvto del item 
                            }
                        }
                        #endregion
                        ...
                        if (itemFooter.DetalleDocu.CantidadSol.Value == 0)
                        {
                            result.Result = ResultValue.NOK;
                            result.ResultMessage = "La cantidad del item " + itemFooter.DetalleDocu.Descriptivo.Value + " debe ser diferente de 0";
                            numeroDoc = 0;
                            return result;
                        }
        ...
        public List<DTO_prOrdenCompraAprob> OrdenCompra_GetPendientesByModulo(int documentID, string actFlujoID, DTO_seUsuario usuario)
        {
            try
            {
                DTO_glDocumento documento = (DTO_glDocumento)GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.glDocumento, documentID.ToString(), true, false);
                this._moduloProyectos = (ModuloProyectos)base.GetInstance(typeof(ModuloProyectos), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                this._dal_prOrdenCompraDocu = (DAL_prOrdenCompraDocu)base.GetInstance(typeof(DAL_prOrdenCompraDocu), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                List<DTO_prOrdenCompraAprob> list = this._dal_prOrdenCompraDocu.DAL_prOrdenCompraDocu_GetPendientesByModulo(documento, actFlujoID, null, usuario);

                this._dal_prSolicitudCargos = (DAL_prSolicitudCargos)base.GetInstance(typeof(DAL_prSolicitudCargos), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);

                foreach (DTO_prOrdenCompraAprob doc in list)
                {
                    doc.OrdenCompraAprobDet = doc.OrdenCompraAprobDet.OrderBy(x => x.Descriptivo.Value).ToList();
                    foreach (DTO_prOrdenCompraAprobDet itemDet in doc.OrdenCompraAprobDet)
                        itemDet.SolicitudCargos = this._dal_prSolicitudCargos.DAL_prSolicitudCargos_GetByID(itemDet.SolicitudDetaID.Value.Value);
                    doc.FileUrl = base.GetFileRemotePath(doc.NumeroDoc.Value.ToString(), TipoArchivo.Documentos);

                    foreach (DTO_prOrdenCompraAprobDet det in doc.OrdenCompraAprobDet)
                    {
                        //DTO_SolicitudTrabajo proy = this._moduloProyectos.SolicitudProyecto_Load(AppDocuments.Proyecto, string.Empty, null, det.Documento4ID.Value, string.Empty, string.Empty, false, false, false, false, true);
                        //DTO_pyProyectoMvto filter = new DTO_pyProyectoMvto();
                        //filter.Consecutivo.Value = det.Detalle4ID.Value;
                        //filter.NumProyecto.Value = det.Documento4ID.Value;
                        //List<DTO_pyProyectoMvto> mvtoProy = this._moduloProyectos.pyProyectoMvto_GetParameter(filter);
                        //if(mvtoProy.Count > 0)
                        //{
                        //    //DTO_pyProyectoTarea tar = this._moduloProyectos.pyProyectoTarea_GetByConsecutivo(mvtoProy.First().ConsecTarea.Value.Value);
                        //    //DTO_pyProyectoDeta tar = this._moduloProyectos.pypr(mvtoProy.First().ConsecTarea.Value.Value);
                        //    //det.PresupuestoTotML.Value = 
                        //}
                        ...
                        //string recurso = det.inReferenciaID.Value;
                        //detaAll = detaAll.FindAll(x => x.RecursoID.Value == recurso).ToList();
                ...
                try
                {
                    ...
                    foreach (DTO_prOrdenCompraResumen itemFooter in footer)
                    {
                        if (itemFooter.CantidadRec.Value.Value > 0)
                        {
                            ...
                            var ordDocu = this.prOrdenCompraDocu_Get(itemFooter.OrdCompraDocuID.Value.Value);
                            ...
                            det.IVAUni.Value = Math.Round(detTemp.IVAUni.Value.Value / detTemp.CantidadxEmpaque.Value.Value,4);
                            if (monedaLocal.Equals(itemFooter.MonedaIDOC.Value))
                            {
                                ...
                                else
                                {
                                    ...
                                    det.ValorTotME.Value = ordDocu.TasaOrden.Value != 0 && ordDocu.TasaOrden.Value != null? Math.Round(det.ValorTotML.Value.Value / ordDocu.TasaOrden.Value.Value,4) : 0;//  detTemp.CantidadOC.Value != 0 ? (detTemp.ValorTotME.Value / (detTemp.CantidadOC.Value / detTemp.CantidadxEmpaque.Value)) * itemFooter.CantidadRec.Value : 0;
                                    det.IvaTotME.Value = ordDocu.TasaOrden.Value != 0 && ordDocu.TasaOrden.Value != null ? Math.Round(det.IvaTotML.Value.Value / ordDocu.TasaOrden.Value.Value, 4) : 0;//detTemp.CantidadOC.Value != 0 ? detTemp.IvaTotME.Value / (detTemp.CantidadOC.Value / detTemp.CantidadxEmpaque.Value) * itemFooter.CantidadRec.Value : 0;
                                }
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    det.ValorTotML.Value = ordDocu.TasaOrden.Value != null? det.ValorTotME.Value * ordDocu.TasaOrden.Value : 0;//  Math.Round(detTemp.CantidadOC.Value != 0 ? (detTemp.ValorTotML.Value.Value / (detTemp.CantidadOC.Value.Value / detTemp.CantidadxEmpaque.Value.Value)) * itemFooter.CantidadRec.Value.Value : 0, 2);
                                    det.IvaTotML.Value = ordDocu.TasaOrden.Value != null ? det.IvaTotME.Value * ordDocu.TasaOrden.Value : 0; // Math.Round(detTemp.CantidadOC.Value != 0 ? detTemp.IvaTotML.Value.Value / (itemFooter.CantidadOC.Value.Value / detTemp.CantidadxEmpaque.Value.Value) * itemFooter.CantidadRec.Value.Value : 0, 2);
                                }
                            }
                            ...
                            if (itemFooter.ClaseBS == TipoCodigo.Inventario)
                            {
                                ...
                                movDet.DescripTExt.Value = det.Descriptivo.Value;
        ...
        public List<DTO_prRecibidoAprob> Recibido_GetRecibidoNoFacturado(int documentID, string actFlujoID, string proveedor, bool loadIVA, int NroDocfacturaExist)
        {
            try
            {
                DTO_glDocumento doc = (DTO_glDocumento)GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.glDocumento, documentID.ToString(), true, false);
                ...
                if (loadIVA)
                {
                    ...
                    foreach (DTO_prRecibidoAprob rec in list)
                    {
                        foreach (DTO_prRecibidoAprobDet det in rec.Detalle)
                        {
                            ...
                            #endregion
                            #region Trae OC Relacionada
                            if (det.OrdCompraDocuID.Value.HasValue)
                            {
                                var ctrlOC = this._moduloGlobal.glDocumentoControl_GetByID(det.OrdCompraDocuID.Value.Value);
                                det.PrefDoc = ctrlOC != null ? ctrlOC.PrefDoc.Value : string.Empty;
                            }
                            #endregion
                        }
                    }        
                        ...
                        if (recibido.Seleccionar.Value.Value)
                        {
                            ...
                            foreach (DTO_prDetalleDocu det in detRecibido)
                            {
                                ...
                                if (recDet != null)
                                {
                                    ...
                                    det.VlrLocal02.Value = recDet.ValorMLDet.Value;
                                    det.VlrExtra02.Value = recDet.ValorMEDet.Value;
                                    det.VlrLocal03.Value = recDet.ValorIvaMLDet.Value;
                                    det.VlrExtra03.Value = recDet.ValorIvaMEDet.Value;
                                }
                                this._dal_prDetalleDocu.DAL_prDetalleDocu_Update(det);
                            }
                        }
                ...
                if (result.Result == ResultValue.OK)
                {
                    if (!insideAnotherTx)
                    {
                        ...
                        if (!update)
                        {
                            ...
                            cxpNroRadica.TerceroID.Value = _dtoCtrlCxP.TerceroID.Value;
                    ...
                    else if (ctrlSelect.DocumentoID.Value == (byte)AppDocuments.Recibido)
                    {
                        ...
                        else
                        {
                            ...
                            if (footer.Detalle4ID.Value != null && footer.Documento4ID.Value != null)
                            {
                                ...
                                if (mvto != null)
                                {
                                    footer.CantidadDoc4.Value = footer.CantidadDoc4.Value ?? 0;
                                    mvto.CantidadREC.Value = mvto.CantidadREC.Value - Math.Abs(footer.CantidadDoc4.Value.Value);
                                    this._dal_pyProyectoMvto.DAL_pyProyectoMvto_Upd(mvto);
                                }
                            }
        ...
        /// Anula Proveedores
        /// </summary>
        /// <param name="numDocFacts">nums de las Proveedores a anular</param>
        /// <returns>Resultado</returns>
        public DTO_TxResult Proveedoress_Anular(int documentID, List<int> numDocs, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            result.Details = new List<DTO_TxResultDetail>();
            ...
            try
            {
                ...
                this._moduloGlobal = (ModuloGlobal)base.GetInstance(typeof(ModuloGlobal), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                result = this._moduloGlobal.glDocumentoControl_Anular(documentID, numDocs, batchProgress, true);
                ...
                foreach (int num in numDocs)
                {
                    ...
                    footerDeta = this.prDetalleDocu_GetByNumeroDoc(num,false);
                                    
                    foreach (DTO_prDetalleDocu d in footerDeta)
                    {
                        #region Solicitud
                        if (ctrl.DocumentoID.Value == (byte)AppDocuments.Solicitud)
                        {
                            //Valida si la solicitud viene del modulo de Proyectos
                            if (d.Detalle4ID.Value != null && d.Documento4ID.Value != null)
                            {
                                //Carga el movimiento de proyectos    
                                this._dal_pyProyectoMvto = (DAL_pyProyectoMvto)base.GetInstance(typeof(DAL_pyProyectoMvto), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                                DTO_pyProyectoMvto mvto = this._dal_pyProyectoMvto.DAL_pyProyectoMvto_GetByConsecutivo(d.Detalle4ID.Value.Value);
                                if (mvto != null)
                                {
                                    mvto.CantidadPROV.Value = mvto.CantidadPROV.Value - d.CantidadDoc4.Value;
                                    this._dal_pyProyectoMvto.DAL_pyProyectoMvto_Upd(mvto);
                                }
                            }
                        }
                        #endregion
                        this._dal_prDetalleDocu.DAL_prDetalleDocu_Delete(d.ConsecutivoDetaID.Value.Value);
                    }
                }
                    ...
                    foreach (DTO_glDocumentoControl doc in ctrls)
                    {
                        ...
                        if (oc != null)
                        {
                            ...
                            foreach (DTO_prOrdenCompraFooter footer in oc.Footer)
                            {
                                ...
                                var docSol = this._moduloGlobal.glDocumentoControl_GetByID(consultaDet.SolicitudDocuID.Value.Value);
                                consultaDet.PrefDocOrig = docSol.PrefDoc.Value;
                                consultaDet.SolicitudDetaID.Value = footer.DetalleDocu.SolicitudDetaID.Value;
                    ...
                    foreach (DTO_glDocumentoControl doc in ctrls)
                    {
                        ...
                        if (rec != null)
                        {
                            ...
                            foreach (DTO_prOrdenCompraFooter footer in rec.Footer)
                            {
                                ...
                                var docSol = this._moduloGlobal.glDocumentoControl_GetByID(consultaDet.OrdCompraDocuID.Value.Value);
                                consultaDet.PrefDocOrig = docSol.PrefDoc.Value;
                                consultaDet.OrdCompraDetaID.Value = footer.DetalleDocu.OrdCompraDetaID.Value;
        ...
        /// <returns>Listado de DTO</returns>
        public List<DTO_ProveedoresTotal> ReportesProveedores_OrdenCompraArchivo(DateTime FechaIni, DateTime FechaFin, string Proveedor, Dictionary<int, string> filtros, bool isDetallado, string Moneda)
        {
            try
            {
                List<DTO_ProveedoresTotal> result = new List<DTO_ProveedoresTotal>();
                ...
                ordComp.DetallesOrdenCompra = this._dal_ReportesProveedores.DAL_ReportesProveedores_OrdenCompraArchivo(FechaIni, FechaFin, Proveedor, filtros, isDetallado, Moneda);
        ...
        public List<DTO_ProveedoresTotal> ReportesProveedores_OrdenCompra(int numDoc)
        {
            try
            {
                ...
                List<string> distinctID = (from c in query select c.Referencia.Value).Distinct().ToList();
                int count = 1;
                foreach (string id in distinctID)
                {
                    List<string> distinctDesc = query.FindAll(x => x.Referencia.Value == id).Select(x=>x.Descripcion.Value).Distinct().ToList();
                    foreach (string desc in distinctDesc)
                    {
                        DTO_ReportOrdenCompraDoc r = new DTO_ReportOrdenCompraDoc();
                        r = query.Find(x => x.Referencia.Value == id && x.Descripcion.Value == desc);
                        r.Cantidad.Value = query.FindAll(x => x.Referencia.Value == id && x.Descripcion.Value == desc).Sum(y => y.Cantidad.Value);
                        r.ValorTotal.Value = query.FindAll(x => x.Referencia.Value == id && x.Descripcion.Value == desc).Sum(y => y.ValorTotal.Value);
                        r.ValorIVA.Value = query.FindAll(x => x.Referencia.Value == id && x.Descripcion.Value == desc).Sum(y => y.ValorIVA.Value);
        ...
        public List<DTO_ProveedoresTotal> ReportesProveedores_OrdenCompraAnexo(int numDoc)
        {
            try
            {
                ...
                foreach (var r in  res.DetallesOrdenCompraDoc)
                {
                    ...
                    //Trae el mvto del proyecto
                    DTO_pyProyectoMvto mvto = listMvto.Find(x => x.Consecutivo.Value == r.ConsProyMvto.Value);
                    ...
                    {
                        //Codigos BS
                        var items = mvto != null? listMvto.FindAll(x => x.RecursoID.Value == mvto.RecursoID.Value && x.CantidadPROV.Value > 0).ToList(): new List<DTO_pyProyectoMvto>();
                        items.ForEach(x => r.Item.Value += (x.TareaCliente.Value + "-"));
                    }
                    r.TareaDesc.Value = mvto!= null? mvto.TareaDesc.Value : string.Empty;
        ...
        /// <param name="isDetallado">Verifica que tipo de reporte mostrar (True: Muestra Orden Compra Detallada, False: Muestra Orden Compra Resumida)</param>
        /// <returns></returns>
        public List<DTO_ProveedoresTotal> ReportesProveedores_Recibidos(DateTime Periodo, string proveedor, bool isDetallado, int? numDoc, Dictionary<int, string> filtros, bool isFacturdo)
        {
            try
            {
                ...
                recibi.DetallesRecibidos = this._dal_ReportesProveedores.DAL_ReportesProveedores_Recibidos(Periodo, proveedor, isDetallado, numDoc, filtros,isFacturdo );
        ...
        {
            this._dal_ReportesProveedores = (DAL_ReportesProveedores)this.GetInstance(typeof(DAL_ReportesProveedores), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
            return this._dal_ReportesProveedores.DAL_ReportesProveedores_ProcedimientoComprasXLS(filtros,fechaIni,fechaFinal);
        }
        #endregion
