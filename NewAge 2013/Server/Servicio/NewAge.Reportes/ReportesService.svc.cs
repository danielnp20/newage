...
namespace NewAge.Server.ReportesService
{
    ...
    public class ReportesService : IReportesService
    {
        ...
        /// <summary>
        /// Genera el reporte de la relación de 
        /// de un recauso masivo
        /// </summary>
        ...
        /// <param name="data">Datos a migrar</param>
        /// <returns>Retorna el nombre del reporte</returns>
        public string Report_Cc_CobroJuridico(Guid channel, int documentID, byte claseDeuda, byte tipoReporte, string cliente, string obligacion, byte tipoEstado)
        {
            int opIndex = -1;
            try
            {
                ...
                {
                    Report_Cc_EstadoClientes reporte = new Report_Cc_EstadoClientes(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    return reporte.GenerateReport(cliente, obligacion,tipoEstado);
                }
        ...
        /// <returns>Retorna el nombre del reporte</returns>
        public string Report_Cc_CobroJuridicoHistoria(Guid channel, int documentID, int numDocCredito,byte claseDeuda)
        {
            int opIndex = -1;
            try
            {
                ...
                return reporte.GenerateReport(numDocCredito, claseDeuda);
            }
            finally
            {
                _currentProcess.Remove(documentID);
                ...
                string zona, string ciudad, string concesionario, string asesor, string lineaCred, string compCart, string pagaduria, string centroPago, byte? agrup, byte? romp, object filter, int? numeroDoc)
        {
            ...
            try
            {
                ...
                else if (documentoID == AppReports.ccEstadoCesionCartera)
                {
                    ...
                    else if (tipoReporte == 5) //Detalle Saldo
                    {
                        Report_Cc_DetalleSaldo reporte = new Report_Cc_DetalleSaldo(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(fechaIni, compCart, cliente, libranza, agrup);
                    }
                    else if (tipoReporte == 6) //Resumen Saldo
                    {
                        Report_Cc_ResumenSaldo reporte = new Report_Cc_ResumenSaldo(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(fechaIni, compCart, cliente, libranza, agrup);
                    }
                }
                ...
                else if (documentoID == AppReports.ccCarteraEspeciales) // Especiales
                {
                    ...
                    else if (tipoReporte == 6) //Creditos nuevos
                    {
                        Report_Cc_LibranzaNuevas reporte = new Report_Cc_LibranzaNuevas(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(fechaIni, fechaFin, cliente);
                    }
                    else if (tipoReporte == 7) //Polizas para Renovar
                    {
                        Report_Cc_PolizasRenovar reporte = new Report_Cc_PolizasRenovar(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(fechaIni, fechaFin, cliente);
                    }
                }

                else if (documentoID == AppReports.ccActivacionReoperacion) // Activacion,reoperacion
                {
                    if (tipoReporte == 1) //Reoperaciones
                    {
                        Report_cc_Reoperaciones reporte = new Report_cc_Reoperaciones(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(fechaIni, fechaFin, cliente);
                    }
                    else if (tipoReporte == 2) //Activaciones
                    {
                        Report_cc_Activaciones reporte = new Report_cc_Activaciones(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(fechaIni, fechaFin, cliente);
                    }
                    else if (tipoReporte == 3) //Reestructuracion con extra
                    {
                        Report_cc_Reestructuracion reporte = new Report_cc_Reestructuracion(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(fechaIni, fechaFin, cliente);
                    }
                }
                else if (documentoID == AppReports.ccReportePagaduria)
                {
                    if (tipoReporte == 1) //Nomina
                    {
                        Report_cc_PagaduriaNomina reporte = new Report_cc_PagaduriaNomina(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(fechaIni, fechaFin, pagaduria);
                    }
                    else //Novedades
                    {
                    }
                ...
                {
                    if (tipoReporte == 2) //Extractos
                    {
                        Report_cc_ExtractoCliente reporte = new Report_cc_ExtractoCliente(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        result = reporte.GenerateReport(libranza,cliente);
                    }
                    else //
                    {
                    }
                ...
                else if (documentoID == AppQueries.ConsultaSolicitud) // anexos Solicitudes-- Libranza(Contrato de mutuo Acuerdo ) Autorizacion de recaudo
                {
                    ...
                    if (tipoReporte == 1) //Contrato mutuo
                    {
                        #region Contrato Mutuo
                        Report_Cc_ContratoMutuo reporte = null;
                        byte[] arr = module.aplReporte_GetByID(AppReports.ccContratoMutuo);
                        if (arr != null)
                        {
                            XtraReport customReport = new Report_Cc_ContratoMutuo();
                            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream(arr))
                                customReport.LoadLayout(memoryStream);

                            reporte = (Report_Cc_ContratoMutuo)customReport;
                            ...
                            reporte = new Report_Cc_ContratoMutuo(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);

                        return reporte.GenerateReport(Convert.ToString(libranza));
                        #endregion
                    }
                    else if (tipoReporte == 2) //Aut Descuento
                    {
                       
                        #region Recaudo
                        Report_Cc_AutorizacionDescuento reporte = null;
                        byte[] arr = module.aplReporte_GetByID(AppReports.ccAutDescuento);
                        if (arr != null)
                        {
                            XtraReport customReport = new Report_Cc_AutorizacionDescuento();
                            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream(arr))
                                customReport.LoadLayout(memoryStream);

                            reporte = (Report_Cc_AutorizacionDescuento)customReport;
                            reporte.InitUserReport(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        }
                        else
                            reporte = new Report_Cc_AutorizacionDescuento(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
        ...
        /// </summary>
        /// <param name="documentID">DocumentoID</param>
        /// <param name="tipoReporte">Tipo de reporte</param>Report_Cc_GestionCobranzaDia
        /// <param name="fechaCorte">fecha periodo</param>
        /// <returns>nombre de reporte</returns>
        public string Report_Cc_GestionCobranza(Guid channel, int document, string tipoReport, DateTime fechaCorte, string etapa,bool excluyeDemanda)
        {
            int opIndex = -1;
            try
            {
                ...
                {
                    Report_Cc_EstadisticoGestResumido reporte = new Report_Cc_EstadisticoGestResumido(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    return reporte.GenerateReport(fechaCorte, etapa);
                }
                else if (tipoReport == "5")
                {
                    Report_Cc_EstadisticoGestDetallado reporte = new Report_Cc_EstadisticoGestDetallado(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    return reporte.GenerateReport(fechaCorte,etapa,excluyeDemanda);
                }                
                else
                {
                    return string.Empty;
                                                     ...
                                                     string CuentaFinal, string libro, string tipoReport, string Moneda, int _fechaIni, int _fechaFin, byte? Combo1, byte? Combo2, string proyecto, string centroCto)
        {
            ...
            try
            {
                ...
                else if(tipoReport.Equals("EstResultados"))
                {
                    ...
                    name = reporte.GenerateReport(Periodo, LongitudCuenta, SaldoIncial, CuentaInicial, CuentaFinal, libro, tipoReport, Moneda, _fechaIni, _fechaFin, Combo1, Combo2,proyecto,centroCto);

                }
                else if (tipoReport.Equals("EstResultadosMes"))
                {
                    Report_Co_BalanceEstadoResultadosMes reporte = new Report_Co_BalanceEstadoResultadosMes(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    name = reporte.GenerateReport(Periodo, LongitudCuenta, SaldoIncial, CuentaInicial, CuentaFinal, libro, tipoReport, Moneda, _fechaIni, _fechaFin, Combo1, Combo2, proyecto, centroCto);
        ...
        /// <summary>
        /// Obtiene un datatable con la info de CxC segun filtros
        /// </summary>
        /// <param name="tipoReporte">Tipo de Reporte a Generar</param>
        /// <param name="fechaIni">Fecha Inicial</param>
        /// <param name="fechaFin">Fecha Final</param>
        ...
        public DataTable Reportes_CC_CxCToExcel(Guid channel,int documentoID, byte? tipoReporte, DateTime fechaIni, DateTime? fechaFin, string tercero, bool isDetallada)
        {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();
                ModuloFacturacion module = (ModuloFacturacion)facade.GetModule(ModulesPrefix.fa, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, this._connLoggerString);
                return module.Reportes_CC_CxCToExcel(documentoID, tipoReporte, fechaIni, fechaFin, tercero, isDetallada);
            }
            finally
            {
                this.ADO_CloseDBConnection(opIndex);
        ...
        /// <returns>URL del Reporte</returns>
        public string ReportesFacturacion_FacturaVenta(Guid channel, int documentID, string numDoc, bool isAprobada, ExportFormatType formatType, decimal valorAnticipo, decimal valorRteGarantia, decimal? porcRteGarantia, bool printAIUind)
        {
            int opIndex = -1;
            try
            {
                ...
                byte[] arr = null;
                if (documentID == AppReports.faCuentaCobro)
                    arr = module.aplReporte_GetByID(documentID);
                else
                    arr = module.aplReporte_GetByID(!printAIUind ? documentID : AppReports.faFacturasVentaAIU);
...
//                      public string GenerateReport(string numDoc, bool isAprobada, decimal valorAnticipo, decimal valorRteGarantia, decimal? porcRteGarantia)
        ...
        /// <summary>
        /// Funcion que retorna el nombre de los reporte de Saldos 
        /// </summary>
        /// <param name="año">Año que se desea ver</param>
        ...
        public string ReportesInventarios_Saldos(Guid channel, int año, int mesIni, string bodega, string tipoBodega,
                    ...
                    string rompimiento, bool _parametro, string _tipoReporte, ExportFormatType formatType)
         {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();
                 ...
                 switch (rompimiento)// Rompimiento Bodega o Referencia oscar
                {
                    case "1":
                        {
                            if (_parametro == true)
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    Report_In_SaldosResumido reporte = new Report_In_SaldosResumido(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, formatType);
                                    return reporte.GenerateReport(año, mesIni, bodega, tipoBodega, referencia, grupo, clase, tipo, serie, material, isSerial, Libro);
                     ...
                     case "2":
                         {
                             ...
                             else
                             {
                                 ...
                                 {
                                     Report_In_SaldosxReferenciaResumido reporte = new Report_In_SaldosxReferenciaResumido(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, formatType);
                                     return reporte.GenerateReport(año, mesIni, bodega, tipoBodega, referencia, grupo, clase, tipo, serie, material, isSerial, Libro);
                                 }
                             }
                         }
                   ...
                   string referencia, string grupo, string clase, string tipo, string serie, string material, bool isSerial, string tipoReporte, ExportFormatType formatType)
        {
            ...
            try
            {
                ...
                return reporte.GenerateReport(año, mesIni, bodega, tipoBodega, referencia, grupo, clase, tipo, serie, material, isSerial,tipoReporte);
            }
            finally
            {
                this.ADO_CloseDBConnection(opIndex);
                                                        ...
                                                        string serie, string material, bool isSerial, string libro, string proyectoID, string mvtoTipoID, object otroFilter, byte? agrup, byte? romp)
        {
            ...
            try
            {
                ...
                return module.Reportes_In_InventarioToExcel(documentID,mesIni,mesFin,bodega,tipoBodega,referencia,grupo,clase,tipo, serie, material, isSerial,libro,proyectoID,mvtoTipoID,otroFilter,  agrup, romp);
            }
            finally
            {
                this.ADO_CloseDBConnection(opIndex);
        ...
        public DataTable Reportes_In_DocumentoToExcel(Guid channel, string movimientoID, string bodegaID, string proyectoID, string tipoReporte, DateTime fechaIni, byte detalle)        
        {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();
                ModuloInventarios module = (ModuloInventarios)facade.GetModule(ModulesPrefix.@in, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, this._connLoggerString);
                return module.Reportes_In_DocumentoToExcel(movimientoID, bodegaID, proyectoID, tipoReporte, fechaIni, detalle);
            }
            finally
            {
                this.ADO_CloseDBConnection(opIndex);
        ...
        public string ReporteInvFisico(Guid channel, int documentID,string bodegaID,DateTime periodoID,List<DTO_inFisicoInventario> _listFisicoInventario, InventarioFisicoReportType tipoReporte)
        {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();
                if (tipoReporte != InventarioFisicoReportType.Diferencia)
                {
                    Report_In_InventarioFisico reporte = new Report_In_InventarioFisico(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    return reporte.GenerateReport(documentID, bodegaID, periodoID, _listFisicoInventario, tipoReporte);
                }
                else
                {
                    Report_In_InventarioFisicoDiferencia reporte = new Report_In_InventarioFisicoDiferencia(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    return reporte.GenerateReport(documentID, bodegaID, periodoID, _listFisicoInventario);
                }
            }
            finally
            {
        ...
        ///<returns>Resultado</returns>
        public string Report_RelacionPrestamos(Guid channel, string proyecto,string inReferenciaID)
        {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();               
                Report_py_RelacionPrestamos reporte = new Report_py_RelacionPrestamos(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                return reporte.GenerateReport(proyecto, inReferenciaID);
            }
            finally
            {
                this.ADO_CloseDBConnection(opIndex);
        ...
        ///<param name="inReferenciaID">Lista de items a mostrar</param>
        ///<returns>Resultado</returns>
        public string Report_Movimientos(Guid channel, string movimientoID,string bodegaID,string proyectoID,string tipoReporte, DateTime fechaIni, byte resumido)
        {
            int opIndex = -1;
            try
            {
                ...
                {
                    case 1:
                        Report_in_Movimientos reporte = new Report_in_Movimientos(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        reportName=reporte.GenerateReport(movimientoID, bodegaID, proyectoID, tipoReporte, fechaIni);
                    break;
                    case 2:
                        Report_in_MovimientosDetallado reportedeta = new Report_in_MovimientosDetallado(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        reportName=reportedeta.GenerateReport(movimientoID, bodegaID, proyectoID, tipoReporte, fechaIni);
                    break;
                    case 3:
                        Report_in_MovimientosConsolidado reportecon = new Report_in_MovimientosConsolidado(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        reportName = reportecon.GenerateReport(movimientoID, bodegaID, proyectoID, tipoReporte, fechaIni);
                    break;
                }
                return reportName;
        ...
        /// <returns>Listado de DTO</returns>
        public string Report_ProveedoresTrazabilizad(Guid channel, DateTime fechaIni, DateTime? fechaFin, bool resumidoInd, bool pendientesInd, string proveedorID, string proyectoID)
        {
            int opIndex = -1;
            try
            {
                ...
                if(resumidoInd)
                {
                    Report_ProveedoresTrazabilizadResum reporte = new Report_ProveedoresTrazabilizadResum(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    return reporte.GenerateReport(fechaIni, fechaFin, resumidoInd, pendientesInd, proveedorID, proyectoID);
                }
                else
                {
                    Report_ProveedoresTrazabilizad reporte = new Report_ProveedoresTrazabilizad(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    return reporte.GenerateReport(fechaIni, fechaFin, resumidoInd, pendientesInd, proveedorID, proyectoID);
                }
            }
            finally
        ...
        /// <param name="formatType">Tipo de formato para exportar el reporte</param>
        /// <returns>Listado de DTO</returns>
        public DTO_TxResult ReportesProveedores_OrdenCompras(Guid channel, DateTime FechaIni, DateTime FechaFin, string Proveedor, Dictionary<int, string> filtros, bool isDetallado, string Moneda, ExportFormatType formatType)
        {
            int opIndex = -1;
            try
            {
                ...
                return reporte.GenerateReport(FechaIni, FechaFin, Proveedor, filtros, isDetallado, Moneda);
            }
            finally
            {
                this.ADO_CloseDBConnection(opIndex);
        ...
        /// <param name="formatType">Tipo de formato para exportar el reporte</param>
        /// <returns>Listado de DTO</returns>
        public DTO_TxResult ReportesProveedores_OrdenComprasDetallada(Guid channel, DateTime FechaIni, DateTime FechaFin, string Proveedor, Dictionary<int, string> filtros, bool isDetallado, string Moneda, ExportFormatType formatType)
        {
            int opIndex = -1;
            try
            {
        ...
        /// <returns>Tabla con la ejecucion presupuestal</returns>
        [OperationContract]
        public DataTable ReportesProveedores_ProcedimientoComprasXLS(Guid channel, Dictionary<int, string> filtros, DateTime fechaIni, DateTime fechaFinal)        
        {
            int opIndex = -1;
            try
            {
                ...
                var response = modulo.ReportesProveedores_ProcedimientoComprasXLS(filtros,fechaIni, fechaFinal);
                return response;
            }
            finally
            {
        ...
        public string ReportesProveedores_OrdenCompra(Guid channel, int numDoc,byte tipoReporte,bool showReport, bool isPreliminar = false)
        {
            ...
            try
            {
                ...
                int documentID = 0;
                if (tipoReporte == 1)
                    documentID = AppDocuments.OrdenCompra;
                else if(tipoReporte == 2)
                    documentID = AppReports.prOrdenCompraAnexo;
                else if (tipoReporte == 3)
                    documentID = AppReports.prOrdenCompraServicio;
                ...
                return reporte.GenerateReport(numDoc, tipoReporte,showReport);
            }
            finally
            {
                this.ADO_CloseDBConnection(opIndex);
        ...
        /// <returns></returns>
        public DTO_TxResult ReportesProveedores_Recibidos(Guid channel, DateTime Periodo, string proveedor, bool isDetallado, Dictionary<int, string> filtros, bool isFacturdo, ExportFormatType exportType)
        {
            int opIndex = -1;
            try
            {
                ...
                return reporte.GenerateReport(Periodo, proveedor, isDetallado,filtros, isFacturdo);
        ...
        /// <param name="exportType">Formato de Ezportacion del reporte</param>
        /// <returns></returns>
        public DTO_TxResult ReportesProveedores_RecibidosDetallado(Guid channel, DateTime Periodo, string proveedor, bool isDetallado, Dictionary<int, string> filtros, bool isFacturdo, ExportFormatType exportType)
        {
            int opIndex = -1;
            try
            {
        ...
        public DTO_TxResult ReportesProveedores_RecibidosNoFacturados(Guid channel, DateTime Periodo, string proveedor, bool isDetallado, Dictionary<int, string> filtros, bool isFacturdo, ExportFormatType exportType)
        {
            int opIndex = -1;
            try
            {
        ...
        public string Reportes_py_PlaneacionCostos(Guid channel, DTO_SolicitudTrabajo solicitud,bool useMultiplicadorInd, byte tipoReport, DateTime? mesIni, DateTime? mesFin)
        {
            ...
            try
            {
                ...
                if (solicitud == null)
                {
                    if (tipoReport == 1)
                    {
                        Report_py_ResumenDocsCotizacion reporte = new Report_py_ResumenDocsCotizacion(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        reportName = reporte.GenerateReport(mesIni.Value, mesFin.Value);
                    }
                    else if (tipoReport == 2)
                    {
                        ...
                        reportName = reporte.GenerateReport( mesIni, mesFin);
                    }
                    else if (tipoReport == 3)
                    {
                        Report_py_PlanCostosResumido reporte = new Report_py_PlanCostosResumido(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                ...
                else
                {
                    ...
                    else if (tipoReport == 10)
                    {
                        Report_py_PlaneacionTiempos reporte = new Report_py_PlaneacionTiempos(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                        reportName = reporte.GenerateReport(solicitud);
                    }
        ...
        /// <param name="periodo">periodo</param>
        /// <returns>nombre del reporte</returns>
        public string Reportes_py_Indicadores(Guid channel,  DateTime? periodo)
        {
            int opIndex = -1;
            string reportName = string.Empty;
            try
            {
                ...
                Report_py_Indicadores reporte = new Report_py_Indicadores(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                reportName = reporte.GenerateReport(periodo.Value);                
                return reportName;
            }
            finally
            {
        ...
        /// <returns>nombre del reporte</returns>
        public string Reportes_py_ActaTrabajo(Guid channel, DTO_SolicitudTrabajo solicitud, int actaNroActual, DateTime fechaActa)
        {
            int opIndex = -1;
            string reportName = string.Empty;
            try
            {
                ...
                reportName = reporte.GenerateReport(solicitud, actaNroActual,fechaActa);
        ...
        /// <param name="actaNroActual">Acta actual</param>
        /// <returns>nombre del reporte</returns>
        public string Reportes_py_ActaEntrega(Guid channel, DTO_SolicitudTrabajo solicitud, int actaNroActual, DateTime fechaActa)
        {
            int opIndex = -1;
            string reportName = string.Empty;
            try
            {
                ...
                ModuloAplicacion module = (ModuloAplicacion)facade.GetModule(ModulesPrefix.apl, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, this._connLoggerString);
                Report_py_ActaEntrega reporte = null;
                byte[] arr = module.aplReporte_GetByID(AppReports.pyActaEntrega);
                if (arr != null)
                {
                    XtraReport customReport = new Report_py_ActaEntrega();
                    using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream(arr))
                        customReport.LoadLayout(memoryStream);

                    reporte = (Report_py_ActaEntrega)customReport;
                    reporte.InitUserReport(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf, null);
                }
                else
                    reporte = new Report_py_ActaEntrega(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
        ...
        /// <returns>nombre del reporte</returns>
        public string Reportes_pyResumenDocsCotiz(Guid channel, DateTime mesIni, DateTime mesFin)
        {
            int opIndex = -1;
            string reportName = string.Empty;
            try
        ...
        public string Report_Ts_Ejecutado(Guid channel, DateTime fechaIni, string tipoReporte)
        {
            string nomreporte = string.Empty;
            int opIndex = -1;
            try
            {
                if (tipoReporte=="1")
                { 
                    Reports_Ts_Ejecutado reporte = new Reports_Ts_Ejecutado();
                    opIndex = this.ADO_ConnectDB();
                    reporte = new Reports_Ts_Ejecutado(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    nomreporte= reporte.GenerateReport(fechaIni); 
                }
                else
                {
                    Report_Ts_Ejecutado_Resumido reporteresu = new Report_Ts_Ejecutado_Resumido();
                    opIndex = this.ADO_ConnectDB();
                    reporteresu = new Report_Ts_Ejecutado_Resumido(this._connLoggerString, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, ExportFormatType.pdf);
                    nomreporte=reporteresu.GenerateReport(fechaIni);    
                }
                return nomreporte;
            }
            finally
