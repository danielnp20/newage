            ...
            foreach (T obj in this.Objects)
            {
                foreach (string col in importableCols)
                {
                    try
                    {
                        ...
                        //Valida si existen comas si no es numerico
                        //if (!pi.PropertyType.Name.Equals("UDT_Valor") && !pi.PropertyType.Name.Equals("UDT_Cantidad") && !pi.PropertyType.Name.Equals("UDT_Porcentaje"))
                            valClean = valClean.Replace(';', ',');
                        //Valida si es un dato numerico para dar formato
                        //if (Convert.ToDecimal(valClean) == 0)
                        //    valClean = "0";

                        //Valida si es un booleano para  dar formato
                        valClean = valClean.Equals("True") ? "X" : valClean;
                        valClean = valClean.Equals("False") ? " " : valClean;

                        sb.Append(valClean).Append(";");
                    }
                    catch (Exception)
                    {
