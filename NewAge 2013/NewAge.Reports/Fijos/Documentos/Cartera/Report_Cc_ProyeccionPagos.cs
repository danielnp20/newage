...
{
    public partial class Report_Cc_ProyeccionPagos : ReportBaseLandScape
    {
        #region Variables
        #endregion
        public Report_Cc_ProyeccionPagos()
        {
            
        }

        public Report_Cc_ProyeccionPagos(string loggerConn, SqlConnection c, SqlTransaction tx, DTO_glEmpresa empresa, int userId, ExportFormatType formatType)
            : base(loggerConn, c, tx, empresa, userId, formatType)
        {
            this.lblReportName.Text = this._moduloGlobal.GetResource(LanguageTypes.Forms, "35104_ProyeccionPagos");          
        }
        ...
        public string GenerateReport(byte? tipoReporte, DateTime? fechaIni, DateTime fechaFin, string cliente, int? libranza, string zonaID, string ciudad, string concesionario, string asesor, string lineaCredi, string compCartera)
        {
            try
            {
                ...
                filter = this._moduloGlobal.GetResource(LanguageTypes.Forms, +AppReports.ReportBase + "_FechaFin") + " " + (DateTime.Today).ToShortDateString() + "\t   ";
