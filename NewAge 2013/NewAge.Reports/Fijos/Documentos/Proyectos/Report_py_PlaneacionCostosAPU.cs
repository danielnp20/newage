    ...
    public partial class Report_py_PlaneacionCostosAPU : ReportBase
    {
        ...
        public string GenerateReport(byte tipoReport, DTO_SolicitudTrabajo solicitud,bool useMultiplicadorInd)
        {
            ...
            try
            {
                ...
                decimal porIVA = solicitud.Header.PorIVA.Value.HasValue ? solicitud.Header.PorIVA.Value.Value/100 : 0;
                porIVA = tipoReport == 4 ? 0 : porIVA;//si es APU del Costo no suma el %del iva
                ...
                #endregion
                if (tipoReport == 5 && solicitud.Header.PorMultiplicadorPresup.Value != 100) //Valores del Cliente
                {
                    solicitud.Header.PorClienteADM.Value = solicitud.Header.PorClienteADM.Value ?? 0;
                    solicitud.Header.PorEmpresaADM.Value = solicitud.Header.PorEmpresaADM.Value ?? 0;
                    solicitud.Header.PorClienteIMP.Value = solicitud.Header.PorClienteIMP.Value ?? 0;
                    solicitud.Header.PorEmpresaIMP.Value = solicitud.Header.PorEmpresaIMP.Value ?? 0;
                    solicitud.Header.PorClienteUTI.Value = solicitud.Header.PorClienteUTI.Value ?? 0;
                    solicitud.Header.PorEmpresaUTI.Value = solicitud.Header.PorEmpresaUTI.Value ?? 0;
                    ...
                    foreach (DTO_pyPreProyectoTarea tarea in solicitud.Detalle)
                    {
                        ...
                        foreach (var det in tarea.Detalle)
                        {
                            ...
                            det.VlrIVAUtil.Value = det.VlrIVAUtil.Value ?? 0;

                            if (solicitud.Header.APUIncluyeAIUInd.Value.Value && det.TipoRecurso.Value != (byte)TipoRecurso.Personal)
                            {
                            ...
                            {
                                //Quita el valor del AIU si no se incluye
                                det.VlrAIUAdmin.Value = 0;
                                det.VlrAIUImpr.Value = 0;
                                det.VlrAIUUtil.Value = 0;
                                det.VlrIVAUtil.Value = 0;
                            }
                        ...
                        tarea.VlrAIUxAPUAdmin.Value = tarea.Detalle.Count> 0?  (tarea.Detalle.Sum(x=>x.CostoLocalTOT.Value) * (solicitud.Header.PorEmpresaADM.Value / 100)) : 0;
                        tarea.VlrAIUxAPUImpr.Value = tarea.Detalle.Count > 0 ?  (tarea.Detalle.Sum(x => x.CostoLocalTOT.Value) * (solicitud.Header.PorEmpresaIMP.Value / 100)) : 0;
                        tarea.VlrAIUxAPUUtil.Value = tarea.Detalle.Count > 0 ? (tarea.Detalle.Sum(x => x.CostoLocalTOT.Value) * (solicitud.Header.PorEmpresaUTI.Value / 100)) : 0;
                        tarea.VlrAIUxAPUIVA.Value = tarea.Detalle.Count > 0 ? tarea.VlrAIUxAPUUtil.Value * porIVA : 0;

                        foreach (var det in tarea.Detalle)
                        {
                            det.VlrAIUAdmin.Value = tarea.VlrAIUxAPUAdmin.Value;
                            det.VlrAIUImpr.Value = tarea.VlrAIUxAPUImpr.Value;
                            det.VlrAIUUtil.Value = tarea.VlrAIUxAPUUtil.Value;
                            det.VlrIVAUtil.Value = tarea.VlrAIUxAPUIVA.Value; 
                        }
                        ...
                        if (solicitud.Header.TipoRedondeo.Value == 2)//Redonde hacia arriba
                        {
                            ...
                            tarea.CostoTotalUnitML.Value += Math.Ceiling(tarea.VlrAIUxAPUAdmin.Value.Value + tarea.VlrAIUxAPUImpr.Value.Value + tarea.VlrAIUxAPUUtil.Value.Value + tarea.VlrAIUxAPUIVA.Value.Value);
                        ...
                        else if (solicitud.Header.TipoRedondeo.Value == 3) //Redondea hacia abajo
                        {
                            ...
                            tarea.CostoTotalUnitML.Value += Math.Floor(tarea.VlrAIUxAPUAdmin.Value.Value + tarea.VlrAIUxAPUImpr.Value.Value + tarea.VlrAIUxAPUUtil.Value.Value + tarea.VlrAIUxAPUIVA.Value.Value);
                        }
                        else //Redondea Normal
                        {
                            tarea.CostoTotalUnitML.Value = Math.Round(tarea.Detalle.Sum(x => x.CostoLocalTOT.Value.Value), 0);
                            tarea.CostoTotalUnitML.Value += Math.Round(tarea.VlrAIUxAPUAdmin.Value.Value + tarea.VlrAIUxAPUImpr.Value.Value + tarea.VlrAIUxAPUUtil.Value.Value + tarea.VlrAIUxAPUIVA.Value.Value, 2);
                        }
                ...
                else
                {
                    foreach (DTO_pyPreProyectoTarea tarea in solicitud.Detalle)
                    {
                        ...
                        //tarea.VlrAIUxAPUIVA.Value = tarea.VlrAIUxAPUIVA.Value ?? 0;

                        tarea.VlrAIUxAPUAdmin.Value =0;
                        tarea.VlrAIUxAPUImpr.Value = 0;
                        tarea.VlrAIUxAPUUtil.Value = 0;
                        tarea.VlrAIUxAPUIVA.Value = 0;

                        tarea.VlrAIUxAPUAdmin.Value = tarea.Detalle.Count > 0 ? tarea.Detalle.First().VlrAIUAdmin.Value : 0;
                        tarea.VlrAIUxAPUImpr.Value = tarea.Detalle.Count > 0 ? tarea.Detalle.First().VlrAIUImpr.Value : 0;
                        tarea.VlrAIUxAPUUtil.Value = tarea.Detalle.Count > 0 ? tarea.Detalle.First().VlrAIUUtil.Value : 0;
                        tarea.VlrAIUxAPUIVA.Value = tarea.Detalle.Count > 0 ? tarea.Detalle.First().VlrIVAUtil.Value : 0;
                        if (solicitud.Header.TipoRedondeo.Value == 2)//Redonde hacia arriba
                        {
                            tarea.CostoTotalUnitML.Value = Math.Ceiling(tarea.Detalle.Sum(x => x.CostoLocalTOT.Value.Value));
                            tarea.CostoTotalUnitML.Value += Math.Ceiling(tarea.VlrAIUxAPUAdmin.Value.Value + tarea.VlrAIUxAPUImpr.Value.Value + tarea.VlrAIUxAPUUtil.Value.Value + tarea.VlrAIUxAPUIVA.Value.Value);
