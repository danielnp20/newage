...
{
    public partial class Report_Cc_ContratoMutuo : XtraReport
    {
        ...
        protected int libranzaID;
        protected DTO_SolicitudLibranza docu;
        ...
        public Report_Cc_ContratoMutuo()
        {
        }
        public Report_Cc_ContratoMutuo(string loggerConn, SqlConnection conn, SqlTransaction tx, DTO_glEmpresa emp, int userID, ExportFormatType formatType, int? numDoc = null)
        {
            InitializeComponent();
        ...
        public Report_Cc_ContratoMutuo(DTO_glEmpresa emp)
        {
        ...
        public string GenerateReport(string libranza)
        {
            ...
            this.libranzaID = Convert.ToInt32(libranza);
            this.docu = this._moduloCartera.SolicitudLibranza_GetByLibranza(this.libranzaID, string.Empty);

            #region Trae la ciudad de Exp
            List<DTO_ccSolicitudDatosPersonales> datos = this._moduloCartera.RegistroSolicitud_GetByLibranza(this.libranzaID);
            foreach (DTO_ccSolicitudDatosPersonales d in datos)
            {
                ...
                if (d.TipoPersona.Value == 1 && ciudad != null)
                {
                    ...
                    this.label3.Text = ciudad.Descriptivo.Value;
                }
                else if (d.TipoPersona.Value == 2 && ciudad != null)
                {
                    this.label6.Text = ciudad.Descriptivo.Value;
                    this.label8.Text = ciudad.Descriptivo.Value;
                }
            }
        ...
        private void lblValorLetra_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(VlrCuota.Text))
            {
                ...
                this.lblValorLetra.Text = CurrencyFormater.GetCurrencyString("ES", "cop", ValorCuotas);
            }
        }
        ...
        private void lblPLazoLetra2_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRLabel Plazo = FindControl("lblPlazo", true) as XRLabel;
            if (!string.IsNullOrEmpty(Plazo.Text))
            {
                Decimal NumeroCuotas = Convert.ToDecimal(Plazo.Text);
                this.lblPLazoLetra2.Text = CurrencyFormater.GetCurrencyString("ES", "num", NumeroCuotas);
            }
        }
        private void lblPeriodo_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRLabel Plazo = FindControl("lblRadicaPeriodo", true) as XRLabel;
            if (!string.IsNullOrEmpty(Plazo.Text))
            {
                Decimal periodo = Convert.ToDecimal(Plazo.Text) - 2000;
                this.lblPeriodo.Text = CurrencyFormater.GetCurrencyString("ES", "num", periodo);
            }
        }

        private void lblPrestamoTexto_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRLabel VlrPrestamo = FindControl("lblVlrPrestamo", true) as XRLabel;

            if (docu != null && docu.Header.VlrLibranza.Value.HasValue)
            {
                //Decimal Prestamo = Convert.ToDecimal(VlrPrestamo.Text);
                this.lblVlrPrestamo.Text = docu.Header.VlrLibranza.Value.Value.ToString("n0");
                this.lblPrestamoLetra.Text = CurrencyFormater.GetCurrencyString("ES", "cop", docu.Header.VlrLibranza.Value.Value);
            }
        }

        private void lblDiaRadica_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRLabel DiaRad = FindControl("lblRadicaDia", true) as XRLabel;
            if (!string.IsNullOrEmpty(DiaRad.Text))
            {
                Decimal dia = Convert.ToDecimal(DiaRad.Text);
                this.lblDiaRadica.Text = CurrencyFormater.GetCurrencyString("ES", "num", dia);
            }
        }
        #endregion

        private void lblPlazoNumero2_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRLabel Plazo = FindControl("lblPlazo", true) as XRLabel;
            this.lblPlazoNumero2.Text = Plazo.Text;
        }
    }
}
