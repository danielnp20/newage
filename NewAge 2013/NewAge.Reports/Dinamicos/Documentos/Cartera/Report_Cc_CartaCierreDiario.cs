...
{
    public partial class Report_Cc_CartaCierreDiario : XtraReport
    {
        ...
        List<DTO_ccHistoricoGestionCobranza> source = new List<DTO_ccHistoricoGestionCobranza>();
        #endregion
        ...
        public Report_Cc_CartaCierreDiario()
        {
          //  this.InitializeComponent();
        }

        public Report_Cc_CartaCierreDiario(DTO_glEmpresa emp)
        {
            this.InitializeComponent();
            this._empresa = emp;
        }
        ...
        public Report_Cc_CartaCierreDiario(string loggerConn, SqlConnection conn, SqlTransaction tx, DTO_glEmpresa emp, int userID, ExportFormatType formatType, int? numDoc = null)
        {
            InitializeComponent();
        ...
        /// </summary>
        public string GenerateReport(int document, List<DTO_ccHistoricoGestionCobranza> dataHistorico)
        {
            try
            {
                this.source = new List<DTO_ccHistoricoGestionCobranza>();
                DTO_ccHistoricoGestionCobranza first = new DTO_ccHistoricoGestionCobranza();

                string filesPath = this._moduloGlobal.GetControlValue(AppControl.RutaFisicaArchivos);
                string docsPath = this._moduloGlobal.GetControlValue(AppControl.RutaPlantillas);

                foreach (DTO_ccHistoricoGestionCobranza hist in dataHistorico)
                {
                    string pathTemplate = filesPath + docsPath + hist.CobranzaGestionID.Value + ".rtf";
                    try { hist.PlantillaCarta.Value = System.IO.File.ReadAllText(pathTemplate); } catch (Exception) { hist.PlantillaCarta.Value = string.Empty; }                 
                    hist.PlantillaCarta.Value = hist.PlantillaCarta.Value.Replace("%FECHA%", DateTime.Now.Day + " de " + DateTime.Now.ToString("MMMM"));
                    hist.PlantillaCarta.Value = hist.PlantillaCarta.Value.Replace("%NOMBRES%", hist.Nombre.Value);
                    hist.PlantillaCarta.Value = hist.PlantillaCarta.Value.Replace("%CREDITO%", hist.Libranza.Value.ToString());
                    hist.PlantillaCarta.Value = hist.PlantillaCarta.Value.Replace("%VALORCUOTA%", hist.VlrCuota.Value.HasValue ? hist.VlrCuota.Value.Value.ToString("n0") : " ");
                    hist.PlantillaCarta.Value = hist.PlantillaCarta.Value.Replace("%FECHAVTO%", hist.FechaControl.Value.HasValue ? hist.FechaControl.Value.Value.Day + " de " + hist.FechaControl.Value.Value.ToString("MMMM") + " de " + hist.FechaControl.Value.Value.Year : string.Empty);
                    hist.PlantillaCarta.Value = hist.PlantillaCarta.Value.Replace("%DIRECCION%", hist.Direccion.Value);
                    hist.PlantillaCarta.Value = hist.PlantillaCarta.Value.Replace("%CIUDAD%", hist.Ciudad.Value);
                    hist.PlantillaCarta.Value = hist.PlantillaCarta.Value.Replace("%DATO1%", hist.Dato1.Value);
                }

                first.Detalle = dataHistorico;
                source.Add(first);
        ...
        private void txtRichFilter_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
