...
namespace NewAge.Reports.Dinamicos
{
    public partial class Report_py_PlaneacionCostosCompar : XtraReport
    {
        ...
        public string GenerateReport(DTO_SolicitudTrabajo sol)
        {
            ...
            if (this._claseProyecto != null && this._claseProyecto.TipoPresupuesto.Value == (byte)TipoPresupuestoProy.Construccion )//calcula el valor del multiplicador si es Proy Cliente
            {
                foreach (DTO_pyPreProyectoTarea tarea in this.solicitud.Detalle)
                {
                    ...
                    tarea.CostoDiferenciaML.Value = tarea.CostoLocalCLI.Value.Value - tarea.CostoTotalML.Value;
