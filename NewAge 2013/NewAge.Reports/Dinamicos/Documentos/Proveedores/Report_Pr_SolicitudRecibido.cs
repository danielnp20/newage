...
{
    public partial class Report_Pr_SolicitudRecibido : XtraReport
    {
        ...
        public Report_Pr_SolicitudRecibido()
        {
          //  this.InitializeComponent();
        }

        public Report_Pr_SolicitudRecibido(DTO_glEmpresa emp)
        {
            this.InitializeComponent();
            this._empresa = emp;
        }
        ...
        public Report_Pr_SolicitudRecibido(string loggerConn, SqlConnection conn, SqlTransaction tx, DTO_glEmpresa emp, int userID, ExportFormatType formatType, int? numDoc = null)
        {
            InitializeComponent();
        ...
        /// </summary>
        public string GenerateReport(int documentoID, int numeroDoc, bool isPreliminar, byte tipoReporte)
        {
            try
            {
                if (documentoID == AppDocuments.Solicitud)
                {
                    this.lblNroInventario.Visible = false;
                    this.txtNroInventario.Visible = false;
                    this.lblReportName.Text = "Solicitud de Proveedores";
                }
                else
                    this.lblReportName.Text = "Recibido de Proveedores";
                ...
                if (documentoID == AppDocuments.Solicitud)
                    data = _moduloProveedores.ReportesProveedores_Solicitudes(null,numeroDoc);
                else if (documentoID == AppDocuments.Recibido)
                    data = _moduloProveedores.ReportesProveedores_Recibidos(DateTime.Now, string.Empty, false, numeroDoc,null, false);
