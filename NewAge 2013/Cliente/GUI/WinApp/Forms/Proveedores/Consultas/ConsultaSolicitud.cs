...
{
    public partial class ConsultaSolicitud : FormWithToolbar
    {
        #region Variables
        ...
        public ConsultaSolicitud()
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "ConsultaOrdenCompra.cs", "ConsultaOrdenCompra: " + ex.Message));
            }
        }
        ...
        {
            this._documentID = AppQueries.QuerySolicitud;
        ...
        private void AddGridColsDocument()
        {
            ...
            //Area Funcional
            GridColumn areaFuncionalID = new GridColumn();
            areaFuncionalID.FieldName = this._unboundPrefix + "AreaFuncionalID";
            areaFuncionalID.Caption = this._bc.GetResource(LanguageTypes.Forms, this._documentID + "_AreaFuncionalID");
            areaFuncionalID.UnboundType = UnboundColumnType.String;
            areaFuncionalID.VisibleIndex = 3;
            areaFuncionalID.Width = 80;
            areaFuncionalID.Visible = true;
            areaFuncionalID.OptionsColumn.AllowEdit = false;
            this.gvDocument.Columns.Add(areaFuncionalID);
            //Cantidad Solicit total
            GridColumn cantidadSolTotal = new GridColumn();
            cantidadSolTotal.FieldName = this._unboundPrefix + "CantidadSol";
            cantidadSolTotal.Caption = this._bc.GetResource(LanguageTypes.Forms, this._documentID + "_CantidadSolTotal");
            cantidadSolTotal.UnboundType = UnboundColumnType.Decimal;
            cantidadSolTotal.AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Far;
            cantidadSolTotal.AppearanceCell.Options.UseTextOptions = true;
            cantidadSolTotal.VisibleIndex = 4;
            cantidadSolTotal.Width = 100;
            cantidadSolTotal.Visible = false;
            cantidadSolTotal.ColumnEdit = this.editCant;
            cantidadSolTotal.OptionsColumn.AllowEdit = false;
            this.gvDocument.Columns.Add(cantidadSolTotal);
            //Cantidad OC total
            ...
            cantidadOCTotal.VisibleIndex = 5;
            cantidadOCTotal.Width = 100;
            cantidadOCTotal.Visible = false;
            cantidadOCTotal.ColumnEdit = this.editCant;
            cantidadOCTotal.OptionsColumn.AllowEdit = false;
            this.gvDocument.Columns.Add(cantidadOCTotal);
            //Cantidad Pend OC total
            GridColumn cantidadPendOCTotal = new GridColumn();
            cantidadPendOCTotal.FieldName = this._unboundPrefix + "CantidadPendOC";
            cantidadPendOCTotal.Caption = this._bc.GetResource(LanguageTypes.Forms, this._documentID + "_CantidadPendOCTotal");
            cantidadPendOCTotal.UnboundType = UnboundColumnType.Decimal;
            cantidadPendOCTotal.VisibleIndex = 6;
            cantidadPendOCTotal.Width = 100;
            cantidadPendOCTotal.Visible = false;
            cantidadPendOCTotal.ColumnEdit = this.editCant;
            cantidadPendOCTotal.OptionsColumn.AllowEdit = false;
            this.gvDocument.Columns.Add(cantidadPendOCTotal);
            //Cantidad Rec Total
            ...
            cantidadRecTotal.VisibleIndex = 7;
            ...
            cantidadPendRecTotal.VisibleIndex = 8;
            ...
            cantidadSol.FieldName = this._unboundPrefix + "CantidadSol";
            cantidadSol.Caption = this._bc.GetResource(LanguageTypes.Forms, this._documentID + "_CantidadSol");
            ...
            cantidadSol.VisibleIndex = 5;
            cantidadSol.Width = 100;
            cantidadSol.Visible = true;
            cantidadSol.ColumnEdit = this.editCant;
            ...
            cantidadOC.VisibleIndex = 6;
            cantidadOC.Width = 100;
            ...
            CantidadRec.VisibleIndex = 7;
            CantidadRec.Width = 100;
        ...
        private void txtNro_Leave(object sender, EventArgs e)
        {
            if (this.masterPrefijo.ValidID && !string.IsNullOrEmpty(this.txtNro.Text) && this.txtNro.Text != "0")
            {
                ...
                docSelect.Add(doc);
                this._listDocuments = this._bc.AdministrationModel.ConsultaCompras_Get(AppDocuments.Solicitud, docSelect);
                this.LoadGridData();
            }
        }
        ...
        private void btnQueryDoc_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ModalQuerySolicitud getDocControl = new ModalQuerySolicitud(docs, true, false);
        ...
        private void LinkEditDocRelacion_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                ctrl = this._bc.AdministrationModel.glDocumentoControl_GetByID(this._listDocumentsRelacion[fila].OrdCompraDocuID.Value.Value);
                comprobante = !string.IsNullOrEmpty(ctrl.ComprobanteID.Value) ? this._bc.AdministrationModel.Comprobante_Get(true, false, ctrl.PeriodoDoc.Value.Value, ctrl.ComprobanteID.Value, ctrl.ComprobanteIDNro.Value.Value, null, null, null) : null;
        ...
        private void gvDetalle_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                //Obtiene Ordenes de Compra segun el identificador de la solicitud
                DTO_ConsultaComprasDet consultaDet = (DTO_ConsultaComprasDet)view.GetRow(view.FocusedRowHandle);
                List<DTO_prDetalleDocu> detalleOC = this._bc.AdministrationModel.prDetalleDocu_GetByDocument(AppDocuments.Solicitud, consultaDet.SolicitudDocuID.Value.Value, consultaDet.SolicitudDetaID.Value.Value);
                this._listDocumentsRelacion = new List<DTO_ConsultaDocRelacion>();
                if (detalleOC.Count > 0)
                {
                    foreach (var det in detalleOC)
                    {
                        if (!this._listDocumentsRelacion.Exists(x => x.OrdCompraDocuID.Value == det.OrdCompraDocuID.Value))
                        {
                            DTO_prOrdenCompra orden = this._bc.AdministrationModel.OrdenCompra_Load(AppDocuments.OrdenCompra, string.Empty, 0, det.OrdCompraDocuID.Value.Value);
                            if (orden != null)
                            {
                                //Llena el detalle de cada Orden de Compra
                                DTO_ConsultaDocRelacion consultaDoc = new DTO_ConsultaDocRelacion();
                                consultaDoc.OrdCompraDocuID.Value = orden.DocCtrl.NumeroDoc.Value;
                                consultaDoc.PrefDoc.Value = orden.DocCtrl.PrefDoc.Value;
                                consultaDoc.Fecha.Value = orden.DocCtrl.FechaDoc.Value;
                                consultaDoc.ProveedorID.Value = orden.HeaderOrdenCompra.ProveedorID.Value;
                                DTO_prProveedor proveedor = (DTO_prProveedor)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.prProveedor, false, orden.HeaderOrdenCompra.ProveedorID.Value, true);
                                consultaDoc.ProveedorNombre.Value = proveedor.Descriptivo.Value;
                                consultaDoc.Cantidad.Value = orden.Footer.Sum(x => x.DetalleDocu.CantidadOC.Value);
                                consultaDoc.Valor.Value = orden.DocCtrl.Valor.Value;
                                consultaDoc.MonedaID.Value = orden.DocCtrl.MonedaID.Value;
