...
{
    public partial class MigracionFactCxC : ProcessForm
    {
        #region Delegados
        ...
        protected override void InitForm()
        {
            try
            {
                this.documentID = AppProcess.MigracionFactCxC;              
                ...
                filtroMaster.Add(new DTO_glConsultaFiltro()
                {
                    ...
                    ValorFiltro = AppDocuments.FacturaVenta.ToString(),
                ...
                filtroMaster.Add(new DTO_glConsultaFiltro()
                {
                    CampoFisico = "DocumentoID",
                    OperadorFiltro = OperadorFiltro.Igual,
                    ValorFiltro = AppDocuments.NotaCredito.ToString()
                });
        ...
        private void InitVars()
        {
            ...
            string formatCxC = this._bc.GetImportExportFormat(typeof(DTO_faFacturaDocu), this.documentMigracionID);
            this.format = formatDoc + this.formatSeparator + formatCxC;
        ...
        private void ValidateDataImport(DTO_glDocumentoControl ctrl, DTO_faFacturaDocu cxc, DTO_TxResultDetail rd, string msgInvalidDate)
        {
            string colRsx;
            ...
            if (cxc.FechaVto.Value.Value < ctrl.FechaDoc.Value.Value)
            {
                string rsxField = this._bc.GetResource(LanguageTypes.Forms, this.documentMigracionID + "_FechaVto");
                rdF = new DTO_TxResultDetailFields();
                rdF.Field = rsxField;
            ...
            if (createDTO)
            {
                ...
                ctrl.Descripcion.Value = "CONTABILIZA MIGRACION DE SALDOS CXC";

                //CxC
                cxc.FechaPtoPago.Value = ctrl.Fecha.Value;
                cxc.MonedaPago.Value = ctrl.MonedaID.Value;
                //cxc.AsesorID.Value = string.Empty;
                //cxc.DocumentoREL.Value = 0;
                //cxc.ClienteID.Value = string.Empty;
                //cxc.ListaPrecioID.Value = string.Empty;
                //cxc.ZonaID.Value = string.Empty; ;
                //cxc.FacturaTipoID.Value = string.Empty;
                //cxc.TasaPago.Value = 0;
                //cxc.FormaPago.Value = string.Empty;
                //cxc.Valor.Value = 0;
                //cxc.Iva.Value = 0;
                ...
                if (this.isMultiMoneda)
                {
                    if (ctrl.MonedaID.Value == this.monedaLocal)
                        cxc.Valor.Value = cxc.Valor.Value;
                    else
                        cxc.Valor.Value = Math.Round((cxc.Valor.Value.Value * ctrl.TasaCambioCONT.Value.Value), 2);
                }
                else
                    cxc.Valor.Value = cxc.Valor.Value;

                this.valorTotal += cxc.Valor.Value.Value;
            }
            #endregion
        }
        ...
        private void ImportThread()
        {
            try
            {
                if (this.pasteRet.Success)
                {
                    ...
                    DTO_faFacturaDocu cxc = new DTO_faFacturaDocu();
                    bool createDTO = true;
                    bool validList = true;
                    //Inicializacion de variables generales
                    this._ctrlList = new List<DTO_glDocumentoControl>();
                    this._facturaDocuList = new List<DTO_faFacturaDocu>();
                    #endregion
                    ...
                    List<PropertyInfo> pisSupl = typeof(DTO_faFacturaDocu).GetProperties().ToList();
                    ...
                    if (validList)
                    {
                        ...
                        for (int index = 0; index < this._ctrlList.Count; ++index)
                        {
                            ...
                            cxc = this._facturaDocuList[index];
                            ...
                            this.ValidateDataImport(ctrl, cxc, rd, msgInvalidDate);
                            if (rd.DetailsFields.Count > 0)
                            {
                                result.Details.Add(rd);
                                rd.Message = "NOK";
        ...
        private void ProcesarThread()
        {
            try
            {
                ...
                DTO_TxResult result = this._bc.AdministrationModel.CuentasXCobrar_Migracion(Convert.ToInt32(this.documentMigracionID), this._actFlujo.ID.Value, this.masterConcSaldo.Value, this._ctrlList, this._facturaDocuList);
                this.StopProgressBarThread();
