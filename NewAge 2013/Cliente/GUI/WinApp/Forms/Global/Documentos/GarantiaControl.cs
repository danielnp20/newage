...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    public partial class GarantiaControl : FormWithToolbar
    {
        ...
        private List<int> itemsDelete = new List<int>();
        #endregion
        ...
        private void Form_Enter(object sender, EventArgs e)
        {
            try
            {
                ...
                if (FormProvider.Master.LoadFormTB)
                {
                    ...
                    FormProvider.Master.itemSearch.Enabled = SecurityManager.HasAccess(this._documentID, FormsActions.Add);
                }
            }
            catch (Exception ex)
            {
        ...
        private void gvGarantia_EmbeddedNavigator_ButtonClick(object sender, DevExpress.XtraEditors.NavigatorButtonClickEventArgs e)
        {
            ...
            if (e.Button.ButtonType == DevExpress.XtraEditors.NavigatorButtonType.Remove && this.gvGarantia.RowCount > 0)
            {
                ...
                {
                    DTO_glGarantiaControl rowCurrent = (DTO_glGarantiaControl)this.gvGarantia.GetRow(this.gvGarantia.FocusedRowHandle);
                    if (rowCurrent != null && rowCurrent.Consecutivo.Value.HasValue && !this.itemsDelete.Contains(rowCurrent.Consecutivo.Value.Value))
                        this.itemsDelete.Add(rowCurrent.Consecutivo.Value.Value);
                    this.deleteOP = true;
        ...
        private void gvGarantia_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            try
            {
                if (this.gvGarantia.FocusedRowHandle >= 0)
                {
                    ...
                    this.pnDatosBasicos.Enabled = true;
                    if (this._currentRow.Consecutivo.Value.HasValue && this._currentRow.Consecutivo.Value != 0)
                        this.masterGarantia.EnableControl(false);
                    else
                        this.masterGarantia.EnableControl(true);
                }
            }
            catch (Exception ex)
            {
        ...
        private void SaveThread()
        {
            try
            {
                ...
                DTO_TxResult results = _bc.AdministrationModel.glGarantiaControl_Add(this._documentID, this.detalle,this.itemsDelete);
