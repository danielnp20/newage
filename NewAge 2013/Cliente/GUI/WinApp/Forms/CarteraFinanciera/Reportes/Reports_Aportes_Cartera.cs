    ...
    public class Reports_Aportes_Cartera : ReportParametersForm, IFiltrable
    {
        ...
        protected override void LoadReportMethod_PDF()
        {
            try
            {
                ...
                {
                    reportName = this._bc.AdministrationModel.Report_Cc_AportesCliente(this._fechaIni, this._filtro, this._formatType);
                    fileURl = this._bc.UrlDocumentFile(TipoArchivo.Temp, null, null, reportName);
                    Process.Start(fileURl);
                }
                if (this._report == 2)
                {
                    reportName = this._bc.AdministrationModel.Report_Cc_EstadoDeCuenta(this._fechaIni, this._fechaFin, this._filtro, this._formatType);
                    fileURl = this._bc.UrlDocumentFile(TipoArchivo.Temp, null, null, reportName);
                    Process.Start(fileURl);
                }
            }
