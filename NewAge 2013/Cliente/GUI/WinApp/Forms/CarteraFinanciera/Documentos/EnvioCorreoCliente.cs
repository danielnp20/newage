    ...
    public partial class EnvioCorreoCliente : FormWithToolbar
    {
        ...
        private void SetInitParameters()
        {
            try
            {
                ...
                this._bc.InitMasterUC(this.masterCorreo, AppMasters.glCorreosEspeciales, true, true, true, true);
        ...
        /// <summary>
        /// Obtiene el texto traducido de cada opcion
        /// </summary>
        /// <param name="option"></param>
        /// <returns></returns>
        private string GetTexTipoCuenta(byte option)
        {
            string texto = string.Empty;
            switch (option)
            {
                case 1: texto = this._bc.GetResource(LanguageTypes.Tables, "tbl_restr_CuentaOrigen_v1");
                    break;
                case 2:
                    texto = this._bc.GetResource(LanguageTypes.Tables, "tbl_restr_CuentaOrigen_v2");
                    break;
                case 3:
                    texto = this._bc.GetResource(LanguageTypes.Tables, "tbl_restr_CuentaOrigen_v3");
                    break;
                case 4:
                    texto = this._bc.GetResource(LanguageTypes.Tables, "tbl_restr_CuentaOrigen_v4");
                    break;
                case 5:
                    texto = this._bc.GetResource(LanguageTypes.Tables, "tbl_restr_CuentaOrigen_v5");
                    break;
            }
            return "(" + texto  + ")";
        }
        ...
        /// <summary>
        /// Evento que se ejecuta al momento de salir del control
        /// </summary>
        private void masterCorreo_Leave(object sender, EventArgs e)
        {
            try
            {
                if (this.masterCorreo.ValidID)
                {
                    DTO_glCorreosEspeciales correo = (DTO_glCorreosEspeciales)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.glCorreosEspeciales, false, this.masterCorreo.Value, true);
                    this._cuentaCorreo = correo.CuentaOrigen.Value.Value;
                    this.txtAsunto.Text = correo.Asunto.Value;
                    this._asunto = correo.Asunto.Value;

                    if (!string.IsNullOrEmpty(this._mensaje))
                    {
                        if (MessageBox.Show("Desea volver a cargar la plantilla del mensaje actual", "Advertencia", MessageBoxButtons.YesNo) == DialogResult.Yes)
                            this.uc_TextoCorreo.ValueHTML = correo.PlantillaEMail.Value;
                    }
                    else
                        this.uc_TextoCorreo.ValueHTML = correo.PlantillaEMail.Value;
                                        
                    this._mensaje = this.uc_TextoCorreo.ValueHTML; 
                }
                else
                {
                    this._cuentaCorreo = 1;
                }
                this.lblTipoCuenta.Text = GetTexTipoCuenta(this._cuentaCorreo);

            }
            catch (Exception ex)
            {
                MessageBox.Show(this._bc.GetResourceForException(ex, "WinApp-EnvioCorreoCliente.cs", "masterCorreo_Leave"));
            }
        }
        ...
        public virtual void SendToApproveThread()
        {
            try
            {
                ...
                foreach (DTO_CorreoCliente mail in this._listCorreos)
                {
                    ...
                    this._bc.SendMail(this.documentID, this._asunto, this._mensaje, mail.Correo.Value,this._cuentaCorreo);
                    #endregion
                }
