    ...
    public partial class PolizaRegistro : FormWithToolbar
    {
        ...
        private void SetInitParameters()
        {
            try
            {
                ...
                dicTipoMvto.Add((byte)EstadoPoliza.PolizaNuevaInmuebles, this._bc.GetResource(LanguageTypes.Tables, "tbl_EstadoPoliza_PolizaNuevaInmueble"));
                dicTipoMvto.Add((byte)EstadoPoliza.PolizaRenuevaInmuebles, this._bc.GetResource(LanguageTypes.Tables, "tbl_EstadoPoliza_PolizaRenovaInmueble"));
        ...
        private void LoadObligaciones()
        {
            ...
            else if (this._tipoMvto == (byte)EstadoPoliza.Renovada || this._tipoMvto == (byte)EstadoPoliza.PolizaExternaRenovada)
            {
                ...
                this.lkpCreditos.Properties.DataSource = cred1;
                     ...
                     this._tipoMvto == (byte)EstadoPoliza.PolizaPagada || this._tipoMvto == (byte)EstadoPoliza.PolizaNuevaInmuebles)
            {
                //Lista de solicitudes
                this._listSolicitudes = this._bc.AdministrationModel.GetSolicitudesByCliente(this.masterTercero.Value);
                if (this._tipoMvto != (byte)EstadoPoliza.PolizaNuevaInmuebles)
                {
                ...
                {
                    List<int?> credOK = new List<int?>();
                    List<int?> credNOK = new List<int?>();
                    foreach (DTO_ccSolicitudDocu sol in this._listSolicitudes)
                    {

                            DTO_ccLineaCredito linea = (DTO_ccLineaCredito)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccLineaCredito, false, sol.LineaCreditoID.Value, true);
                            DTO_ccClasificacionCredito clase = (DTO_ccClasificacionCredito)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccClasificacionCredito, false, linea != null ? linea.ClaseCredito.Value : string.Empty, true);
                            if (clase != null && clase.HipotecarioInd.Value.HasValue && clase.HipotecarioInd.Value.Value && !credOK.Exists(x => x == sol.Libranza.Value))
                                credOK.Add(sol.Libranza.Value);
                            else
                                credNOK.Add(sol.Libranza.Value);
                    }
                    //foreach (int? lib in credNOK)
                    //{
                    //    this._listSolicitudes.RemoveAll(x => x.Libranza.Value == lib);
                    //}
                    this.lkpSolicitudes.Properties.DataSource = credOK;
                }
            }
            else if (this._tipoMvto == (byte)EstadoPoliza.PolizaRenuevaInmuebles)
            {
                //Lista de créditos
                this._listCreditos = this._bc.AdministrationModel.GetCreditoByCliente(this.masterTercero.Value);
                ...
                List<int?> credOK = new List<int?>();
                List<int?> credNOK = new List<int?>();
                foreach (DTO_ccCreditoDocu cred in this._listCreditos)
                {
                    if(!cred.CanceladoInd.Value.Value)
                    {
                        DTO_ccLineaCredito linea = (DTO_ccLineaCredito)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccLineaCredito, false, cred.LineaCreditoID.Value, true);
                        DTO_ccClasificacionCredito clase = (DTO_ccClasificacionCredito)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccClasificacionCredito, false, linea != null? linea.ClaseCredito.Value : string.Empty, true);
                        if (clase != null && clase.HipotecarioInd.Value.HasValue && clase.HipotecarioInd.Value.Value && !credOK.Exists(x => x == cred.Libranza.Value))
                            credOK.Add(cred.Libranza.Value);
                        else
                            credNOK.Add(cred.Libranza.Value);
                    }
                    else
                        credNOK.Add(cred.Libranza.Value);
                }
                //foreach (int? lib in credNOK)
                //{
                //    this._listCreditos.RemoveAll(x=>x.Libranza.Value == lib);
                //}

                //List<int?> cred1 = (from p in this._listCreditos where !p.CanceladoInd.Value.Value select p.Libranza.Value).Distinct().ToList();
                this.lkpCreditos.Properties.DataSource = credOK;
            }
        }
        ...
        private bool ValidateHeader()
        {
            ...
            #endregion
            #region Valida Valor con saldo de Cta Gastos
            if (this._tipoMvto == (byte)EstadoPoliza.PolizaRenuevaInmuebles || this._tipoMvto == (byte)EstadoPoliza.PolizaNuevaInmuebles)
            {
                if ((Convert.ToDecimal(this.txtValor.EditValue) - this.GetSaldoGastosPol()) > 0)
                {
                    MessageBox.Show(this._bc.GetResource(LanguageTypes.Messages, "El valor de la póliza no corresponde con el saldo de la cuenta de Gastos de Póliza Inmuebles (" + this._bc.GetControlValueByCompanyAllowEmpty(ModulesPrefix.cc, AppControl.cc_CuentaGastosPolInmuebles) + ")"));
                    this.txtValor.Focus();
                    return false;
                }
            }
        ...
        /// <summary>
        /// Obtiene el saldo del tercero de cta de gastos
        /// </summary>
        /// <returns></returns>
        private decimal GetSaldoGastosPol()
        {
            try
            {
                decimal saldoGastos = 0;
                string cuentaGastosPol = this._bc.GetControlValueByCompanyAllowEmpty(ModulesPrefix.cc, AppControl.cc_CuentaGastosPolInmuebles);
                ...
                saldoFilter.CuentaID.Value = cuentaGastosPol;
                saldoFilter.TerceroID.Value = this.masterTercero.Value;
                if (!string.IsNullOrEmpty(saldoFilter.CuentaID.Value))
                {
                    ...
                    saldoGastos = Math.Abs(saldosList.Sum(x => x.DbOrigenLocML.Value.Value + x.DbOrigenExtML.Value.Value + x.CrOrigenLocML.Value.Value +
            ...
            {
                MessageBox.Show(this._bc.GetResourceForException(ex, "WinApp-PolizaRegistro.cs", "GetSaldoGastosPol"));
                return 0;
            }
        }
