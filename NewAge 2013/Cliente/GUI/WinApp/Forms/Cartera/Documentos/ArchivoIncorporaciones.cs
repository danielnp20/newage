...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    public partial class ArchivoIncorporaciones : FormWithToolbar
    {
        ...
        private DTO_ccPagaduria dtoPagaduria = new DTO_ccPagaduria();
        ...
        public ArchivoIncorporaciones(string mod)         
        {
            InitializeComponent();
            this.SetInitParameters();
            this._frmName = _bc.GetResource(LanguageTypes.Forms, this._documentID.ToString());
        ...
        private void LoadDocuments()
        {
            try
            {
                ...
                {
                    this.incorporaciones = this._bc.AdministrationModel.GetArchivosIncorporacion(this.dtPeriodo.DateTime.Date, this.masterPagaduria.Value);
        ...
        private void masterPagaduria_Leave(object sender, EventArgs e)
        {
            try
            {
                if (this.pagaduriaID != this.masterPagaduria.Value)
                {
                    this.pagaduriaID = this.masterPagaduria.Value;
                    this.dtoPagaduria = (DTO_ccPagaduria)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccPagaduria, false, this.masterPagaduria.Value, true);
                    if (this.dtoPagaduria != null)
                    {
                        this.lkp_Tipo.EditValue = (byte)TipoIncorporaCartera.Afiliaciones;
                        this.LoadDocuments();
                    }
                    ...
                    {
                        string cp = this.masterPagaduria.Value;
                        this.CleanData();
                        this.masterPagaduria.Value = cp;
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ArchivoIncorporaciones", "masterPagaduria_Leave"));
            }
        }
            ...
            try
            {
                ...
                if (this.incorporaciones != null && this.incorporaciones.Count > 0)
                {
                    ...
                    switch (tipo)
                    {
                        case (byte)TipoIncorporaCartera.Afiliaciones:
                            codExportacion = this.dtoPagaduria.CodTransmisionAf.Value;
                        ...
                        case (byte)TipoIncorporaCartera.Desafiliaciones:
                            codExportacion = this.dtoPagaduria.CodTransmisionDesaf.Value;
                        ...
                        case (byte)TipoIncorporaCarteraArchivos.Incorporaciones:
                            codExportacion = this.dtoPagaduria.CodTransmisionInc.Value;
                        ...
                        case (byte)TipoIncorporaCarteraArchivos.Desincorporaciones:
                            codExportacion = this.dtoPagaduria.CodTransmisionDesinc.Value;
                            this.incorporacionesFilter = this.incorporaciones.Where(i => i.TipoNovedad.Value == (byte)TipoNovedad.Desincorpora).ToList();
                ...
                if(this.incorporaciones != null && this.incorporaciones.Count > 0)
                {
                    if (string.IsNullOrWhiteSpace(codExportacion))
                    {
                        ...
                        MessageBox.Show(string.Format(msg, this.dtoPagaduria.ID.Value));
                    }
                    else
