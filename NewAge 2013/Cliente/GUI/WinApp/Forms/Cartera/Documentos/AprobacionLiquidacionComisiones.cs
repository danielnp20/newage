    ...
    public partial class AprobacionLiquidacionComisiones : DocumentAprobBasicForm
    {
        ...
        protected override void LoadDocuments()
        {
            try
            {
                ...
                DateTime PeriodoAprobar = new DateTime(this.dtAprobComisiones.DateTime.Year, this.dtAprobComisiones.DateTime.Month, 1);

                this.liquidaComisionDeta = this._bc.AdministrationModel.LiquidacionComisionesCartera_GetForAprobacion(this.actividadFlujoID, PeriodoAprobar);
        ...
        /// <param name="sender">Objeto que envia el evento</param>
        /// <param name="e">Evento</param>
        protected virtual void dtAprobComisiones_Leave(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(this.dtAprobComisiones.Text))
            {
                this.dtAprobComisiones.DateTime = DateTime.Now;
            }
            ...
            try
            {
                ...
                if (checkResults)
                {
                    foreach (object obj in results)
                    {
                        ...
                        {
                            bool isOK = this._bc.SendDocumentMail(MailType.NotSend, AppDocuments.AprobacionReintegroClientes, this.actividadDTO.seUsuarioID.Value, obj, false);
                            if (!isOK)
                            {
