...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    public partial class AprobacionGiro : DocumentAprobBasicForm
    {
        ...
        private DTO_ccCreditoDocu crediDocuCurrent = null;
        ...
        protected override void SetInitParameters()
        {
            ...
            this.gvDocuments.OptionsCustomization.AllowSort = true;
        }
        ...
        private void PagoDirecto()
        {
            ...
            foreach (DTO_ccCreditoDocu item in this.creditosTemp)
            {
                ...
                this.crediDocu = item;
                this.crediDocu.Nombre.Value = cliente.ApellidoPri.Value +" "+ cliente.ApellidoSdo.Value + " " + cliente.NombrePri.Value + " " + cliente.NombreSdo.Value;
        ...
        protected override void chkSeleccionar_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (this.chkSeleccionar.Checked)
            {
                ...
                {
                    DTO_ccCreditoDocu cred = (DTO_ccCreditoDocu)this.gvDocuments.GetRow(i);
                    cred.Aprobado.Value = true;
                    cred.Rechazado.Value = false;
                    valorTotal += cred.VlrGiro.Value.Value;
                }
        ...
        protected override void gvDocuments_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                this.crediDocuCurrent = (DTO_ccCreditoDocu)this.gvDocuments.GetRow(e.FocusedRowHandle);
                ...
                if (this.currentRow != -1)
                {
                    if (e.FocusedRowHandle >= 0 && this.lkp_TipoPago.EditValue == "1" && e.FocusedRowHandle <= this.gvDocuments.RowCount - 1)
                    {
                        ...
                        this.txtVlrGiro.EditValue = this.crediDocuCurrent.VlrGiro.Value;
                    }
                }
                else
                {
        ...
        protected override void gvDocuments_CellValueChanging(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                if (e.RowHandle >= 0 && this.crediDocuCurrent != null)
                {
                    ...
                    if (fieldName == "Aprobado")
                    {
                        ...
                        {
                            add = true;
                            this.crediDocuCurrent.Aprobado.Value = true;
                            this.crediDocuCurrent.Rechazado.Value = false;
                            if (String.IsNullOrWhiteSpace(this.crediDocuCurrent.Observacion.Value))
                                this.gvDocuments.Columns[11].Visible = false;
                        }
                        else
                        {
                            delete = true;
                            this.crediDocuCurrent.Aprobado.Value = false;
                        }
                    }
                    if (fieldName == "Rechazado")
                    {
                        if ((bool)e.Value)
                        {
                            ...
                            if (this.crediDocuCurrent.Aprobado.Value.Value)
                                delete = true;
                            this.crediDocuCurrent.Aprobado.Value = false;
                            this.crediDocuCurrent.Rechazado.Value = true;
                        }
                        else
                        {
                            this.crediDocuCurrent.Rechazado.Value = false;
                    ...
                    {
                        this.crediDocuCurrent.Observacion.Value = e.Value.ToString();
                    }
                    #endregion
                    ...
                    if (this.lkp_TipoPago.EditValue == "2")
                    {
                        ...
                        if (add)
                            vlrTotal += this.crediDocuCurrent.VlrGiro.Value.Value;
                        else if (delete)
                            vlrTotal -= this.crediDocuCurrent.VlrGiro.Value.Value;
