...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    public partial class GestionCobranza : FormWithToolbar
    {
        ...
        List<DTO_GestionCobranza> cobranzasGuarda = new List<DTO_GestionCobranza>();
        private bool deleteOP = false;
        private bool isValid = false;
        private bool isSaved = false;
        DTO_GestionCobranza gestion = null;
        #endregion
        ...
        private void InitControls()
        {
            ...
            this._bc.InitMasterUC(this.masterGestionCobranza, AppMasters.ccCobranzaGestion, true, true, false, false, null);
            this._bc.InitMasterUC(this.masterEstadoCobranza, AppMasters.ccCobranzaEstado, true, true, true, false);
            this._bc.InitMasterUC(this.masterSiniestro, AppMasters.ccSiniestroEstado, true, true, true, false);
        ...
        private void LoadData()
        {
            try
            {
                if (this.ValidateHeader())
                {
                    ...
                    //this.cobranzas=this.cobranzas(x=>x.)
        ...
        private void AddGridCols()
        {
            try
            {
                ...
                //Selecciona
                GridColumn SeleccionaInd = new GridColumn();
                SeleccionaInd.FieldName = this._unboundPrefix + "SeleccionaInd";
                SeleccionaInd.Caption = " ";
                SeleccionaInd.UnboundType = UnboundColumnType.Boolean;
                SeleccionaInd.VisibleIndex = 0;
                SeleccionaInd.Width = 40;
                SeleccionaInd.Visible = true;
                SeleccionaInd.OptionsColumn.AllowEdit = true;
                this.gvData.Columns.Add(SeleccionaInd);
                ...
                Valor1.FieldName = this._unboundPrefix + "Valor";
        ...
        private void gvData_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                {
                    this.gestion = (DTO_GestionCobranza)this.gvData.GetRow( e.FocusedRowHandle);
                    ...
                    this.chkMantenerGestion.Checked = this.gestion.MantenerGestionInd.Value.HasValue ? this.gestion.MantenerGestionInd.Value.Value : false;

                    this.masterGestionCobranza.Value = gestion.CobranzaGestionCierre.Value;
                    if (!string.IsNullOrEmpty(gestion.CobranzaGestionCierre.Value))
                    {
                        DTO_ccCobranzaGestion cob = (DTO_ccCobranzaGestion)this._bc.GetMasterDTO(AppMasters.MasterType.Simple,AppMasters.ccCobranzaGestion,false,this.masterGestionCobranza.Value,true);
                        if (cob != null)
                        {
                            //if (cob.ControlTipo.Value == 1 || cob.ControlTipo.Value == 2)//PErmite cambiar si es ORden Inmovilizacion
                            if (cob.GestionDemanda.Value == null || cob.GestionDemanda.Value == 0)//PErmite cambiar si es ORden Inmovilizacion
                                this.masterGestionCobranza.EnableControl(true);
                            else 
                                this.masterGestionCobranza.EnableControl(false);

                            if (cob.GestionDemanda.Value == 2)
                                this.chkMantenerGestion.Enabled = true;
                            else
                                this.chkMantenerGestion.Enabled = false;
                        }
                    }
                    this.masterEstadoCobranza.Value = gestion.CobranzaEstadoID.Value;
                    this.masterSiniestro.Value = gestion.SiniestroEstadoID.Value;
                    if (gestion.FechaConfirmacionHist.Value.HasValue)
                        this.dtFechaConf.DateTime = gestion.FechaConfirmacionHist.Value.Value;
                    else
                    {
                        this.dtFechaConf.ResetText();
                    }
                    this.txtCodConfir.Text = gestion.CodConfirmacionHist.Value;   
                    this.gvCodeudor.RefreshData();
                    this.txtHistoria.Focus();
                    SendKeys.Send("^{END}");//Permite ir al final del texto del control en foco
                }
            }
            catch (Exception ex)
            {
        ...
        {
            if (!string.IsNullOrWhiteSpace(gestion.SeleccionaInd.ToString()))
            {
                string msgTitleSearch = string.Format(_bc.GetResource(LanguageTypes.Messages, DictionaryMessages.Title_Warning));
                string msgFechaEmpty = _bc.GetResource(LanguageTypes.Messages, "No ha guardado los datos modificados al cambiar de registro perdera la informacion digitada, desea continuar");
                e.Allow = true;
                if (!this.isSaved)
                {
                    ...
                    {
                        //this.gvData.FocusedRowHandle = e.RowHandle;
                        e.Allow = false;
        ...
        public override void TBNew()
        {
            try
            {
                ...
                this.masterSiniestro.Value = string.Empty;
                this.chkMantenerGestion.Checked = false;
            }
            catch (Exception ex)
            {
        ...
        public override void TBSave()
        {
            ...
            {
                isSaved = true;
                if (this.cobranzas.Any(x=>x.SeleccionaInd.Value == true))
                    cobranzasGuarda = cobranzas.FindAll(x => x.SeleccionaInd.Value == true);
                else
                {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void dtFechaConf_EditValueChanged(object sender, EventArgs e)
        {
            if (this.gvData.FocusedRowHandle >= 0 && this.gvData.DataRowCount > 0)
            {
                DTO_GestionCobranza gestion = (DTO_GestionCobranza)this.gvData.GetRow(this.gvData.FocusedRowHandle);
                if (!string.IsNullOrEmpty(this.dtFechaConf.Text))
                    gestion.FechaConfirmacionHist.Value = this.dtFechaConf.DateTime;
                else
                    gestion.FechaConfirmacionHist.Value = null;
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txtCodConfir_TextChanged(object sender, EventArgs e)
        {
            if (this.gvData.FocusedRowHandle >= 0 && this.gvData.DataRowCount > 0)
            {
                DTO_GestionCobranza gestion = (DTO_GestionCobranza)this.gvData.GetRow(this.gvData.FocusedRowHandle);
                gestion.CodConfirmacionHist.Value = this.txtCodConfir.Text;
        ...
        private void masterGestionCobranza_Leave(object sender, EventArgs e)
        {
            try
            {
                if (this.gvData.FocusedRowHandle >= 0 && this.gvData.DataRowCount > 0)
                {
                    string gestionAnterior = this.gestion.CobranzaGestionCierre.Value;
                    DTO_GestionCobranza gestion = (DTO_GestionCobranza)this.gvData.GetRow(this.gvData.FocusedRowHandle);

                    DTO_ccCobranzaGestion cobranzaAnterior = (DTO_ccCobranzaGestion)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccCobranzaGestion, false, gestionAnterior, true);
                    DTO_ccCobranzaGestion cobranzaNueva = (DTO_ccCobranzaGestion)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccCobranzaGestion, false, this.masterGestionCobranza.Value, true);

                    string msgValid = string.Empty;
                    //Si ControlTipo = 2 (manual)
                    if (cobranzaAnterior != null && cobranzaAnterior.ControlTipo.Value == 2)
                    {
                        if (cobranzaNueva != null && cobranzaNueva.ControlTipo.Value != 2)
                            msgValid = "El código digitado debe corresponder a una gestion de Control Tipo Manual";
                    }
                    //Si ControlTipo = 1 (Automatico) 
                    else if (cobranzaAnterior != null && cobranzaAnterior.ControlTipo.Value == 1)
                    {                  
                       if (cobranzaNueva != null && cobranzaNueva.ControlTipo.Value != 2 && cobranzaNueva.GestionDemanda.Value != 2)//PErmite cambiar si es ORden Inmovilizacion
                            msgValid = "El código digitado debe corresponder a una gestión Control Tipo Manual y Demanda de Orden de Inmovilización";
                    }
                    ////Si es cualquier otro ControlTipo
                    //else if(cobranzaAnterior != null)
                    //    msgValid = "El código digitado no debe ser de Control Tipo Manual";

                    if(!string.IsNullOrEmpty(msgValid))
                    {
                        MessageBox.Show(msgValid);
                        this.masterGestionCobranza.Value = gestionAnterior;
                    }
                    else
                        gestion.CobranzaGestionCierre.Value = this.masterGestionCobranza.Value;
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-masterGestionCobranza_Leave.cs", "TBNew"));
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void masterEstadoCobranza_Leave(object sender, EventArgs e)
        {
            if (this.gvData.FocusedRowHandle >= 0 && this.gvData.DataRowCount > 0)
            {
                DTO_GestionCobranza gestion = (DTO_GestionCobranza)this.gvData.GetRow(this.gvData.FocusedRowHandle);
                gestion.CobranzaEstadoID.Value = this.masterEstadoCobranza.Value;
            }
        }
        /// <summary>
        /// 
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void masterEstadoSiniestro_Leave(object sender, EventArgs e)
        {
            if (this.gvData.FocusedRowHandle >= 0 && this.gvData.DataRowCount > 0)
            {
                DTO_GestionCobranza gestion = (DTO_GestionCobranza)this.gvData.GetRow(this.gvData.FocusedRowHandle);
                gestion.SiniestroEstadoID.Value = this.masterSiniestro.Value;
            }
        }
        #endregion
        ...
        private void SaveThread()
        {
            try
            {
                ...
                DTO_TxResult results = this._bc.AdministrationModel.GestionCobranza_Add(this._documentID, this.cobranzasGuarda);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void chkCambioGestion_CheckedChanged(object sender, EventArgs e)
        {
            if (this.gvData.FocusedRowHandle >= 0 && this.gvData.DataRowCount > 0)
            {
                DTO_GestionCobranza gestion = (DTO_GestionCobranza)this.gvData.GetRow(this.gvData.FocusedRowHandle);
                gestion.MantenerGestionInd.Value = this.chkMantenerGestion.Checked;
            }
        }
