...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    public partial class RecaudosManuales : DocumentForm
    {
        ...
        private string _sectorCartera = string.Empty;
        ...
        protected override void SetInitParameters()
        {
            ...
            this._sectorCartera = this._bc.GetControlValueByCompany(ModulesPrefix.cc, AppControl.cc_SectorCartera);
        ...
        protected virtual void AddDocumentCols()
        {
            try
            {
                ...
                vlrPago.OptionsColumn.AllowEdit = true;// this._allowEditValuesInd;
        ...
        private void lkp_Libranzas_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                string compMora = this._bc.GetControlValueByCompany(ModulesPrefix.cc, AppControl.cc_ComponenteMora);
                    ...
                    && (this.libranzaID != this.lkp_Libranzas.Text || this.fechaRecaudo != this.dtFechaConsigna.DateTime.Date))
                {
                    ...
                    if (temp.Count > 0)
                    {
                        ...
                        foreach (DTO_ccCreditoPlanPagos pp in this.infoCartera.PlanPagos)
                        {
                            #region Valida Si es credito en Cobro Juridico para financieras
                            if (Convert.ToByte(_sectorCartera) == (byte)SectorCartera.Financiero)
                            {
                                if (this._credito.TipoEstado.Value == (byte)TipoEstadoCartera.CobroJuridico || this._credito.TipoEstado.Value == (byte)TipoEstadoCartera.AcuerdoPagoIncumplido)
                                {
                                    #region Valida el valor de la mora del Capital
                                    DTO_ccSaldosComponentes saldoCompMora = this.infoCartera.SaldosComponentes.Find(x => x.CuotaID.Value == pp.CuotaID.Value && x.ComponenteCarteraID.Value == compMora);
                                    //Trae el saldo del Capital de la cuota del credito
                                    decimal? vlrCapital = this.infoCartera.SaldosComponentes.Where(x => x.CuotaID.Value == pp.CuotaID.Value && x.ComponenteCarteraID.Value == compCapital).Sum(x => x.TotalSaldo.Value);
                                    vlrCapital = vlrCapital ?? 0;
                                    if (vlrCapital > 0)
                                    {
                                        decimal saldoMoraCap = 0;
                                        //Trae el ultimo abono de capital realizado y trae el saldo de interes de Mora
                                        var cjHistoricos = _bc.AdministrationModel.ccCJHistorico_GetForAbono(numeroDoc, false);
                                        DTO_ccCJHistorico cjHistoricoPrincipal = cjHistoricos.Item1;
                                        if (cjHistoricoPrincipal != null && cjHistoricoPrincipal.SaldoInteres.Value.HasValue)
                                            saldoMoraCap = cjHistoricoPrincipal.SaldoInteres.Value.Value;
                                        if (saldoCompMora != null)
                                        {
                                            saldoCompMora.CuotaSaldo.Value = saldoMoraCap;
                                            saldoCompMora.TotalSaldo.Value = saldoMoraCap;
                                        }

                                        decimal? vlrSeguro = this.infoCartera.SaldosComponentes.Where(x => x.CuotaID.Value == pp.CuotaID.Value && x.ComponenteCarteraID.Value == compSeguro).Sum(x => x.TotalSaldo.Value);

                                        //Elimina todos los componentes diferentes al capital y Mora si no hay seguro
                                        if (vlrSeguro != 0)                                          
                                            this.infoCartera.SaldosComponentes.RemoveAll(x => x.CuotaID.Value == pp.CuotaID.Value && !x.ComponenteCarteraID.Value.Equals(compCapital) && !x.ComponenteCarteraID.Value.Equals(compMora));
                                    }
                                    #endregion
                                }
                                else if (this._credito.TipoEstado.Value == (byte)TipoEstadoCartera.AcuerdoPago)
                                {
                                    #region Valida el valor de la mora del Seguro
                                    DTO_ccSaldosComponentes saldoCompMora = this.infoCartera.SaldosComponentes.Find(x => x.CuotaID.Value == pp.CuotaID.Value && x.ComponenteCarteraID.Value == compMora);
                                    //Trae el saldo del Capital de la cuota del credito
                                    decimal? vlrSeguro = this.infoCartera.SaldosComponentes.Where(x => x.CuotaID.Value == pp.CuotaID.Value && x.ComponenteCarteraID.Value == compSeguro).Sum(x => x.TotalSaldo.Value);
                                    vlrSeguro = vlrSeguro ?? 0;
                                    if (vlrSeguro > 0)
                                    {
                                        decimal saldoMoraSeg = 0;
                                        //Trae el ultimo abono del seguro realizado y trae el saldo de interes de Mora
                                        var cjHistoricos = _bc.AdministrationModel.ccCJHistorico_GetForAbono(numeroDoc, false);
                                        DTO_ccCJHistorico cjHistoricoPoliza = cjHistoricos.Item2;
                                        if (cjHistoricoPoliza != null && cjHistoricoPoliza.SaldoInteres.Value.HasValue)
                                            saldoMoraSeg = cjHistoricoPoliza.SaldoInteres.Value.Value;
                                        if (saldoCompMora != null)
                                        {
                                            saldoCompMora.CuotaSaldo.Value = saldoMoraSeg;
                                            saldoCompMora.TotalSaldo.Value = saldoMoraSeg;
                                        }
                                        //Elimina el componentes de capital
                                        this.infoCartera.SaldosComponentes.RemoveAll(x => x.CuotaID.Value == pp.CuotaID.Value && x.ComponenteCarteraID.Value.Equals(compCapital));
                                    }
                                    #endregion
                                }
                            }
                            ...
                            pp.NuevoVlrMoraLiquida.Value = comps.Where(x => x.ComponenteCarteraID.Value == compMora).Sum(x => x.CuotaSaldo.Value);
                            pp.VlrMora.Value = pp.NuevoVlrMoraLiquida.Value;
                            pp.VlrMoraLiquida.Value = pp.NuevoVlrMoraLiquida.Value;
                        }
        ...
        protected override void gvDocument_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (fieldName == "PagoInd")
            {
                ...
                {
                    this.cuota = (DTO_ccCreditoPlanPagos)this.gvDocument.GetRow(e.RowHandle);

                    this.cuota.VlrPagadoCuota.Value = 0;
                    this.cuota.PagoInd.Value = false;

                    //Actualiza la info de los componentes
                    infoCartera.SaldosComponentes.RemoveAll(c => c.CuotaID.Value == this.cuota.CuotaID.Value && c.Editable.Value.Value);
                    infoCartera.SaldosComponentes.Where(c => c.CuotaID.Value == this.cuota.CuotaID.Value).ToList().ForEach(cc => cc.AbonoValor.Value = 0);
                    this.gcDocument.RefreshDataSource();
                }
        ...
        protected override void gvDocument_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                if (row > 0)
                {
                    ...
                    {
                        this.gvDocument.Columns[this.unboundPrefix + "VlrPagadoCuota"].OptionsColumn.AllowEdit = true;//this._allowEditValuesInd;
                    }
                    else
        ...
        private void gvPagos_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (fieldName == "VlrPago")
            {
                ...
                SectorCartera sector = (SectorCartera)Enum.Parse(typeof(SectorCartera), this._sectorCartera);
                decimal vlr = e.Value != null? (decimal)e.Value : 0;
                if (sector == SectorCartera.Solidario && Convert.ToDecimal(this.txtTotalPlanPagos.EditValue) != vlr)
                {
                    int rowCurrent = 0;
                    decimal valorRecaudo = e.Value != null? (decimal)e.Value : 0;
                    this.infoCartera.PlanPagos.ForEach(x => {
                                            x.PagoInd.Value = false;
                                            x.VlrPagadoCuota.Value = 0;
                                            //Actualiza la info de los componentes
                                            infoCartera.SaldosComponentes.RemoveAll(c => c.CuotaID.Value == x.CuotaID.Value && c.Editable.Value.Value);
                                            infoCartera.SaldosComponentes.Where(c => c.CuotaID.Value == x.CuotaID.Value).ToList().ForEach(cc => cc.AbonoValor.Value = 0);
                    });
                    foreach (DTO_ccCreditoPlanPagos pp in infoCartera.PlanPagos)
                    {
                        rowCurrent++;
                        this.cuota = pp;
                        this.componentes = this.infoCartera.SaldosComponentes.Where(x => x.CuotaID.Value == pp.CuotaID.Value).ToList();
                        this.vlrUsura = (from c in this.componentes where c.ComponenteCarteraID.Value == this.componenteUsura select c.CuotaSaldo.Value.Value).Sum();
                        this.componentes.RemoveAll(x => x.CuotaSaldo.Value == 0 && x.TotalSaldo.Value == 0 && x.AbonoValor.Value == 0);
                        this.componentes.OrderBy(x => x.ComponenteCarteraID.Value).ToList();

                        if (valorRecaudo >= pp.VlrSaldo.Value)
                        {
                            pp.VlrPagadoCuota.Value = pp.VlrSaldo.Value;
                            pp.PagoInd.Value = true;
                            this.CalcularPagoComponentes(e.RowHandle, pp.VlrSaldo.Value.Value);
                            valorRecaudo -= pp.VlrSaldo.Value.Value;
                        }
                        else
                        {
                            decimal abono = valorRecaudo;
                            pp.VlrPagadoCuota.Value = abono;
                            if (pp.VlrPagadoCuota.Value < 0)
                            {
                                //string msg = string.Format(this.msgValorNeg, fieldName);
                                //this.gvDocument.SetColumnError(col, msg);
                                //this.validPlanPagos = false;
                            ...
                            if (this.validPlanPagos)
                            {
                                ...
                                if (pp.VlrPagadoCuota.Value > 0)
                                    ...
                                    pp.PagoInd.Value = false;

                                //if (pp.VlrPagadoCuota.Value != pp.VlrCuota.Value)
                                //{
                                //    for (int i = e.RowHandle + 1; i < this.infoCartera.PlanPagos.Count; ++i)
                                //    {
                    ...
                    this.gvDocument.FocusedRowHandle = rowCurrent;
                    this.CalcularTotal_PlanPagos();
        ...
        public override void TBSave()
        {
            try
            {
                ...
                if (isValid && this.validPlanPagos && this.validFormasPago)
                {
                    ...
                    reciboCaja.Descripcion.Value = this.txtDescriptivo.Text;
                    #endregion
