...
{
    public partial class SolicitudLibranza : FormWithToolbar
    {
        #region Variables
        ...
        /// Constructor del Documento
        /// </summary>
        public SolicitudLibranza(string mod)
        {
        ...
        public void Constructor(string mod = null)
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "Constructor"));
            }
        }
        ...
        private void SetInitParameters()
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "SetInitParameters"));
            }
        }
        ...
        private bool ValidateHeader()
        {
            ...
            this._header.CentroPagoID.Value = this._centroPagoXDef;
        ...
        private void GetValues()
        {
            ...
            this._centroPagoID = this._centroPagoXDef;
            this._pagaduriaID = this.masterPagaduria.Value;
        }

        /// <summary>
        /// Obtiene los saldos de credito de la cedula digitada
        /// </summary>
        private void GetSaldosCedula(string master)
        {           
            decimal saldoPersona = 0;
            #region Trae los saldos Cliente 
            if (master.Equals("masterCliente"))
            {
                if (this.masterCliente.ValidID)
                {                   
                    //Trae creditos por deudor
                    List<DTO_ccCreditoDocu> creditos = this._bc.AdministrationModel.GetCreditosPendientesByCliente(this.masterCliente.Value);
                    //Trae creditos por codeudor
                    creditos.AddRange(this._bc.AdministrationModel.GetCreditosByCodeudor(this.masterCliente.Value));
            ...
            #endregion
            #region Trae los saldos Cod 1 
            else if (master.Equals("masterCodeudor1"))
            {
                if (this.masterCodeudor1.ValidID)
                {
                    //Trae creditos por deudor
                    List<DTO_ccCreditoDocu> creditos = this._bc.AdministrationModel.GetCreditosPendientesByCliente(this.masterCodeudor1.Value);
                    //Trae creditos por codeudor
                    creditos.AddRange(this._bc.AdministrationModel.GetCreditosByCodeudor(this.masterCodeudor1.Value));
                    //Obtiene el saldo total de creditos por cedula
                    foreach (DTO_ccCreditoDocu cred in creditos)
                    {
                        DTO_InfoCredito saldo = this._bc.AdministrationModel.GetSaldoCredito(cred.NumeroDoc.Value.Value, DateTime.Now, false, false, false);
            ...
            #endregion
            #region Trae los saldos Cod 2
            else if (master.Equals("masterCodeudor2"))
            {
                if (this.masterCodeudor2.ValidID)
                {
                    //Trae creditos por deudor
                    List<DTO_ccCreditoDocu> creditos = this._bc.AdministrationModel.GetCreditosPendientesByCliente(this.masterCodeudor2.Value);
                    //Trae creditos por codeudor
                    creditos.AddRange(this._bc.AdministrationModel.GetCreditosByCodeudor(this.masterCodeudor2.Value));
                    //Obtiene el saldo total de creditos por cedula
                    foreach (DTO_ccCreditoDocu cred in creditos)
                    {
                        DTO_InfoCredito saldo = this._bc.AdministrationModel.GetSaldoCredito(cred.NumeroDoc.Value.Value, DateTime.Now, false, false, false);
        ...
        protected virtual void Form_Enter(object sender, EventArgs e)
        {
            try
            {
                ...
                //Valida si permite editar, no debe tener ninguna actividad pendiente
                if (this._ctrl.NumeroDoc.Value.HasValue)
                {
                    if (this._sector != SectorCartera.Financiero && !string.IsNullOrEmpty(this._bc.AdministrationModel.glActividadEstado_GetActFlujoByNumeroDoc(this._ctrl.NumeroDoc.Value.Value, true)))
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "Form_Enter"));
            }
        }
        ...
        protected virtual void Form_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "Form_Closing"));
            }
        }
        ...
        protected virtual void Form_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "Form_FormClosed"));
            }
        }
                ...
                if (this._clienteID != this.masterCliente.Value)
                {
                    ...
                    if (Convert.ToInt32(this.lkp_TipoGarantia.EditValue) == (int)TipoGarantia.Real)
                    {
                        ...
                        if (creditos.Count == 1)
                        {
                            ...
                            #region Solicitud de credito existente
        ...
        private void masterPagaduria_Leave(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "masterPagaduria_Leave"));
            }
        ...
        private void txtLibranza_Leave(object sender, EventArgs e)
        {
            try
            {
                if (this._libranzaID != this.txtLibranza.Text.Trim() && !this.btnCambiarLibranza.Visible)
                {
                    this._isLoaded = false;
                    ...
                    if (solicitud.DocCtrl != null)
                    {
                        ...
                        if (this._ctrl.Estado.Value.Value == (int)EstadoDocControl.SinAprobar)
                        {
                            this._header.DevueltaInd.Value = false;
                            this.GetValues();
                            this.EnableHeader(true);
                        ...
                        else if (this._ctrl.Estado.Value.Value == (int)EstadoDocControl.Anulado)
                        {
                            //Mostrar mensaje de que esta libranza esta cerrada
                            if (MessageBox.Show("Esta solicitud se encuentra rechazada, desea reactivarla?", "Estado Solicitud", MessageBoxButtons.YesNo) == DialogResult.Yes)
                            {
                                this._header.DevueltaInd.Value = false;
                                this.GetValues();
                                this.EnableHeader(true);
                    ...
                    {
                        DTO_ccCreditoDocu credExist = this._bc.AdministrationModel.GetCreditoByLibranza(libranzaTemp);
                        if(credExist != null)
                        {
                            MessageBox.Show(this._bc.GetResource(LanguageTypes.Error, DictionaryMessages.Err_Cc_LibranzaLiquidada));
                            this.CleanData();
                            this.txtLibranza.Focus();
                        }
                        else
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "txtLibranza_Leave"));
            }
        }
        ...
        private void btnCambiarLibranza_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "btnCambiarLibranza_Click"));
            }
        }
        ...
        private void lkp_TipoGarantia_EditValueChanged(object sender, EventArgs e)
        {
            if (this._ctrl != null && ( this._ctrl.Estado.Value == (int)EstadoDocControl.Aprobado || 
                                        this._ctrl.Estado.Value == (int)EstadoDocControl.Anulado ||
        ...
        private void txtSaldoDeudor_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "txtSaldoDeudor_ButtonClick"));
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txtSaldoCod1_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            try
            {
                Dictionary<string, object> filter = new Dictionary<string, object>();
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "txtSaldoCod1_ButtonClick"));
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txtSaldoCod2_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            try
            {
                Dictionary<string, object> filter = new Dictionary<string, object>();
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "txtSaldoCod2_ButtonClick"));
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void masterCodeudor1_Leave(object sender, EventArgs e)
        {
            this.GetSaldosCedula(this.masterCodeudor1.Name);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void masterCodeudor2_Leave(object sender, EventArgs e)
        {
            this.GetSaldosCedula(this.masterCodeudor2.Name);
        }
        ...
        public override void TBNew()
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "TBNew"));
            }
        }
        ...
        public override void TBSave()
        {
            try
            {
                if (this.ValidateHeader())
                {
                    ...
                    List<DTO_glDocumentoChequeoLista> tareas = new List<DTO_glDocumentoChequeoLista>();
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-SolicitudLibranza.cs", "TBSave"));
            }
        }
