...
{
    public partial class MigracionEstadoCartera : ProcessForm
    {
        #region Delegados
        ...
        private string msgImportKey = string.Empty;

        //Variables del formulario
        private bool _isOK;
        private List<DTO_MigracionEstadoCartera> _creditos;
        ...
        /// CONSTRUCTOR
        /// </summary>
       // public MigracionEstadoCartera() { this.InitializeComponent(); }
        ...
        protected override void InitForm()
        {
            try
            {
                this.documentID = AppProcess.MigracionEstadoCartera;
                ...
                this.format = _bc.GetImportExportFormat(typeof(DTO_MigracionEstadoCartera), this.documentID);

                //Rsx
                this.msgImportKey = _bc.GetResource(LanguageTypes.Messages, DictionaryMessages.Cc_MigracionEstadoInvalidTuple);
            }
            catch (Exception ex)
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-MigracionEstadoCartera.cs", "InitForm"));
            }
        }
        ...
        private void ValidateDataImport(DTO_MigracionEstadoCartera credito, DTO_TxResultDetail rd, string msgComponentes)
        {
            string colRsx;
            DTO_TxResultDetailFields rdF;

            #region Libranza
            colRsx = _bc.GetResource(LanguageTypes.Forms, this.documentID.ToString() + "_Libranza");
            rdF = _bc.ValidGridCell(colRsx, credito.Libranza.Value.ToString(), false, false, false,null);
            if (rdF != null)
            {
            ...
            #endregion
            #region Fks
            #region CobranzaEstadoID
            colRsx = _bc.GetResource(LanguageTypes.Forms, this.documentID.ToString() + "_CobranzaEstadoID");
            rdF = _bc.ValidGridCell(colRsx, credito.CobranzaEstadoID.Value, true, true, false, AppMasters.ccCobranzaEstado);
            if (rdF != null)
            {
                rd.DetailsFields.Add(rdF);
            }
            #endregion
            #region CobranzaGestionID
            colRsx = _bc.GetResource(LanguageTypes.Forms, this.documentID.ToString() + "_CobranzaGestionID");
            rdF = _bc.ValidGridCell(colRsx, credito.CobranzaGestionID.Value, true, true, false, AppMasters.ccCobranzaGestion);
            if (rdF != null)
            {
                rd.DetailsFields.Add(rdF);
            }
            ...
            colRsx = _bc.GetResource(LanguageTypes.Forms, this.documentID.ToString() + "_NovedadIncorporaID");
            rdF = _bc.ValidGridCell(colRsx, credito.NovedadIncorporaID.Value, true, true, false, AppMasters.ccIncorporacionNovedad);
            if (rdF != null)
            {
                rd.DetailsFields.Add(rdF);
            }
            #endregion 
            #region SiniestroEstadoID
            colRsx = _bc.GetResource(LanguageTypes.Forms, this.documentID.ToString() + "_SiniestroEstadoID");
            rdF = _bc.ValidGridCell(colRsx, credito.SiniestroEstadoID.Value, true, true, false, AppMasters.ccSiniestroEstado);
            if (rdF != null)
            {
                rd.DetailsFields.Add(rdF);
            }
            #endregion 
            #endregion
            #region Valida la combinación de cobranza
            if(!string.IsNullOrWhiteSpace(credito.CobranzaEstadoID.Value) && string.IsNullOrWhiteSpace(credito.CobranzaGestionID.Value) ||
                string.IsNullOrWhiteSpace(credito.CobranzaEstadoID.Value) && !string.IsNullOrWhiteSpace(credito.CobranzaGestionID.Value))
            {
                rdF = new DTO_TxResultDetailFields();
                rdF.Field = _bc.GetResource(LanguageTypes.Forms, this.documentID.ToString() + "_CobranzaEstadoID") + "-" + _bc.GetResource(LanguageTypes.Forms, this.documentID.ToString() + "_CobranzaGestionID");
                rdF.Message = this.msgImportKey;
        ...
        private void btnTemplate_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-MigracionEstadoCartera.cs", "btnTemplate_Click"));
            }
        }
        ...
        /// <param name="sender">Objeto que envia el evento</param>
        /// <param name="e">Evento</param>
        private void btnGuardarData_Click(object sender, EventArgs e)
        {
            try
            {
                this.Enabled = false;
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-MigracionEstadoCartera.cs", "btnGenerarDocumentos_Click"));
            }
        }
        ...
        private void ImportThread()
        {
            try
            {
                if (this.pasteRet.Success)
                {
                    ...
                    //Popiedades de un credito
                    DTO_MigracionEstadoCartera credito = new DTO_MigracionEstadoCartera();
                    bool createDTO = true;
                    bool validList = true;
                    //Inicializacion de variables generales
                    this._creditos = new List<DTO_MigracionEstadoCartera>();
                    #endregion
                    ...
                    List<PropertyInfo> propInfo = typeof(DTO_MigracionEstadoCartera).GetProperties().ToList();
                    ...
                    for (int i = 0; i < lines.Length; ++i)
                    {
                        ...
                        if (i > 0 && line.Length > 0)
                        {
                            ...
                            if (createDTO)
                            {
                                credito = new DTO_MigracionEstadoCartera();
                                for (int colIndex = 0; colIndex < colNames.Count; colIndex++)
                                {
                                    ...
                                    catch (Exception ex1)
                                    {
                                        DTO_TxResultDetailFields rdF = new DTO_TxResultDetailFields();
                                        rdF.Field = colRsx;
                                        rdF.Message = _bc.GetResourceForException(ex1, "WinApp", "MigracionEstadoCartera.cs - Creacion de DTO y validacion Formatos");
                                        rd.DetailsFields.Add(rdF);
                    ...
                    if (validList)
                    {
                        ...
                        for (int index = 0; index < this._creditos.Count; ++index)
                        {
                            ...
                            this.ValidateDataImport(credito, rd, msgEmptyField);
                            if (rd.DetailsFields.Count > 0)
                            {
                                this.result.Details.Add(rd);
                                rd.Message = "NOK";
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-MigracionEstadoCartera.cs", "ImportThread"));
            }
            finally
            {
                this.Invoke(this.endImportarDelegate);
        ...
        private void ProcesarThread()
        {
            try
            {
                ...
                this.results = this._bc.AdministrationModel.MigracionEstadoCartera(this.documentID,this._creditos);
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-MigracionEstadoCartera.cs", "btnProcesar_Click"));
                this.StopProgressBarThread();
            }
            finally
            {
