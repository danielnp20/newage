    ...
    public partial class Solicitudes : FormWithToolbar
    {
       ...
       public byte _report = 1;
        ...
        private void SetInitParameters()
        {
            ...
            // carga combos
            Dictionary<string, string> tipoReporte = new Dictionary<string, string>();
            tipoReporte.Add("0", this._bc.GetResource(LanguageTypes.Tables, " "));
            tipoReporte.Add("1", this._bc.GetResource(LanguageTypes.Tables, "tbl_restr_TipoReporteSolicitud_V1"));
            tipoReporte.Add("2", this._bc.GetResource(LanguageTypes.Tables, "tbl_restr_TipoReporteSolicitud_V2"));
            
            this.cmbTipoReporte.Properties.ValueMember = "Key";
            this.cmbTipoReporte.Properties.DisplayMember = "Value";
            this.cmbTipoReporte.Properties.DataSource = tipoReporte;
            this.cmbTipoReporte.EditValue = "0";
        }
        ...
        private void btnAnalisis_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<string> actividades = _bc.AdministrationModel.glActividadFlujo_GetActividadesByDocumentID(AppDocuments.AnalisisRiesgo);
                List<DTO_MasterBasic> chequeos = _bc.AdministrationModel.glActividadChequeoLista_GetByActividad(actividades.Count > 0? actividades[0] : string.Empty);
                foreach (DTO_MasterBasic basic in chequeos)
                {
                    ...
                    chequeo.ActividadFlujoID.Value = basic.ID.Value;
                    chequeo.Descripcion.Value = basic.Descriptivo.Value;
                    chequeo.TerceroID.Value = doc.ClienteID.Value;
        ...
        public override void TBPrint()
        {
            try
            {
                this._report = Convert.ToByte(this.cmbTipoReporte.EditValue);

                string reportName;
                string fileURl;
                DateTime _fechaFin = DateTime.Now;

                reportName = this._bc.AdministrationModel.Report_Cc_CarteraByParameter(AppQueries.ConsultaSolicitud, this._report, null, _fechaFin, null,Convert.ToInt32(this.txtLibranza.EditValue), string.Empty, string.Empty,
                                                                                      string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, null, null, 1);
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-Solicitudes.cs", "TBPrint"));
            }
        }
