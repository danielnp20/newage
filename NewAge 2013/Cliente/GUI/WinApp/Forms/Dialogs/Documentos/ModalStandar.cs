...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    public partial class ModalStandar : Form
    {
        ...
        private void AddGridCols()
        {
            try
            {
               if(this._documentID == AppDocuments.PlaneacionRecursos)
               {
               ...
               else if (this._documentID == AppForms.EnvioCorreoClientes)
               {
                ...
                else if(this._documentID == AppDocuments.DigitacionCreditoFinanciera || this._documentID == AppDocuments.SolicitudLibranza)
                {
                    this.lblTitle.Text ="Saldos por " +this._bc.GetResource(LanguageTypes.Forms, "1013_Libranza") ;
                    #region Columnas
                    GridColumn Libranza = new GridColumn();
                    Libranza.FieldName = this.unboundPrefix + "Libranza";
                    Libranza.Caption = _bc.GetResource(LanguageTypes.Forms, "260_Obligacion");
                    Libranza.UnboundType = UnboundColumnType.String;
                    Libranza.VisibleIndex = 1;
                    Libranza.Width = 60;
                    Libranza.Visible = true;
                    ...
                    GridColumn ClienteID = new GridColumn();
                    ClienteID.FieldName = this.unboundPrefix + "ClienteID";
                    ClienteID.Caption = _bc.GetResource(LanguageTypes.Forms, "Cliente");
                    ClienteID.UnboundType = UnboundColumnType.String;
                    ...
                    GridColumn Nombre = new GridColumn();
                    Nombre.FieldName = this.unboundPrefix + "Nombre";
                    Nombre.Caption = _bc.GetResource(LanguageTypes.Forms, "Nombre");
                    Nombre.UnboundType = UnboundColumnType.String;
                    ...
                    GridColumn Otro = new GridColumn();
                    Otro.FieldName = this.unboundPrefix + "Otro";
                    Otro.Caption = _bc.GetResource(LanguageTypes.Forms, "Relación");
                    Otro.UnboundType = UnboundColumnType.String;
                    Otro.VisibleIndex = 4;
                    Otro.Width = 50;
                    Otro.Visible = true;
                    Otro.OptionsColumn.AllowEdit = false;
                    this.gvData.Columns.Add(Otro);

                    GridColumn VlrSaldo = new GridColumn();
                    VlrSaldo.FieldName = this.unboundPrefix + "VlrSaldo";
                    VlrSaldo.Caption = _bc.GetResource(LanguageTypes.Forms,AppDocuments.EstadoCuenta+ "_VlrSaldo");
                    ...
                    this.editCant2.Mask.EditMask = "c2";
                    VlrSaldo.ColumnEdit = this.editCant2;
                    this.gvData.Columns.Add(VlrSaldo);
                    this.gvData.OptionsBehavior.Editable = true;
                    this.gvData.OptionsView.ColumnAutoWidth = true;
                    #endregion
                }
        ...
        private void Loadata(Dictionary<string, object> filter)
        {
            try
            {
                if(this._documentID == AppDocuments.PlaneacionRecursos)
                {
                    ...
                    mvtosxReferencia = this._bc.AdministrationModel.inControlSaldosCostos_GetByParameter(this._documentID, filterInv);
                    ...
                    foreach (DTO_inControlSaldosCostos inv in mvtosxReferencia.FindAll(x=>x.BodegaTipo.Value == (byte)TipoBodega.Stock))
                    {
                        ...
                        var bodega = (DTO_inBodega)_bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.inBodega, false, inv.BodegaID.Value, true);
                        inv.BodegaDesc.Value = bodega.Descriptivo.Value;
                        inv.CantidadDisp.Value = this._bc.AdministrationModel.Transaccion_SaldoExistByReferencia(this._documentID, inv, ref costos);
                        listSaldosCostosGrid.Add(inv);
                ...
                {
                    string tipoPersona = filter.First().Key;
                    string cedula = filter.First().Value.ToString();
                    ...
                    {
                        DTO_ccCliente cli = (DTO_ccCliente)_bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.ccCliente, false, cred.ClienteID.Value, true);
                        ...
                        if (saldo != null)
                        {
                            cred.VlrSaldo.Value = saldo.PlanPagos.Sum(y => y.VlrSaldo.Value);
                            cred.Otro.Value = cred.ClienteID.Value == cedula ? "Deudor" : "Codeudor";
                        }
                        cred.Nombre.Value = cli.Descriptivo.Value;
                    }
                    this.gcData.DataSource = creditos;
                }
            }
            catch (Exception ex)
            {
