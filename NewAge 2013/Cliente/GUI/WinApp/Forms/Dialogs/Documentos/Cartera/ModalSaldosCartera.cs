...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    /// </summary>
    public partial class ModalSaldosCartera : Form
    {
        #region Variables
        ...
        public ModalSaldosCartera(DTO_InfoCredito infoCartera, bool isSaldo)
        {
        ...
        public ModalSaldosCartera(int numDoc, int libranza, DateTime fechaCorte, bool isSaldo)
        {
            //Inicializa el formulario
            InitializeComponent();
            ...
            if (isSaldo)
                infoCredito = this._bc.AdministrationModel.GetSaldoCredito(numDoc, fechaCorte, true, true, true);
            else
                infoCredito = this._bc.AdministrationModel.GetPagosCredito(numDoc, fechaCorte);
        ...
        private void AddGridPlanPagosCols()
        {
            try
            {
                ...
                if (this._isSaldo)
                {
                    ...
                    //SaldoCuota
                    GridColumn SaldoCuota = new GridColumn();
                    SaldoCuota.FieldName = this.unboundPrefix + "VlrCuota";
                    SaldoCuota.Caption = _bc.GetResource(LanguageTypes.Forms, this.documentID + "_VlrCuota");
                    SaldoCuota.UnboundType = UnboundColumnType.Integer;
                    SaldoCuota.VisibleIndex = 3;
                    SaldoCuota.Width = 200;
                    SaldoCuota.Visible = true;
                    SaldoCuota.OptionsColumn.AllowEdit = false;
                    this.gvPlanPagos.Columns.Add(SaldoCuota);
        ...
        private void LoadGridPlanPagos(DTO_InfoCredito infoCartera)
        {
            try
            {
                ...
                foreach (var item in infoCartera.PlanPagos)
                {
                    item.VlrMoraLiquida.Value = this.componentes.FindAll(x => x.CuotaID.Value == item.CuotaID.Value &&
                                                                         (x.TipoComponente.Value != (byte)TipoComponente.CapitalSolicitado &&
                                                                          x.TipoComponente.Value != (byte)TipoComponente.ComponenteCuota)).Sum(x => x.CuotaSaldo.Value);
