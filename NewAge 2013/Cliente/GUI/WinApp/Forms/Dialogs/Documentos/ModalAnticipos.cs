...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    public partial class ModalAnticipos : Form
    {
        ...
        private void LoadGridData()
        {
            try
            {
                List<DTO_AnticiposResumen> anticipos = _bc.AdministrationModel.cpAnticipos_GetResumen(this._periodo, this._tipoMoneda, this._tercero);
                List<DTO_AnticiposResumen> newData = new List<DTO_AnticiposResumen>();

                //Carga la informacion de los anticipos de acuerdo con el tipo de moneda
                anticipos.ForEach(newAnt =>
                {
                    #region Calcula el maximo valor
                    newAnt.MaxVal.Value = this._tipoMoneda == TipoMoneda.Local ? newAnt.ML.Value : newAnt.ME.Value;
                    #endregion
                    #region Revisa los anticipos que ya fueron asignados
                    this._currentData.ForEach(ant =>
                    {
                        bool finded = false;
                        if
                        (
                            !finded &&
                            ant.CuentaID.Value == newAnt.CuentaID.Value &&
                            ant.TerceroID.Value == newAnt.TerceroID.Value &&
                            ant.ProyectoID.Value == newAnt.ProyectoID.Value &&
                            ant.CentroCostoID.Value == newAnt.CentroCostoID.Value &&
                            ant.LineaPresupuestoID.Value == newAnt.LineaPresupuestoID.Value &&
                            ant.ConceptoSaldoID.Value == newAnt.ConceptoSaldoID.Value &&
                            ant.ConceptoCargoID.Value == newAnt.ConceptoCargoID.Value &&
                            ant.IdentificadorTR.Value == newAnt.IdentificadorTR.Value
                        )
                        {
                            finded = true;
                            newAnt.Seleccionar.Value = true;
                        }
                    });
                    #endregion
                    newData.Add(newAnt);
