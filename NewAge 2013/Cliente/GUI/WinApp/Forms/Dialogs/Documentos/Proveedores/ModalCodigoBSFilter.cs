...
{
    public partial class ModalCodigoBSFilter : Form
    {
        #region Variables
        ...
        //Variables de data
        private DTO_prBienServicio _rowCurrent = new DTO_prBienServicio();
        private List<DTO_prBienServicio> _listCurrent = null;
       ...
       /// <param name="bodegaIni">Bodega para consultar las existencias</param>
        public ModalCodigoBSFilter(int document)
        {
            ...
            try
            {
                ...
                this.Text = this._bc.GetResource(LanguageTypes.Forms, this._documentID + "_frmCodigoBS");
            }
            catch (Exception ex)
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinAppModalCodigoBSFilter.cs", "ModalCodigoBSFilter: " + ex.Message));
            }
        }
        ...
        private void AddGridCols()
        {
            ...
            ID.Caption = _bc.GetResource(LanguageTypes.Forms, this._documentID + "_CodigoBSID");
            ...
            Descriptivo.Caption = _bc.GetResource(LanguageTypes.Forms, this._documentID + "_CodigoBSDesc");
        ...
        private void LoadGridData()
        {
            ...
            {
                long count = 0;
                this._listCurrent = new List<DTO_prBienServicio>();
                ...
                count = this._bc.AdministrationModel.MasterSimple_Count(AppMasters.prBienServicio, consulta, null, true);
                List<DTO_prBienServicio> _dtoListCodigoBS = this._bc.AdministrationModel.MasterHierarchy_GetPaged(AppMasters.prBienServicio, count, 1, consulta, null, null).Cast<DTO_prBienServicio>().ToList();

                #region Filtra la referencia
                ...
                if (filtros.Count > 0)
                {
                    ...
                    List<DTO_inReferencia> _dtoListReferen = this._bc.AdministrationModel.MasterSimple_GetPaged(AppMasters.inReferencia, count, 1, consulta, null, null).Cast<DTO_inReferencia>().ToList();

                    foreach (var bs in _dtoListCodigoBS)
                    {
                        if (_dtoListReferen.Exists(x => x.ID.Value == bs.ID.Value))
                            this._listCurrent.Add(bs);
                    }
                }
                else
                    this._listCurrent = _dtoListCodigoBS;
                #endregion
                this.pgGrid.UpdatePageNumber(this._listCurrent.Count, true, true, false);
                var tmp = this._listCurrent.Skip((this.pgGrid.PageNumber - 1) * this._pageSize).Take(this._pageSize).ToList<DTO_prBienServicio>();
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ModalCodigoBSFilter.cs", "ModalCodigoBSFilter(" + this._documentID + ")-LoadGridData: " + ex.Message));
            }
        }
        ...
        private void pagging_Click(object sender, System.EventArgs e)
        {
            try
            {              
                var tmp = this._filterActive ?  this._listCurrent.Skip((this.pgGrid.PageNumber - 1) * this._pageSize).Take(this._pageSize).ToList<DTO_prBienServicio>() :
                                                this._listCurrent.Skip((this.pgGrid.PageNumber - 1) * this._pageSize).Take(this._pageSize).ToList<DTO_prBienServicio>(); ;
                this.pgGrid.UpdatePageNumber(this._listCurrent.Count, false, false, false);
                this.gvData.MoveFirst();
                this.gcData.DataSource = tmp;

                if (this.gvData.DataRowCount > 0)
                    this._rowCurrent = (DTO_prBienServicio)this.gvData.GetRow(this.gvData.FocusedRowHandle);
                else
                    this._rowCurrent = new DTO_prBienServicio();

            }
            catch (Exception ex)
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ModalCodigoBSFilter.cs", "pagging_Click: " + ex.Message)); 
            }
        }
        ...
        private void gvTarea_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            try
            {
                if (e.FocusedRowHandle >= 0)
                    this._rowCurrent = (DTO_prBienServicio)this.gvData.GetRow(e.FocusedRowHandle);
            }
            catch (Exception ex)
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ModalCodigoBSFilter.cs", "gvTarea_FocusedRowChanged: " + ex.Message));
            }
        }
        ...
        private void gvTarea_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            ...
            {
                if (e.RowHandle >= 0)
                    this._rowCurrent = (DTO_prBienServicio)this.gvData.GetRow(e.RowHandle);               
            }
            catch (Exception ex)
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ModalCodigoBSFilter.cs", "gvTarea_RowClick: " + ex.Message));
            }
        }
