...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    public partial class ModalTrasladoInv : Form
    {
        ...
        private string _proyectoIDOrigen = string.Empty;
        private string _proyectoIDDestino = string.Empty;
        ...
        List<DTO_glMovimientoDeta> _mvtosPrestamos = new List<DTO_glMovimientoDeta>();
        List<DTO_glMovimientoDeta> _mvtosDevoluciones = new List<DTO_glMovimientoDeta>();
        DTO_SolicitudTrabajo _mvtosProyectoOrig = new DTO_SolicitudTrabajo();
        private string _empaqueInvIdDef = string.Empty;
        private decimal _tasaCambio = 0;
        private byte _tipoTrasladoSelect = 0;
        #endregion
        ...
        /// </summary>
        public byte TipoTrasladoSelect
        {
            get { return _tipoTrasladoSelect; }
        }
        ...
        public ModalTrasladoInv(List<DTO_inMovimientoFooter> detalleExist, string bodegaIni,string bodegaFin, decimal tasaCambio)
        {
            this.InitializeComponent();
            try
            {
                this._documentID = AppForms.ModalSolicitudesForm;
                DTO_inBodega bodIni = (DTO_inBodega)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.inBodega, false, bodegaIni, true);
                DTO_inBodega bodFin = (DTO_inBodega)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.inBodega, false, bodegaFin, true);
                this._bodegaIni = bodegaIni;
                this._bodegaFin = bodegaFin;             
                this._tasaCambio = tasaCambio;
                this._detalleInvExist = detalleExist;
                this._proyectoIDOrigen = bodIni!= null? bodIni.ProyectoID.Value : string.Empty;
                this._proyectoIDDestino = bodFin != null ? bodFin.ProyectoID.Value : string.Empty;
        ...
        private void SetInitParameters()
        {
            ...
            this._bc.InitMasterUC(this.masterProyectoOrigen, AppMasters.coProyecto, true, true, false, false);
            this._bc.InitMasterUC(this.masterProyectoDestino, AppMasters.coProyecto, true, true, false, false);
            ...
            this.masterProyectoOrigen.Value = this._proyectoIDOrigen;
            this.masterProyectoDestino.Value = this._proyectoIDDestino;

            Dictionary<string, string> datosTipo = new Dictionary<string, string>();
            datosTipo.Add("10", "Seleccione...");
            datosTipo.Add("0", "Traslado Stock");
            datosTipo.Add("1", "Traslado por Consumo Interno ");
            datosTipo.Add("2", "Traslado por Prestamo");
            datosTipo.Add("3", "Traslado por Devolución Prestamo");
            datosTipo.Add("4", "Salida Stock");
            datosTipo.Add("5", "Salida por Ventas");
            this.cmbTipoTraslado.Properties.ValueMember = "Key";
            this.cmbTipoTraslado.Properties.DisplayMember = "Value";
            this.cmbTipoTraslado.Properties.DataSource = datosTipo;
            this.cmbTipoTraslado.EditValue = "10";
            #endregion
        ...
        private void LoadExistencias()
        {
            try
            {
                //Trae el proyecto de stock para validar el consumo interno corresponda 
                string proyxDefectoStock = this._bc.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_ProyectoXDefecto);
                if (!string.IsNullOrEmpty(this._bodegaIni))
                {
                    ...
                    saldos.BodegaID.Value = this._bodegaIni;
                    int index = 0;
                    listSaldosCostos = _bc.AdministrationModel.inControlSaldosCostos_GetByParameter(this._documentID, saldos);
                    listSaldosCostos = listSaldosCostos.FindAll(x => x.CantidadDisp.Value != 0).ToList();
                    foreach (DTO_inControlSaldosCostos inv in listSaldosCostos)
                    {
                        DTO_inMovimientoFooter f;
                        DTO_inReferencia refer ;

                        //Traslado Normal
                        if (this._tipoTrasladoSelect == (byte)TipoTraslado.Traslado || this._tipoTrasladoSelect == 4)
                        {
                            #region Asigna datos Normal
                            f = new DTO_inMovimientoFooter();
                            refer = (DTO_inReferencia)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.inReferencia, false, inv.inReferenciaID.Value, true);

                            if (this._detalleInvExist.Count > 0 && this._detalleInvExist.Exists(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value && x.DetalleSolicitud.Count == 0))
                            {
                                f = this._detalleInvExist.Find(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value && x.DetalleSolicitud.Count == 0);
                                f.SelectInd.Value = true;
                            }
                            else
                            {
                                f.SelectInd.Value = false;                                
                                f.Movimiento.EmpresaID.Value = empresa;
                                f.Movimiento.RefProveedor.Value = refer.RefProveedor.Value;
                                f.Movimiento.MarcaDesc.Value = refer.MarcaInvDesc.Value;
                                f.UnidadRef.Value = refer.UnidadInvID.Value;
                                f.Movimiento.DescripTExt.Value = refer.Descriptivo.Value;
                                f.Movimiento.inReferenciaID.Value = inv.inReferenciaID.Value;
                                f.ReferenciaIDP1P2.Value = inv.ReferenciaP1P2ID.Value;
                                f.ReferenciaIDP1P2Desc.Value = inv.ReferenciaP1P2Desc.Value;
                                f.Movimiento.Parametro1.Value = parametro1;
                                f.Movimiento.Parametro2.Value = parametro2;
                                f.Movimiento.BodegaID.Value = this._bodegaIni;
                                f.Movimiento.EstadoInv.Value = (byte)EstadoInv.Activo;
                                f.Movimiento.ActivoID.Value = 0;
                                f.Movimiento.IdentificadorTr.Value = 0;
                                f.Movimiento.SerialID.Value = string.Empty;
                                f.Movimiento.EmpaqueInvID.Value = string.Empty;
                                f.Movimiento.CantidadDoc.Value = 0;
                                f.Movimiento.CantidadEMP.Value = 0;
                                f.Movimiento.CantidadUNI.Value = 0;
                                f.CantidadPendiente.Value = 0;
                                f.CantidadDispon.Value = inv.CantidadDisp.Value;
                                f.Movimiento.CantidadEMP.Value = 0;
                                f.Movimiento.EntradaSalida.Value = (byte)EntradaSalida.Salida;
                                f.Movimiento.ValorUNI.Value = 0;
                                f.Movimiento.DocSoporte.Value = 0;
                                f.Movimiento.DocSoporteTER.Value = string.Empty;
                                f.Movimiento.Valor1LOC.Value = 0;
                                f.Movimiento.Valor2LOC.Value = 0;
                                f.Movimiento.Valor1EXT.Value = 0;
                                f.Movimiento.Valor2EXT.Value = 0;                                                       
                            }
                            f.Movimiento.Index = index;
                            this._detalleInv.Add(f);       
                            index++;
                            #endregion
                        }
                        //Traslado Consumo Interno - traslado entre bodegas(Solicitud Proveedores)
                        else if (this._tipoTrasladoSelect == (byte)TipoTraslado.ConsumoInterno && this._solicitudesProv.Exists(x => x.inReferenciaID.Value == inv.inReferenciaID.Value) && _proyectoIDOrigen.Equals(proyxDefectoStock))
                        {
                            foreach (DTO_prSolicitudResumen sol in this._solicitudesProv.FindAll(x => x.inReferenciaID.Value == inv.inReferenciaID.Value))
                            {
                                #region Asigna datos de Solicitud
                                f = new DTO_inMovimientoFooter();
                                refer = (DTO_inReferencia)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.inReferencia, false, inv.inReferenciaID.Value, true);
                                if (this._detalleInvExist.Count > 0 && this._detalleInvExist.Exists(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value && x.DetalleSolicitud.Exists(y => y.ConsecutivoDetaID.Value == sol.ConsecutivoDetaID.Value)))
                                {
                                    f = this._detalleInvExist.Find(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value && x.DetalleSolicitud.Exists(y => y.ConsecutivoDetaID.Value == sol.ConsecutivoDetaID.Value));
                                    f.SelectInd.Value = true;
                                }
                                else
                                {
                                    ...
                                    f.Movimiento.CantidadUNI.Value = 0;
                                    f.CantidadDispon.Value = inv.CantidadDisp.Value;
                                    f.CantidadPendiente.Value = sol.CantidadSol.Value;
                                    f.Movimiento.CantidadEMP.Value = 0;
                                    f.Movimiento.EntradaSalida.Value = (byte)EntradaSalida.Salida;
                                    f.Movimiento.ValorUNI.Value = 0;
                                    f.Movimiento.DocSoporte.Value = sol.Detalle4ID.Value;
                                    f.Movimiento.DocSoporteTER.Value = string.Empty;
                                    f.Movimiento.Valor1LOC.Value = 0;
                                    f.Movimiento.Valor2LOC.Value = 0;
                                    f.Movimiento.Valor1EXT.Value = 0;
                                    f.Movimiento.Valor2EXT.Value = 0;
                                    // Datos Solicitud
                                    f.PrefDoc = sol.PrefDoc;
                                    f.Movimiento.ProyectoID.Value = sol.ProyectoID.Value;
                                    f.Movimiento.CodigoBSID.Value = sol.CodigoBSID.Value;
                                    f.Movimiento.LineaPresupuestoID.Value = sol.LineaPresupuestoID.Value;
                                    f.FechaSol.Value = sol.Fecha.Value;
                                    f.Movimiento.DatoAdd4.Value = sol.Documento4ID.Value.ToString(); //Permite filtrar en reporte de costos de Proyectos
                                    f.Movimiento.DatoAdd5.Value = sol.DatoAdd4.Value.ToString();
                                    f.FechaSol.Value = sol.Fecha.Value;
                                    f.OrigenMonetario.Value = sol.OrigenMonetario.Value;
                                    f.DetalleSolicitud.Add(sol);
                                }
                                f.Movimiento.Index = index;
                                this._detalleInv.Add(f);
                                index++;
                        ...
                        //Traslado Prestamo Proyecto - tralado entre bodegas de Proyecto
                        else if (this._tipoTrasladoSelect == (byte)TipoTraslado.PrestamoProyecto && this._solicitudesProv.Exists(x => x.inReferenciaID.Value == inv.inReferenciaID.Value))
                        {
                            foreach (DTO_pyProyectoMvto mvtoProy in this._mvtosProyectoOrig.Movimientos.FindAll(x => x.RecursoID.Value == inv.inReferenciaID.Value))
                            {
                                #region Asigna datos de Solicitud
                                f = new DTO_inMovimientoFooter();
                                refer = (DTO_inReferencia)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.inReferencia, false, inv.inReferenciaID.Value, true);
                                ...
                                else
                                {
                                    ...
                                    f.Movimiento.CantidadUNI.Value = 0;
                                    f.CantidadDispon.Value = inv.CantidadDisp.Value;
                                    f.CantidadPendiente.Value = this._solicitudesProv.FindAll(x=>x.ProyectoID.Value == this._proyectoIDDestino && x.inReferenciaID.Value == inv.inReferenciaID.Value).Sum(x=>x.CantidadSol.Value);
                                    f.Movimiento.CantidadEMP.Value = 0;
                                    f.Movimiento.EntradaSalida.Value = (byte)EntradaSalida.Salida;
                                    f.Movimiento.ValorUNI.Value = 0;
                                    f.Movimiento.DocSoporte.Value = mvtoProy.Consecutivo.Value;
                                    f.Movimiento.DocSoporteTER.Value = string.Empty;
                                    f.Movimiento.Valor1LOC.Value = 0;
                                    f.Movimiento.Valor2LOC.Value = 0;
                                    f.Movimiento.Valor1EXT.Value = 0;
                                    f.Movimiento.Valor2EXT.Value = 0;
                                    // Datos Mvtos
                                    f.PrefDoc = this._mvtosProyectoOrig.DocCtrl.PrefDoc.Value;
                                    f.Movimiento.ProyectoID.Value = this._mvtosProyectoOrig.DocCtrl.ProyectoID.Value;
                                    f.Movimiento.CodigoBSID.Value = mvtoProy.CodigoBSID.Value;
                                    f.Movimiento.LineaPresupuestoID.Value = mvtoProy.LineaPresupuestoID.Value;
                                    f.FechaSol.Value = this._mvtosProyectoOrig.DocCtrl.FechaDoc.Value;
                                    f.Movimiento.DatoAdd4.Value = mvtoProy.NumeroDoc.Value.ToString(); //Permite filtrar en reporte de costos de Proyectos
                                    f.Movimiento.DatoAdd5.Value = mvtoProy.TareaCliente.Value.ToString();
                                    f.OrigenMonetario.Value = 1;
                                }
                                f.Movimiento.Index = index;
                                this._detalleInv.Add(f);
                                index++;
                            ...
                            //this._detalleInv = this._detalleInv.FindAll(x=>x.Movimiento.ProyectoID.Value == this._proyectoIDOrigen);
                        }
                        //Traslado Prestamo Proyecto - tralado entre bodegas de Proyecto
                        else if (this._tipoTrasladoSelect == (byte)TipoTraslado.DevolucionProyecto && this._mvtosPrestamos.Exists(x => x.inReferenciaID.Value == inv.inReferenciaID.Value))
                        {
                            foreach (DTO_glMovimientoDeta mvto in this._mvtosPrestamos.FindAll(x => x.inReferenciaID.Value == inv.inReferenciaID.Value))
                            {
                                #region Asigna datos de Mvtos
                                f = new DTO_inMovimientoFooter();
                                refer = (DTO_inReferencia)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.inReferencia, false, inv.inReferenciaID.Value, true);
                                if (this._detalleInvExist.Count > 0 && this._detalleInvExist.Exists(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value))
                                {
                                    f = this._detalleInvExist.Find(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value && x.DetalleSolicitud.Count == 0);
                                    f.SelectInd.Value = true;
                                }
                                ...
                                {
                                    f.Movimiento = ObjectCopier.Clone(mvto);
                                    f.Movimiento.ConsecutivoPrestamo.Value = f.Movimiento.Consecutivo.Value;
                                    f.Movimiento.Consecutivo = null;
                                    f.SelectInd.Value = false;
                                    f.Movimiento.EmpresaID.Value = empresa;
                                    f.Movimiento.RefProveedor.Value = refer.RefProveedor.Value;
                                    f.Movimiento.MarcaDesc.Value = refer.MarcaInvDesc.Value;
                                    ...
                                    f.ReferenciaIDP1P2Desc.Value = inv.ReferenciaP1P2Desc.Value;
                                    f.CantidadDispon.Value = inv.CantidadDisp.Value;
                                    f.CantidadPendiente.Value = this._mvtosPrestamos.FindAll(x => x.inReferenciaID.Value == inv.inReferenciaID.Value).Sum(x=>x.CantidadEMP.Value) - this._mvtosDevoluciones.FindAll(y => y.inReferenciaID.Value == inv.inReferenciaID.Value).Sum(y => y.CantidadEMP.Value);
                                    f.Movimiento.CantidadDoc.Value = 0;
                                    f.Movimiento.CantidadEMP.Value = 0;
                                    f.Movimiento.CantidadUNI.Value = 0;
                                    f.Movimiento.EntradaSalida.Value = (byte)EntradaSalida.Salida;
                                    f.Movimiento.EntradaSalidaLetras.Value = "S";
                                }
                                f.Movimiento.Index = index;
                                if (!this._detalleInv.Exists(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value))
                                {
                                    this._detalleInv.Add(f);
                                    index++;
                                }                               
                        ...
                        //Salida de Facturas de ventas
                        else if (this._tipoTrasladoSelect == 5 && this._detSaldosProyectoFacVenta.Exists(x => x.inReferenciaID.Value == inv.inReferenciaID.Value))
                        {
                            foreach (DTO_glMovimientoDeta mvto in this._detSaldosProyectoFacVenta.FindAll(x => x.inReferenciaID.Value == inv.inReferenciaID.Value))
                            {
                                #region Asigna datos de Mvtos
                                f = new DTO_inMovimientoFooter();
                                refer = (DTO_inReferencia)this._bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.inReferencia, false, inv.inReferenciaID.Value, true);
                                if (this._detalleInvExist.Count > 0 && this._detalleInvExist.Exists(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value))
                                {
                                    //f = this._detalleInvExist.Find(x => x.Movimiento.inReferenciaID.Value == inv.inReferenciaID.Value && x.DetalleSolicitud.Exists(y => y.ConsecutivoDetaID.Value == mvto.ConsecutivoDetaID.Value));
                                    //f.SelectInd.Value = true;
                                }
                                else
                                {
                                    ...
                                    f.Movimiento.CantidadUNI.Value = 0;
                                    f.CantidadDispon.Value = inv.CantidadDisp.Value;
                                    f.CantidadPendiente.Value = mvto.CantidadUNI.Value;
                                    f.Movimiento.CantidadEMP.Value = 0;
                                    f.Movimiento.EntradaSalida.Value = (byte)EntradaSalida.Salida;
                                    f.Movimiento.ValorUNI.Value = 0;
                                    f.Movimiento.DocSoporte.Value = mvto.DocSoporte.Value;
                                    f.Movimiento.DocSoporteTER.Value = string.Empty;
                                    f.Movimiento.Valor1LOC.Value = 0;
                                    f.Movimiento.Valor2LOC.Value = 0;
                                    f.Movimiento.Valor1EXT.Value = 0;
                                    f.Movimiento.Valor2EXT.Value = 0;
                                    f.Movimiento.ProyectoID.Value = mvto.ProyectoID.Value;
                                    f.Movimiento.LineaPresupuestoID.Value = mvto.LineaPresupuestoID.Value;
                                    f.Movimiento.DatoAdd4.Value = mvto.DatoAdd4.Value.ToString(); //Permite filtrar en reporte de costos de Proyectos
                                    //f.Movimiento.DatoAdd5.Value = mvto.DatoAdd4.Value.ToString();
                                }
                                f.Movimiento.Index = index;
                                this._detalleInv.Add(f);
                                index++;
        ...
        {
            this._solicitudesProv = _bc.AdministrationModel.Solicitud_GetResumen(AppDocuments.OrdenCompra, this._bc.AdministrationModel.User, ModulesPrefix.pr, TipoMoneda.Local);
            this._solicitudesProv = this._solicitudesProv.OrderByDescending(x => x.ConsecutivoDetaID.Value).ToList();
        }
        ...
        private void LoadDetalleFacturaVenta()
        {
            try
            {
                ...
                this._detSaldosProyectoFacVenta = this._bc.AdministrationModel.glMovimientoDetaPRE_GetSaldosInvByProyecto(periodoInv, this._proyectoIDOrigen, false);
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ModalTraslado.cs", "LoadDetalleFacturaVenta"));
            }

        }

        /// <summary>
        /// Carga los movimientos de inventarios prestados por proyecto
        /// </summary>
        private void LoadMvtosForDevolucion()
        {
            try
            {
                //Obtiene los items por proyecto
                DTO_glMovimientoDeta filter = new DTO_glMovimientoDeta();
                filter.ProyectoID.Value = this._proyectoIDOrigen;
                this._mvtosPrestamos = this._bc.AdministrationModel.glMovimientoDeta_GetByParameter(filter, false);
                //Filtra los mvtos de inventarios y aprobados
                this._mvtosPrestamos = this._mvtosPrestamos.FindAll(x => (x.DocumentoID.Value == AppDocuments.TransaccionManual || x.DocumentoID.Value == AppDocuments.TransaccionAutomatica) && x.EstadoDocCtrl.Value == (Int16)EstadoDocControl.Aprobado);

                //Obtiene las devoluciones realizadas en salidas
                this._mvtosDevoluciones = this._mvtosPrestamos.FindAll(x => x.TipoTrasladoInv.Value == (byte)TipoTraslado.DevolucionProyecto && x.EntradaSalida.Value == (byte)EntradaSalida.Salida);

                //Obtiene los prestamos realizados por entradas
                this._mvtosPrestamos = this._mvtosPrestamos.FindAll(x => x.TipoTrasladoInv.Value == (byte)TipoTraslado.PrestamoProyecto && x.EntradaSalida.Value == (byte)EntradaSalida.Entrada);

            }
            catch (Exception ex)
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ModalTraslado.cs", "LoadMvtosForDevolucion"));
            }
        }

        /// <summary>
        /// Carga los movimientos del proyecto para prestamos
        /// </summary>
        private void LoadMvtosProyecto()
        {
            try
            {
                //Obtiene los items por proyecto
                DTO_glMovimientoDeta filter = new DTO_glMovimientoDeta();
                filter.ProyectoID.Value = this._proyectoIDOrigen;
               this._mvtosProyectoOrig = this._bc.AdministrationModel.SolicitudProyecto_Load(AppDocuments.Proyecto,string.Empty,null,null,string.Empty,this._proyectoIDOrigen,false,true,false,false,false);
                //Filtra los mvtos de inventarios y aprobados
                this._mvtosPrestamos = this._mvtosPrestamos.FindAll(x => (x.DocumentoID.Value == AppDocuments.TransaccionManual || x.DocumentoID.Value == AppDocuments.TransaccionAutomatica) && x.EstadoDocCtrl.Value == (Int16)EstadoDocControl.Aprobado);
        ...
        private void chkSelect_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < this.gvDetalle.DataRowCount; i++)
                {
                    ...
                    {
                        if (this._tipoTrasladoSelect == 0 || this._tipoTrasladoSelect == 4) //Traslado o Salida Stock
                            d.Movimiento.CantidadEMP.Value = d.CantidadDispon.Value;
                        else
                        {
        ...
        /// <summary>
        /// Permite seleccionar el tipo de Traslado o Salida
        /// </summary>
        /// <param name="sender">Objeto de seleccion</param>
        /// <param name="e">Evento</param>
        private void cmbTipoTraslado_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                string proyxDefectoStock = this._bc.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_ProyectoXDefecto);

                this._tipoTrasladoSelect = Convert.ToByte(this.cmbTipoTraslado.EditValue);
                if (this._tipoTrasladoSelect == (byte)TipoTraslado.Traslado || this._tipoTrasladoSelect == 4) //Traslado o Salida Stock
                {
                    this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].VisibleIndex = 1;
                    this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].Visible = false;
                    this.gvDetalle.Columns[this._unboundPrefix + "ProyectoID"].VisibleIndex = 1;
                    ...
                    //Si los proyectos de traslado son diferentes Valida que la Bodega Destino sea Stock
                    if ((this._tipoTrasladoSelect == (byte)TipoTraslado.Traslado && this._proyectoIDOrigen != this._proyectoIDDestino))
                    {
                        //if (this._proyectoIDDestino != proyxDefectoStock)
                        //{
                        //    MessageBox.Show(this._bc.GetResource(LanguageTypes.Messages, "El bodega destino debe ser de stock, verifique"));
                        //    this.cmbTipoTraslado.EditValue = "10";//OPcion por defecto
                        //}
                    }
                    else 
                    {
                        //if (this._proyectoIDOrigen != proyxDefectoStock)
                        //{
                        //    MessageBox.Show(this._bc.GetResource(LanguageTypes.Messages, "El bodega destino debe ser de stock, verifique"));
                        //    this.cmbTipoTraslado.EditValue = "10";//OPcion por defecto
                        //}
                ...
                else if (this._tipoTrasladoSelect == (byte)TipoTraslado.ConsumoInterno || this._tipoTrasladoSelect == (byte)TipoTraslado.PrestamoProyecto)
                {
                    this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].VisibleIndex = 1;
                    this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].Visible = true;
                    this.gvDetalle.Columns[this._unboundPrefix + "ProyectoID"].VisibleIndex = 2;
                    ...
                    this.gvDetalle.Columns[this._unboundPrefix + "CantidadPendiente"].VisibleIndex = 12;
                    this.gvDetalle.Columns[this._unboundPrefix + "CantidadPendiente"].Visible = true;
                    if (this._tipoTrasladoSelect == (byte)TipoTraslado.PrestamoProyecto)
                    {
                        this.LoadMvtosProyecto();
                        this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].Caption = "Doc Proy";
                    }
                    this.LoadSolicitudesProv();
                    this.LoadExistencias();
                    this.masterProyectoOrigen.Visible = true;
                    this.masterProyectoDestino.Visible = true;

                    //Si es consumo interno valida que la bod Origen sea Stock y la  bod destino sea Proyecto
                    //if ((this._tipoTrasladoSelect == (byte)TipoTraslado.ConsumoInterno && (this._proyectoIDOrigen != proyxDefectoStock || this._proyectoIDDestino == proyxDefectoStock)))
                    //{
                    //        MessageBox.Show(this._bc.GetResource(LanguageTypes.Messages, DictionaryMessages.In_TipoTrasladoConsumoInt));
                    //        this.cmbTipoTraslado.EditValue = "10";//OPcion por defecto
                    //}
                }
                else if (this._tipoTrasladoSelect == (byte)TipoTraslado.DevolucionProyecto)
                {
                    this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].VisibleIndex = 1;
                    this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].Visible = false;
                    this.gvDetalle.Columns[this._unboundPrefix + "ProyectoID"].VisibleIndex = 2;
                    ...
                    this.gvDetalle.Columns[this._unboundPrefix + "CantidadPendiente"].VisibleIndex = 12;
                    this.gvDetalle.Columns[this._unboundPrefix + "CantidadPendiente"].Visible = true;
                    this.LoadMvtosForDevolucion();
                    this.LoadExistencias();
                    this.masterProyectoOrigen.Visible = true;
                    this.masterProyectoDestino.Visible = true;
                }
                else if (this._tipoTrasladoSelect == 5)//Salida Ventas
                {
                    this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].VisibleIndex = 1;
                    this.gvDetalle.Columns[this._unboundPrefix + "PrefDoc"].Visible = false;
                    this.gvDetalle.Columns[this._unboundPrefix + "ProyectoID"].VisibleIndex = 2;
                ...
                else if (this._tipoTrasladoSelect == 10)//Opcion por defecto ...Seleccione
                {
                    this.LoadExistencias();
                    this.masterProyectoOrigen.Visible = false;
                    this.masterProyectoDestino.Visible = false;
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ModalTrasladoInv.cs", "cmbTipoTraslado_EditValueChanged"));
            }
            ...
            if (this._rowCurrent != null)
            {
                ...
                if (fieldName == "SelectInd")
                {
                    if (Convert.ToBoolean(e.Value))
                    {
                        ...
                        else  //Traslado Consumo Interno o Salida facturas
