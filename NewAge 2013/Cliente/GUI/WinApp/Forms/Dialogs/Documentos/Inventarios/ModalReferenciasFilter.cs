        ...
        private void LoadGridData()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(this._bodegaSalida))
                {
                    ...
                    List<DTO_inReferencia> listRefSelected = new List<DTO_inReferencia>();
                    ...
                    saldos.BodegaID.Value = this._bodegaSalida;
                    ...
                    {
                        DTO_inReferencia refer = _dtoList.Where(x => x.ID.Value.Equals(inv.inReferenciaID.Value)).FirstOrDefault();
                        if (refer != null)
                        {
                            saldos.inReferenciaID.Value = inv.inReferenciaID.Value;
                            refer.SaldoExistencia.Value = inv.CantidadDisp.Value;
                            refer.CostosExistencia = inv.CostosExistencia;
                            bool refExist = listRefSelected.Exists(x => x.ID.Value.Equals(inv.inReferenciaID.Value));
                            if (!refExist && inv.CantidadDisp.Value != 0)
                                listRefSelected.Add(refer);
                        }                       
                    }                   
                    #endregion 
                    _dtoList = listRefSelected;
                }
