...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    public partial class ModalConsultaInventario : Form
    {
        ...
        private int _documentID = AppQueries.QueryReferencia;
        ...
        private void AddGridColsMvto(bool consultaUserInd)
        {
            try
            {
                ...
                DocumentoID.ColumnEdit = this.editLink;
                this.gvData.Columns.Add(DocumentoID);
                ...
                //TipoTraslado
                GridColumn TipoTraslado = new GridColumn();
                TipoTraslado.FieldName = this._unboundPrefix + "DescriptivoTarea";
                TipoTraslado.Caption = _bc.GetResource(LanguageTypes.Forms, this._documentID + "_TipoTraslado");
                TipoTraslado.UnboundType = UnboundColumnType.String;
                TipoTraslado.VisibleIndex = 5;
                TipoTraslado.Width = 70;
                TipoTraslado.Visible = true;
                TipoTraslado.OptionsColumn.AllowEdit = false;
                this.gvData.Columns.Add(TipoTraslado);
        ...
        private void LoadGridData(bool isDetalle, string referencia, string BodegaID, bool consultaUserInd)
        {
            try
            {
                ...
                if (isDetalle)
                {
                    ...
                    result = _bc.AdministrationModel.inControlSaldosCostos_GetByParameter(AppQueries.QueryReferencia, listSaldosCostos);
                    foreach (var item in result)
                    {
                        ...
                        cantidadDisp = this._bc.AdministrationModel.Transaccion_SaldoExistByReferencia(this._documentID, item, ref costos);
                        item.CantidadDisp.Value = cantidadDisp;
                        item.ValorLocalDisp.Value = (costos.CtoLocSaldoIni.Value.Value + costos.CtoLocEntrada.Value.Value - costos.CtoLocSalida.Value.Value) +
                                                (costos.FobLocSaldoIni.Value.Value + costos.FobLocEntrada.Value.Value - costos.FobLocSalida.Value.Value);
                        item.ValorExtranjeroDisp.Value = (costos.CtoExtSaldoIni.Value.Value + costos.CtoExtEntrada.Value.Value - costos.CtoExtSalida.Value.Value) +
                                                     (costos.FobExtSaldoIni.Value.Value + costos.FobExtEntrada.Value.Value - costos.FobExtSalida.Value.Value);
                    }
                ...
                else
                {
                    ...
                    result = result.FindAll(x => x.DocumentoID.Value != 90051 && x.DocumentoID.Value != 90056).OrderByDescending(x =>x.Fecha.Value).ToList();
                    ...
                    foreach (DTO_glMovimientoDeta mvto in result)
                    {
                        if(mvto.TipoTrasladoInv.Value.HasValue)
                        {
                            if (mvto.TipoTrasladoInv.Value == (byte)TipoTraslado.ConsumoInterno)
                                mvto.DescriptivoTarea.Value = "Consumo Interno";
                            else if (mvto.TipoTrasladoInv.Value == (byte)TipoTraslado.PrestamoProyecto)
                                mvto.DescriptivoTarea.Value = "Prestamo";
                            else if (mvto.TipoTrasladoInv.Value == (byte)TipoTraslado.DevolucionProyecto)
                                mvto.DescriptivoTarea.Value = "Devolución";
                        }                         
                    }
                    this.gcData.DataSource = result;
                }
