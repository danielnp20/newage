...
{
    public partial class PagoNomina : DocumentNominaPagosForm
    {
        #region Variables

        BaseController _bc = BaseController.GetInstance();
        private decimal _totalNomina = 0;
        private List<DTO_noPagoLiquidaciones> _liquidaciones = null;
        private List<DTO_noLiquidacionesDetalle> _liqDetalle = null;
        private bool IsFirstTime = false;
        private DTO_glDocumentoControl _dtoCtrl;
        private int IndPagoDetallado = 0;
        ...
        protected override void LookUpDocumentosDataSource()
        {
            ...
            dtoDoc = (DTO_glDocumento)_bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.glDocumento, true, AppDocuments.PagoNomina.ToString(), true);
            this.documentos.Add(dtoDoc.ID.Value, dtoDoc.Descriptivo.Value);
            dtoDoc = (DTO_glDocumento)_bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.glDocumento, true, AppDocuments.PagoVacaciones.ToString(), true);
            this.documentos.Add(dtoDoc.ID.Value, dtoDoc.Descriptivo.Value);
            dtoDoc = (DTO_glDocumento)_bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.glDocumento, true, AppDocuments.PagoPrima.ToString(), true);
            this.documentos.Add(dtoDoc.ID.Value, dtoDoc.Descriptivo.Value);
            dtoDoc = (DTO_glDocumento)_bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.glDocumento, true, AppDocuments.PagoliqContrato.ToString(), true);
            this.documentos.Add(dtoDoc.ID.Value, dtoDoc.Descriptivo.Value);
            dtoDoc = (DTO_glDocumento)_bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.glDocumento, true, AppDocuments.PagoCesantias.ToString(), true);        
        ...
        protected override void LoadData()
        {
            if (this.IsFirstTime)
            {
                ...
                this.Liquidaciones = this._bc.AdministrationModel.Nomina_NominaPagosGet(this.actividadFlujoID, this.dtPeriod.DateTime);
                this._totalNomina = this.Liquidaciones.Sum(x => x.Valor.Value).Value;
                this.gcDocument.DataSource = this.Liquidaciones;

                //Detalle Primera liquidación
                if (this.Liquidaciones != null && this.Liquidaciones.Count > 0)
                {
                    this._liqDetalle.Clear();            
                    this._liqDetalle = this._bc.AdministrationModel.Nomina_GetDetallePago(this.Liquidaciones[0].DocumentoID.Value.Value, this.Liquidaciones[0].PeriodoID.Value.Value, this.Liquidaciones[0].EmpleadoID.Value);
                    this.gcDetail.DataSource = this._liqDetalle;
                }
        ...
        protected override void SetInitParameters()
        {
            ...
            this.documentID = AppDocuments.PagoNomina;
            ...
            this.IndPagoDetallado = int.Parse(this._bc.GetControlValueByCompany(ModulesPrefix.no, AppControl.no_OrdenPagoTesoreria));
            this._liqDetalle = new List<DTO_noLiquidacionesDetalle>();
        ...
        {           
            ...
            this.txtTotalNomina.EditValue = Math.Round(this._totalNomina,0).ToString();
        }
        ...
        protected override void gvDocument_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            if (fieldName == "BancoID")
            {
                e.RepositoryItem = this.editBtnGrid;
            }
            if (fieldName == "TipoCuenta")
            {
                e.RepositoryItem = this.editLook;
            }
            if (fieldName == "Valor")
            {
        ...
        protected override void gvDocument_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (fieldName == "CuentaAbono")
            {
                if (MessageBox.Show(DictionaryMessages.No_EmpleadoCuentaChange, DictionaryMessages.No_EmpleadoCuenta, MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    this.Liquidaciones[e.RowHandle].Empleado.CuentaAbono.Value = this.gvDocument.GetRowCellValue(e.RowHandle, col).ToString();

                    //Actualiza la maestra de empleados
                    DTO_noEmpleado empTemp = this.Liquidaciones[this.gvDocument.FocusedRowHandle].Empleado;
                    empTemp.CuentaAbono.Value = this.gvDocument.GetRowCellValue(e.RowHandle, col).ToString();
                    this._bc.AdministrationModel.MasterSimple_Update(AppMasters.noEmpleado, empTemp);
                }
            }
        ...
        protected override void gvDocument_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (e.FocusedRowHandle >= 0)
            {
                if (this.Liquidaciones != null && this.Liquidaciones.Count > 0)
                {
                    ...
                    if (e.FocusedRowHandle > 0)
                    {
                        this._liqDetalle = this._bc.AdministrationModel.Nomina_GetDetallePago(this.Liquidaciones[e.FocusedRowHandle].DocumentoID.Value.Value, this.Liquidaciones[e.FocusedRowHandle].PeriodoID.Value.Value, this.Liquidaciones[e.FocusedRowHandle].EmpleadoID.Value);
                        this.gcDetail.DataSource = this._liqDetalle;
                        if (this._liqDetalle != null && this._liqDetalle.Count > 0)
                            this.txtTotalEmpleado.EditValue = Math.Round(this._liqDetalle.Sum(x => x.Valor.Value.Value), 0).ToString();
                    }
                }
            }
        }
        ...
        protected void editLook_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (colName == "TipoCuenta")
            {
                if (MessageBox.Show(DictionaryMessages.No_EmpleadoTipoCuentaChange, DictionaryMessages.No_EmpleadoTipoCuenta, MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    string key = (string)origin.EditValue;
                    this.Liquidaciones[this.gvDocument.FocusedRowHandle].Empleado.TipoCuenta.Value = Convert.ToByte(key);

                    //Actualiza la maestra de empleados
                    DTO_noEmpleado empTemp = this.Liquidaciones[this.gvDocument.FocusedRowHandle].Empleado;
                    empTemp.TipoCuenta.Value = Convert.ToByte(key);
                    this._bc.AdministrationModel.MasterSimple_Update(AppMasters.noEmpleado, empTemp);
        ...
        protected void editBtnGrid_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            if (colName == "BancoID")
            {
                if (MessageBox.Show(DictionaryMessages.No_EmpleadoBancoChange, DictionaryMessages.No_EmpleadoBanco, MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    DTO_tsBanco dto = (DTO_tsBanco)_bc.GetMasterDTO(AppMasters.MasterType.Simple, AppMasters.tsBanco, false, this.gvDocument.EditingValue.ToString(), true);
                    this.Liquidaciones[this.gvDocument.FocusedRowHandle].BancoID.Value = dto.ID.Value;
                    this.Liquidaciones[this.gvDocument.FocusedRowHandle].BancoDesc.Value = dto.Descriptivo.Value;
                    this.gcDocument.RefreshDataSource();
                    this.gvDocument.PostEditor();

                    //Actualiza la maestra de empleados
                    DTO_noEmpleado empTemp = this.Liquidaciones[this.gvDocument.FocusedRowHandle].Empleado;
                    empTemp.BancoID.Value = dto.ID.Value;
                    this._bc.AdministrationModel.MasterSimple_Update(AppMasters.noEmpleado, empTemp);
                }
            }
        }
        ...
        public override void SaveThread()
        {
            try
            {
                ...
                FormProvider.Master.FuncProgressBarThread = () => (this._bc.AdministrationModel.ConsultarProgresoNomina(this.documentID));

                ParameterizedThreadStart pth = new ParameterizedThreadStart(FormProvider.Master.CheckServerProcessStatus);
                FormProvider.Master.ProgressBarThread = new Thread(pth);
                FormProvider.Master.ProgressBarThread.Start(this.documentID);

                //Ejecuta el Pago de la Nomina
                List<DTO_TxResult> results = new List<DTO_TxResult>();
                if (this.IndPagoDetallado == 1)
                {
                   results = this._bc.AdministrationModel.Nomina_PagoNominaDetallada(this.documentID, this.dtPeriod.DateTime, this.dtFecha.DateTime, this.Liquidaciones);
                }
                else 
                { 
                   var result = this._bc.AdministrationModel.Nomina_PagoNomina(this.documentID, this.dtPeriod.DateTime, this.dtFecha.DateTime, this.Liquidaciones, actividadFlujoID);
                   results.Add(result);
                }
        ...
        /// </summary>
        protected virtual List<DTO_noPagoLiquidaciones> Liquidaciones
        {
