...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    /// </summary>
    public partial class AltaInvActivos : DocumentForm
    {       
        #region Delegados
        /// <summary>
        ...
        protected override void LoadData(bool firstTime)
        {
            try
            {
                ...
                filter.MvtoTipoInvID.Value = this._bc.GetControlValueByCompany(ModulesPrefix.@in, AppControl.in_TipoMovEntradaComprasLoc);
                filter.EntradaSalida.Value = (byte)EntradaSalida.Entrada;
                filter.BodegaID.Value = this.uc_Bodega.Value;
                filter.EmpresaID.Value = this.empresaID;

                List<DTO_glMovimientoDeta> temp = this._bc.AdministrationModel.glMovimientoDeta_GetByParameter(filter,false);
                filter.MvtoTipoInvID.Value = this._bc.GetControlValueByCompany(ModulesPrefix.@in, AppControl.in_TipoMovEntradaComprasExt);
                temp.AddRange(this._bc.AdministrationModel.glMovimientoDeta_GetByParameter(filter, false));

                DTO_acActivoControl act = null;
                foreach (var item in temp)
                {
                    act = new DTO_acActivoControl();
                    act.ActivoID.Value = item.ActivoID.Value;
                    act.BodegaID.Value = item.BodegaID.Value;
                    act.inReferenciaID.Value = item.inReferenciaID.Value;
                    act.EstadoInv.Value = item.EstadoInv.Value;
                    act.CodigoBSID.Value = item.CodigoBSID.Value;
                    act.SerialID.Value = item.SerialID.Value;
                    act.DocumentoTercero.Value = item.DocSoporteTER.Value;
                    act.TerceroID.Value = item.TerceroID.Value;
                    act.ProyectoID.Value = item.ProyectoID.Value;
                    act.CentroCostoID.Value = item.CentroCostoID.Value;
        ...
        {
            this.documentID = AppDocuments.AltaInvActivos;
            InitializeComponent();
        ...
        protected override void AfterInitialize()
        {
            ...
            if (!this.multiMoneda)
            {
                ...
                this.lblTcIFRS.Visible = false;
                this.txtTcIFRS.Visible = false;
            }
        ...
        public void InitControls()
        {
            ...
            #region Filtros Tipo de Movimientos (Alta Inventarios)
            ...
            eg.ValorFiltro = "11"; //Corresponde al tipo de movientos (11) Altas Inventarios
            ...
            this._bc.InitMasterUC(this.uc_Bodega, AppMasters.inBodega, true, true, true, false);
            ...
            if (!string.IsNullOrEmpty(indTcIFRS))
            {
                ...
                {
                    this.txtTcIFRS.Text = this._tasaCambio.ToString();
                }
            }
        ...
        private void EnableFooter(bool enable)
        {
            if (!enable)
            {
                ...
                //Totales
                this.txtLocalPesos.Text = string.Empty;
                this.txtIFRSPesos.Text = string.Empty;
            }
        ...
        {            
            ...
            if (this._lActivos != null && this._lActivos.Count > 0)
            {
                ...
                if (acClase != null)
                {
                    this.txtValorSalvamentoLocal.Text = (((acClase.PorcSalvamentoLOC.Value != null ? acClase.PorcSalvamentoLOC.Value : 0) * currentActivo.CostoLOC.Value) / 100).ToString();
                    this.txtValorSalvamentIFRS.Text = (((acClase.PorcSalvamentoIFRS.Value != null ? acClase.PorcSalvamentoIFRS.Value : 0) * currentActivo.CostoLOC.Value) / 100).ToString();
                    this.txtValorSalvamentUSG.Text = (((acClase.PorcSalvamentoUSG.Value != null ? acClase.PorcSalvamentoUSG.Value : 0) * currentActivo.CostoEXT.Value) / 100).ToString();
                ...
                //Libro Local
                this.txtLocalPesos.EditValue = currentActivo.CostoLOC.Value ?? 0;
                this.txtLocalDolares.EditValue = currentActivo.CostoEXT.Value ?? 0;

                //Libro IFRS
                this.txtIFRSPesos.EditValue = currentActivo.CostoLOC.Value ?? 0 + currentActivo.ValorRetiroIFRS.Value ?? 0;
                this.txtIFRSDolares.EditValue = currentActivo.CostoEXT.Value ?? 0  + ((currentActivo.ValorRetiroIFRS.Value ?? 0) / Convert.ToDecimal(this.txtTcIFRS.EditValue, CultureInfo.InvariantCulture)); 
            }
        }
        ...
        private void cmbControl_Leave(object sender, EventArgs e)
        {
            if (this._lActivos != null && this._lActivos.Count > 0)
            {
                ...
                switch (ctrl.Name)
                {
                    // Combo Tipo depreciacion Local
                    case "cmbTipoDecpreciacionLOCAL":
                        this._lActivos[index].TipoDepreLOC.Value = Convert.ToByte((this.cmbTipoDecpreciacionLocal.SelectedItem as ComboBoxItem).Value);
                        if (this.cmbTipoDecpreciacionLocal.SelectedIndex == Convert.ToInt32(TipoDepreciacion.SaldosDecrecientes))
                    ...
                    // Combo Tipo depreciacion IFRS
                    case "cmbTipoDecpreciacionIFRS":
                        this._lActivos[index].TipoDepreIFRS.Value = Convert.ToByte((this.cmbTipoDecpreciacionIFRS.SelectedItem as ComboBoxItem).Value);
                        if (this.cmbTipoDecpreciacionIFRS.SelectedIndex == Convert.ToInt32(TipoDepreciacion.SaldosDecrecientes))
                    ...
                    // Combo Tipo depreciacion USG
                    case "cmbTipoDecpreciacionUSG":
                        this._lActivos[index].TipoDepreUSG.Value = Convert.ToByte((this.cmbTipoDecpreciacionUSG.SelectedItem as ComboBoxItem).Value);
                        if (this.cmbTipoDecpreciacionIFRS.SelectedIndex == Convert.ToInt32(TipoDepreciacion.SaldosDecrecientes))
                            this.txtValorSalvamentIFRS.Enabled = true;
                        else
                            this.txtValorSalvamentIFRS.Enabled = false;
        ...
        private void cmbTipoDecpreciacionUSG_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (this.cmbTipoDecpreciacionUSG.SelectedIndex == Convert.ToInt32(TipoDepreciacion.SaldosDecrecientes))
            {
                this.cmbTipoDecpreciacionUSG.Enabled = true;
            }
            else
        ...
        private void txtValorRetiro_EditValueChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(this.txtValorRetiro.Text))
            {
                ...
                this.txtLocalPesos.EditValue = (this._lActivos[fila].CostoLOC.Value + this._lActivos[fila].ValorRetiroIFRS.Value);
                this.txtLocalDolares.EditValue = (this._lActivos[fila].CostoEXT.Value + (this._lActivos[fila].ValorRetiroIFRS.Value / Convert.ToDecimal(this.txtTcIFRS.EditValue, CultureInfo.InvariantCulture)));
                this.CalcularTotal();
            }
        }
        ...
        public override void SaveThread()
        {
            try
            {
                ...
                List<DTO_TxResult> results = _bc.AdministrationModel.acActivoControl_AddList(this.documentID, this._actFlujo.ID.Value, this.txtPrefix.Text, listAppr, this.masterMovimientoTipo.Value, 0);
