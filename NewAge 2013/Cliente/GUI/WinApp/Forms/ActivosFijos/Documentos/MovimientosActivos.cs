...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    /// </summary>
    public partial class MovimientosActivos : DocumentForm
    {       
        #region Delegados
        /// <summary>
        ...
        public void InitControls()
        {
            ...
            #region Filtros Tipo de Movimientos (Traslado)
            ...
            eg.ValorFiltro = "4"; //Corresponde al tipo de movientos (4) Transalado
            eg.OperadorSentencia = "OR";

            DTO_glConsultaFiltro egTerceros = new DTO_glConsultaFiltro();
            egTerceros.CampoFisico = "TipoMvto";
            egTerceros.OperadorFiltro = OperadorFiltro.Igual;
            egTerceros.ValorFiltro = "6"; //Corresponde al tipo de movientos (6) En poder de Terceros
            egTerceros.OperadorSentencia = "OR";

            DTO_glConsultaFiltro egArriendo = new DTO_glConsultaFiltro();
            egArriendo.CampoFisico = "TipoMvto";
            egArriendo.OperadorFiltro = OperadorFiltro.Igual;
            egArriendo.ValorFiltro = "9"; //Corresponde al tipo de movientos (9) Arriendo
            egArriendo.OperadorSentencia = "OR";

            DTO_glConsultaFiltro egMantenimiento = new DTO_glConsultaFiltro();
            egMantenimiento.CampoFisico = "TipoMvto";
            egMantenimiento.OperadorFiltro = OperadorFiltro.Igual;
            egMantenimiento.ValorFiltro = "13"; //Corresponde al tipo de movientos (13) Mantenimiento
            egMantenimiento.OperadorSentencia = "OR";
            ...
            _bc.InitMasterUC(this.uc_Tercero, AppMasters.coTercero,true, true, true, false);
            this.uc_Tercero.EnableControl(false);
        ...
        private void DefaultData()
        {
            ...
            {                
                ...
                acCtrl.Propietario.Value = _bc.GetControlValueByCompany(ModulesPrefix.co, AppControl.co_TerceroXDefecto);
        ...
        private void uc_TipoMovimiento_Leave(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    if (MvtoTipo.TipoMvto.Value != (byte)TipoMovimientoAct.Traslado)
                        this.uc_Tercero.EnableControl(false);
                    else
                        this.uc_Tercero.EnableControl(false);
        ...
        /// <summary>
        /// Accede a la lista de activos que desea mover
        /// </summary>
        ...
        private void btnActivos_Click(object sender, EventArgs e)
        {
            ...
            if (this._lActivos != null && this._lActivos.Count > 0)
                this.btnActivos.EditValue = this._lActivos[0].ActivoID.Value.ToString();
            this.LoadData(true);
        }
                        ...
                        if (acTipo != null)
                        {
                            ...
                            if (acTipo.ContenedorInd.Value.Value)
                            {
                                ...
                                foreach (var actChild in children)
                                {
                                    ...
                                    {
                                        if (!this._lActivos.Any(x => x.PlaquetaID.Value == act.PlaquetaID.Value))
        ...
        public override void SaveThread()
        {
            try
            {
                ...
                //Valida que se haya seleccionado Tipo Movimiento
                if (string.IsNullOrEmpty(this.uc_TipoMovimiento.Value))
                {
                    MessageBox.Show(this._bc.GetResource(LanguageTypes.Messages, DictionaryMessages.Ac_invalidMove));
                    return;
                }

                if (string.IsNullOrEmpty(this.uc_CentroCosto.Value) && string.IsNullOrEmpty(this.uc_Proyecto.Value) && string.IsNullOrEmpty(this.uc_LocFisica.Value))
                {
                    MessageBox.Show("Debe seleccionar Centro de Costo, Proyecto ó Loc Fisica de destino");
                    return;
                }
                ...
                List<DTO_TxResult> results = this._bc.AdministrationModel.acActivoControl_MovActivos(this.documentID, this._actFlujo.ID.Value, this.prefijoID, 
                                                                                                     this._lActivos, this.uc_TipoMovimiento.Value, 
                                                                                                     this.uc_Proyecto.Value, this.uc_CentroCosto.Value, 
                                                                                                     this.uc_LocFisica.Value, this.userID.ToString());                
