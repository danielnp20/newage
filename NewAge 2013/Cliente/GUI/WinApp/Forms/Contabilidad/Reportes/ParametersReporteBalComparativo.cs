...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    public class ParametersReporteBalComparativo : ReportParametersForm
    {
        ...
        protected override void Report_PDF()
        {
            ...
            if (this.periodoFilter1.Months[0] == 13)
                {
                string fechaIniString = this.periodoFilter1.txtYear.Text + " / 12 / " + "02";
                this._fechaIni = Convert.ToDateTime(fechaIniString);
                }
            else
                {
                string fechaIniString = this.periodoFilter1.txtYear.Text + " / " + this.periodoFilter1.Months[0] + " / " + "01";
                this._fechaIni = Convert.ToDateTime(fechaIniString);
                }

            if (this.periodoFilter1.Months[1] == 13)
                {
                string fechaFinString = this.periodoFilter1.txtYear.Text + " / 12 / " + "02";
                this._fechaFin = Convert.ToDateTime(fechaFinString);
                }
            else
                {
                string fechaFinString = this.periodoFilter1.txtYear.Text + " / " + this.periodoFilter1.Months[1] + " / " + "01";
                this._fechaFin = Convert.ToDateTime(fechaFinString);
                }
