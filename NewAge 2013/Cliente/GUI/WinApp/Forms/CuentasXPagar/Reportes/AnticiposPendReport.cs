...
{
    class AnticiposPendReport : BaseReport //XtraReport//
    {
        #region Variables
        ...
        public AnticiposPendReport(List<DTO_ReportAnticipo> reportData, ArrayList fieldList, TipoMoneda MM, DateTime Date, List<string> selectedFiltersList, bool detailInd, string docID)
        {
            this._docID = docID;
            this.lblReportName.Text = _bc.GetResource(Librerias.Project.LanguageTypes.Forms, (AppReports.cpAnticiposPendientes).ToString());
            ...
            foreach (string fieldName in fieldList)
            {
                ...
                if (fieldName.Contains("Observacion") || fieldName.Contains("Desc"))
                    tableHeaderCell.WidthF = columnWidth + 200;
                else
                    tableHeaderCell.WidthF = columnWidth;
                string resourceFieldID = (AppReports.cpAnticiposPendientes).ToString() + "_" + fieldName;
        ...
        private void groupHeaderCell_Name_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRTableCell headerCell = (XRTableCell)sender;
                ...
                if (!string.IsNullOrEmpty(headerCell.Text) && !string.IsNullOrWhiteSpace(headerCell.Text))
                    headerCell.Text = _bc.GetResource(Librerias.Project.LanguageTypes.Forms, (AppReports.cpAnticiposPendientes).ToString() + "_" + headerCell.Text);
