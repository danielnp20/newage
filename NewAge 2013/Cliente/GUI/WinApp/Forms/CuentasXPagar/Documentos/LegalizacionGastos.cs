...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    /// Comprobante Manual
    /// </summary>
    public partial class LegalizacionGastos : DocumentLegalizaForm
    {
        public LegalizacionGastos()
        {
        ...
        {
            this.documentID = AppDocuments.LegalizacionGastos;
            InitializeComponent();
        ...
        /// <param name="sender">Objeto que envia el evento</param>
        /// <param name="e">Evento</param>
        private void masterTerceroLeg_Leave(object sender, EventArgs e)
        {
            try
            {
                if (this.masterTerceroHeader.ValidID)
                {
                   ...
                   // Trae el comprobante asociado y lo asigna
                    ...
                    if (docCtrlExist != null)
                    {
                        ...
                        //Valida si existe
                        if (Leg != null)
                            if (this.masterTerceroHeader.Value == Leg.DocCtrl.TerceroID.Value && Leg.DocCtrl.Estado.Value != (byte)EstadoDocControl.Aprobado)
                            {
                                this.data = new DTO_Legalizacion();
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-LegalizacionGastos.cs", "LegalizacionGastos.cs-masterTerceroLeg_Leave"));
            }
        }
        ...
        private void btnAnticipo_Click(object sender, EventArgs e)
        {
            ...
            {
                decimal tc = Convert.ToDecimal(this.txtTasaCambioActual.EditValue, CultureInfo.InvariantCulture);
                ModalAnticipos ant = new ModalAnticipos(this._anticipos, this.dtPeriod.DateTime, this._tipoMoneda, tc, this.masterTerceroHeader.Value);
                ant.ShowDialog();
                if (ant.ReturnVals)
                {
        ...
        public override void SaveThread()
        {
            try
            {
                ...
                else
                {
                    ...
                    result = this._bc.AdministrationModel.Legalizacion_Update(this.documentID, this.data.Footer, this.data.Header);
                }
