...
{
    public partial class InventarioFisicoReport : BaseCommonReport
    {
        #region Variables
        ...
        /// InventarioFisicoReport(AppReports.inFisicoInventario, new List<DTO_ReportInvFisico> { reportInvFisico }, detailFieldList1, detailFieldList2, suppl);
        public InventarioFisicoReport(int docId, List<DTO_ReportInvFisico> documentoData, ArrayList detailFieldList, CommonReportDataSupplier supplier, InventarioFisicoReportType reporteTipo)
            : base(supplier)
        {
            ...
            for (int i = 0; i < documentoData.Count; i++)
            {
                ...
                documentoHeaderTableCell_Name.Text = "Bodega:";//base._dataSupplier.GetResource(Librerias.Project.LanguageTypes.Forms, docID.ToString() + "_Para");
                ...
                documentoHeaderTableCell_Value.DataBindings.Add("Text", documentoData[i].Header, "Bodega");
                ...
                documentoHeaderTableCell_Value.WidthF = headerTableWidth / 4;
                documentoHeaderTableCell_Value.TextAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleCenter;
                documentoHeaderTableCell_Value.DataBindings.Add("Text", documentoData[i].Header, "Fecha");
                documentoHeaderTableRow.Cells.Add(documentoHeaderTableCell_Value);
            ...
            foreach (string fieldName in detailFieldList)
            {
                ...
                tableDetailCell.WidthF = tableHeaderCell.WidthF;
                #region Físico
                if (reporteTipo == InventarioFisicoReportType.Fisico)
                {
                    if (fieldName.Contains("Codigo"))
                    {
                    ...
                    if (fieldName.Contains("Descripcion") || fieldName.Contains("DescripReferencia"))
                    {
                        tableDetailCell.TextAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleCenter;
                        tableDetailCell.DataBindings.Add("Text", documentoData, "Detail." + fieldName);
                    }
                    if (fieldName.Contains("Kardex"))
                    {
                        tableDetailCell.TextAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleCenter;
                        tableDetailCell.DataBindings.Add("Text", documentoData, "Detail." + fieldName);
                    }
                    if (fieldName.Contains("Fisico"))
                    {
                        tableDetailCell.TextAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleCenter;
                        tableDetailCell.DataBindings.Add("Text", documentoData, "Detail." + fieldName);
                    }
