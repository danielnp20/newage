...
namespace NewAge.Server.GlobalService
{
    ...
    public class GlobalService : IGlobalService, IDisposable
    {
        ...
        public DTO_TxResult DevolverFlujoDocumento(Guid channel, int documentActividad, string actFlujoNueva, int? numeroDoc, string observacion)
        {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();
                ModuloGlobal mod = (ModuloGlobal)facade.GetModule(ModulesPrefix.gl, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, this._connLoggerString);
                var response = mod.DevolverFlujoDocumento(documentActividad, actFlujoNueva, numeroDoc, observacion,false);
        ...
        /// <returns>Resultado</returns>
        public DTO_TxResult glGarantiaControl_Add(Guid channel,int documentID, List<DTO_glGarantiaControl> garantias)
        {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();
                ModuloGlobal mod = (ModuloGlobal)facade.GetModule(ModulesPrefix.gl, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, this._connLoggerString);
                var response = mod.glGarantiaControl_Add(documentID, garantias,DictionaryProgress.BatchProgress,false);
        ...
        /// <param name="estado"> Estado de la garantia</param>
        /// <returns>Lista </returns>
        public List<DTO_QueryGarantiaControl> glGarantiaControl_Decisor(Guid channel, int numerodoc)
        {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();
                ModuloGlobal mod = (ModuloGlobal)facade.GetModule(ModulesPrefix.gl, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, this._connLoggerString);
                var response = mod.glGarantiaControl_Decisor(numerodoc);
        ...
        public List<DTO_glMovimientoDeta> glMovimientoDeta_GetByParameter(Guid channel, DTO_glMovimientoDeta filter, bool isPre)
        {
            int opIndex = -1;
            try
            {
                opIndex = this.ADO_ConnectDB();
                ModuloGlobal mod = (ModuloGlobal)facade.GetModule(ModulesPrefix.gl, this._mySqlConnections[opIndex], null, _channels[channel].Item1, _channels[channel].Item2, this._connLoggerString);
                var response = mod.glMovimientoDeta_GetByParameter(filter, isPre);
