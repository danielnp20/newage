    ...
    public class DAL_ReportesFacturacion : DAL_Base
    {
        ...
        public List<DTO_ReportFacturaVenta> DAL_ReportesFacturacion_FacturaVenta(string numDoc, DTO_coTercero dtoTerceroEmpr , bool isAprobada)
        {
            ...
            try
            {
                ...
                mySqlCommandSel.CommandText =
                    ...
                    "        faDocu.Retencion8,faDocu.Retencion9,faDocu.Retencion10,faDocu.FormaPago, faDocu.DatoAdd1,faDocu.DatoAdd2,faDocu.DatoAdd3,faDocu.DatoAdd4,faDocu.DatoAdd5 " +
            ...
            try
            {
                ...
                if (isDetallada)
                {
                    ...
                    mySqlCommandSel.CommandText =
                                        " SELECT Detalle.Factura, Detalle.FechaPtoPago, Detalle.FechaVto,  Detalle.CuentaID, TerceroID, Descriptivo,Valor, DifDia, " +  
	                                        " No_Vencidas = CASE WHEN DifDia <= 0 then 'No Vencidas' end,  " +
	                                        ...
	                                        " MasCOchenta = CASE WHEN DifDia > 181 then No_Vencidas else 0 end  FROM   " +
			                                         ...
			                                         " SELECT faDocu.NumeroDoc as Factura, faDocu.FechaPtoPago, faDocu.FechaVto,      " +
			                                             ...
			                                             " CrSaldoIniLocML + CrSaldoIniExtML) * -1)as No_Vencidas, tercero.TerceroID, tercero.Descriptivo , faDocu.Valor, saldo.PeriodoID ,  " +
			                                         ...
			                                         " FROM faFacturaDocu faDocu   WITH(NOLOCK)  " +
			                                             ...
			                                             " INNER JOIN coTercero tercero WITH(NOLOCK)  on tercero.TerceroID = ctrl.TerceroID  " +    
			                                         " WHERE  " +  
                ...
                else
                {
                    ...
                    mySqlCommandSel.CommandText =
                        ...
                        " SELECT *, (Treinta +Sesenta +Noventa+COchenta+MasCOchenta) AS total " + 
                        " FROM  " + 
                        " ( " + 
                        " SELECT  TerceroID, Descriptivo, " +   
	                        "  No_Vencidas = CASE WHEN DifDia <= 0 then 'No Vencidas'  else ''end,  " + 
	                        ...
	                        " COchenta = SUM(CASE WHEN DifDia BETWEEN 91 AND 180 then No_Vencidas else 00 end), " +  
                        ...
                        " SELECT faDocu.NumeroDoc as Factura, faDocu.FechaPtoPago, faDocu.FechaVto, " +      
	                        ...
	                        " WHERE    " + 
		                        ...
		                        " AND faDocu.EmpresaID = @EmpresaID  " +
                                    terceroFil +                      
		                        " ) AS Detalle " +  
                        " GROUP BY Descriptivo, Detalle.TerceroID, Detalle.DifDia) as consulta ";
        ...
        public List<DTO_ReportLibroVentas> DAL_ReportesFacturacion_LibroVentas(DateTime periodo, int MesFinal, string cliente, string prefijo, string NroFactura)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText =
                    " SELECT ctrl.DocumentoNro, ctrl .PrefijoID, RTRIM(CAST(ctrl.PrefijoID as CHAR(10)) + ' ' + '-' + ' ' + CAST(ctrl.DocumentoNro as CHAR(10))) as factura, " +
        ...
        public List<DTO_ReportCuentasPorCobrar> DAL_ReportesFacturacion_CuentasPorCobrar(string Tercero, int Moneda, string Cuenta, DateTime fecha)
        {
            ...
            try
            {
                ...
                mySqlCommandSel.CommandText =
                   ...
                   "            INNER JOIN glTasaCambio cambio WITH(NOLOCK) ON cambio.Fecha=@Fecha and cambio.EmpresaGrupoID=@EmpresaID " +
