    ...
    public class DAL_glMovimientoDeta : DAL_Base
    {
        ...
        public List<DTO_glMovimientoDeta> DAL_glMovimientoDeta_GetByNumeroDoc(int NumeroDoc, bool trasladoNotaEnvio = false)
        {
            try
            {
                ...
                mySqlCommand.CommandText = "select * from glMovimientoDeta with(nolock) where glMovimientoDeta.NumeroDoc = @NumeroDoc ";
        ...
        public List<DTO_glMovimientoDeta> DAL_glMovimientoDeta_GetByParameter(DTO_glMovimientoDeta filter,bool isPre)
        {
            try
            {
                ...
                query = " Select mvto.*,ctrl.PrefijoID,ctrl.Estado, ctrl.DocumentoNro,ctrl.FechaDoc as Fecha,ctrl.DocumentoID, refer.RefProveedor, marca.Descriptivo as MarcaDesc,docu.FacturaTipoID " + 
                        "  from "+ tabla + " mvto with(nolock) " +
                        " Inner join glDocumentoControl as ctrl  with(nolock) on ctrl.NumeroDoc = mvto.NumeroDoc " +
                        " Left join faFacturaDocu as docu  with(nolock) on docu.NumeroDoc = ctrl.NumeroDoc " +
                ...
                {
                    query += "and mvto.DocumentoNro = @DocumentoNro ";
                ...
                while (dr.Read())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dr["Estado"].ToString()))
                        mvto.EstadoDocCtrl.Value = Convert.ToByte(dr["Estado"]);
        ...
        public List<DTO_glMovimientoDeta> DAL_glMovimientoDetaPRE_Get(int NumeroDoc)
        {
            try
            {
                ...
                mySqlCommand.CommandText = "select * from glMovimientoDetaPRE with(nolock) where glMovimientoDetaPRE.NumeroDoc = @NumeroDoc ";
