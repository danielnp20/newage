    ...
    public class DAL_ccCierreDia : DAL_Base
    {
        ...
        public List<DTO_GestionCobranza> DAL_ccCierreDiaCartera_GetGestionCobranza(string cliente, DateTime fechaCierre, string actividadGestCobranza)
        {
            try
            {
                ...
                mySqlCommand.CommandText =
                "SELECT cr.NumeroDoc, cr.Libranza,cr.Plazo,cr.NumSolicitud, cd.Consecutivo,cd.CtasVencidas,cd.FechaMora,cd.DiasMora, cd.NumDocControlIncumplido," +
                ...
                "       cd.ClienteID,cl.Descriptivo as ClienteDesc,cl.TerceroID,ie.Numero1,ie.FechaCompromiso, ie.Valor1,ie.Observaciones,ie.Consecutivo as ConsEstadoIncumplido, '' as  ObservacionCompromiso, " +
                "       ctrl.Observacion as ObservacionHistoria,IsNull((Select top(1) ac.CerradoInd from glActividadEstado ac where ac.NumeroDoc = cr.NumeroDoc and ac.ActividadFlujoID = @ActividadFlujoCob),0) as CumplidoInd " + 
                ...
                "       LEFT JOIN glIncumpleCambioEstado ie WITH(NOLOCK) ON ie.Consecutivo = cd.ConsEstadoIncumplido " +
                " WHERE	cd.Fecha = @diaCierre " +
                "	    and cd.NumDocControlIncumplido is not null " + query +
                "	    and (ie.FechaCompromiso < GETDATE() or ie.FechaCompromiso is null )"+
                " GROUP BY cr.NumeroDoc,cr.Libranza,cr.Plazo,cr.NumSolicitud, cd.Consecutivo, cd.CtasVencidas,cd.FechaMora,cd.DiasMora, cd.NumDocControlIncumplido," +
                "        cd.SaldoVencido,cd.VlrIntMora,cd.VlrPrejuridico,cd.VlrOtros," +
                "        cd.ClienteID,cl.Descriptivo,cl.TerceroID,ie.Numero1,ie.FechaCompromiso, ie.Valor1,ie.Observaciones,ie.Consecutivo, ctrl.Observacion";
