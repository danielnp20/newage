        ...
        public List<DTO_ccReincorporacionDeta> DAL_ccReincorporacionDeta_GetForReincorporacion(DateTime periodo, string centroPagoID)
        {
            try
            {
                ...
                mySqlCommandSel.CommandText =
                    ...
                    "   nom.ClienteID, cli.Descriptivo AS Nombre, cli.ProfesionID, nom.nom_CentroPagoID AS CentroPagoID, cp.Descriptivo AS CentroPagoDesc, cli.EmpleadoCodigo, " +
                    ...
                    "		nom.FechaNomina AS nom_FechaNomina, nom.EstadoCruce as nom_EstadoCruce, nom.CentroPagoID as nom_CentroPagoID, nom.eg_ccCentroPagoPAG as nom_eg_ccCentroPagoPAG " +
                    ...
                    ")as nom " +
                    "	inner join ccCliente cli with(nolock) on nom.ClienteID = cli.ClienteID and nom.eg_ccCliente = cli.EmpresaGrupoID " +
                    "	inner join ccCentroPagoPAG cp with(nolock) on nom.nom_CentroPagoID = cp.CentroPagoID and nom.nom_eg_ccCentroPagoPAG = cp.EmpresaGrupoID " +
                    "	inner join  " +
                    "	( " +
                    ...
                    "order by numerodoc, nom.nom_CentroPagoID desc, CAST(Libranza AS INTEGER) ";
