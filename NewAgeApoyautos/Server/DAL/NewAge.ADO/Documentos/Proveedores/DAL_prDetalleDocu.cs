    ...
    public class DAL_prDetalleDocu : DAL_Base
    {
        ...
        public DTO_prDetalleDocu DAL_prDetalleDocu_GetByID(int ConsecutivoDetaID)
        {
            try
            {
                ...
                mySqlCommand.CommandText = "select * from prDetalleDocu with(nolock) where ConsecutivoDetaID = @ConsecutivoDetaID";
        ...
        public List<DTO_prRecibidoAprob> DAL_prDetalleDocu_GetPendientesCargosRecib()
        {
            try
            {
                ...
                mySqlCommand.CommandText =
                "select distinct ctrl.EmpresaID,ctrl.FechaDoc Fecha,det.ConsecutivoDetaID,doc.DocumentoID, ctrl.NumeroDoc,ctrl.DocumentoNro,usr.UsuarioID,rec.ProveedorID," +
                    "det.RecibidoDocuID,det.RecibidoDetaID,det.OrdCompraDocuID,det.OrdCompraDetaID," +
                    "det.SolicitudDetaID,det.SolicitudDocuID,det.CantidadRec, det.CodigoBSID,det.ValorUni,det.IVAUni," +
                ...
                "group By ctrl.EmpresaID,ctrl.FechaDoc,det.ConsecutivoDetaID,doc.DocumentoID,ctrl.NumeroDoc,ctrl.DocumentoNro, usr.UsuarioID,  rec.ProveedorID," +
                     ...
                     "det.ValorTotML,det.IvaTotML, det.ValorTotME, det.IvaTotME ";
        ...
        public List<DTO_prDetalleDocu> DAL_prDetalleDocu_GetByDocument(int document, int numeroDoc, int consecutivoDeta)
        {
            try
            {
                ...
                else if (document == AppDocuments.OrdenCompra)
                {
                    ...
                    query += " where det.OrdCompraDocuID = @OrdCompraDocuID  and det.OrdCompraDocuID <> det.ConsecutivoDetaID and det.RecibidoDocuID is not null and det.FacturaDocuID is null ";
                    if (consecutivoDeta != 0)
                    {
                        mySqlCommand.Parameters.Add("@OrdCompraDetaID", SqlDbType.Int);
            ...
            try
            {
                ...
                if (documentFilter == (byte)AppDocuments.Solicitud)
                {
                    ...
                    mySqlCommand.CommandText =
                     ...
                     " solCargo.ProyectoID,solCargo.CentroCostoID,refer.MarcaInvID,refer.RefProveedor,IsNull(empaque.Cantidad,1) as CantidadxEmpaque,temp.CantidadSum CantidadPend,det.* " +
                     ...
                     "            where ctrlSol.EmpresaID = @EmpresaID and doc.ModuloID = 'pr'  " +
                     "                and ( (ctrlSol.DocumentoID = @DocumentoID1 and ctrlSol.Estado = @Estado)  or ctrlSol.DocumentoID = @DocumentoID2 or ctrlSol.DocumentoID = @DocumentoID3) " +
                     ...
                     " left join inReferencia refer with(nolock) on refer.inReferenciaID = det.CodigoBSID and refer.EmpresaGrupoID = det.eg_prBienServicio  " +
                     " left join inEmpaque empaque with(nolock) On empaque.EmpaqueInvID = det.EmpaqueInvID and empaque.EmpresaGrupoID = det.eg_inEmpaque  " +
                ...
                else if (documentFilter == (byte)AppDocuments.Recibido)
                {
                    ...
                    mySqlCommand.CommandText =
                     " Select ctrl.NumeroDoc,ctrl.PeriodoDoc,ctrl.FechaDoc,ctrl.PrefijoID,ctrl.DocumentoNro,docu.ProveedorID, " +
                     " solCargo.ProyectoID,solCargo.CentroCostoID,refer.MarcaInvID,refer.RefProveedor,IsNull(empaque.Cantidad,1) as CantidadxEmpaque,temp.CantidadSum CantidadPend,det.* " +
                     " from    (  Select det.RecibidoDocuID,det.RecibidoDetaID,SUM(det.CantidadOC) CantidadSum " +
                     "            from prDetalleDocu det with(nolock)  " +
                     "                inner join glDocumentoControl ctrlOC with(nolock) on det.NumeroDoc = ctrlOC.NumeroDoc " +
                     "                inner join glDocumento doc with(nolock) on ctrlOC.DocumentoID = doc.DocumentoID  " +
                     "            where ctrlOC.EmpresaID = @EmpresaID and doc.ModuloID = 'pr'  " +
                     ...
                     " inner join prDetalleDocu det with(nolock) on temp.RecibidoDetaID = det.ConsecutivoDetaID " +
