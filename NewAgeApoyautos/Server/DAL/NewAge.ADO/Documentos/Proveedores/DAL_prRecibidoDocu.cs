            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    else
                    {
                        ...
                        dto.CostoML.Value = dto.CostoML.Value ?? 0;
                        dto.CostoME.Value = dto.CostoME.Value ?? 0;
                        dto.CostoIvaML.Value = dto.CostoIvaML.Value ?? 0;
                        dto.CostoIvaME.Value = dto.CostoIvaME.Value ?? 0; 
                    }

                    DTO_prRecibidoAprobDet dtoDet = new DTO_prRecibidoAprobDet(dr);
                    try { dtoDet.OrigenMonetario.Value = Convert.ToByte(dr["OrigenMonetario"]); } catch (Exception) { ;} 
                    //dtoDet.ValorUni.Value = dtoDet.ValorUni.Value * dtoDet.CantidadRec.Value;
                    dto.Detalle.Add(dtoDet);
