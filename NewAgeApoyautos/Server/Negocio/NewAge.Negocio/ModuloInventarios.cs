    ...
    public class ModuloInventarios : ModuloBase
    {
            ...
            try
            {
                ...
                foreach (var inv in listInvFisico)
                {
                    if (inv.CantAjuste.Value > 0)
                    {
                        ...
                        mov.Valor1LOC.Value = (inv.CostoLocal.Value.Value / inv.CantKardex.Value.Value) * inv.CantAjuste.Value;
                        mov.Valor1EXT.Value = (inv.CostoExtra.Value.Value / inv.CantKardex.Value.Value) * inv.CantAjuste.Value;
                    ...
                    if (inv.CantAjuste.Value < 0)
                    {
                        ...
                        mov.CentroCostoID.Value = bodega.CentroCostoID.Value;
                        mov.ProyectoID.Value = bodega.ProyectoID.Value;
                        mov.Valor1LOC.Value = Math.Abs((inv.CostoLocal.Value.Value / inv.CantKardex.Value.Value) * inv.CantAjuste.Value.Value);
                        mov.Valor1EXT.Value = Math.Abs((inv.CostoExtra.Value.Value / inv.CantKardex.Value.Value) * inv.CantAjuste.Value.Value);
            ...
            try
            {
                ...
                if (!update)
                {
                    ...
                    foreach (DTO_inMovimientoFooter item in transaccion.Footer)
                    {
                        ...
                        #region Valida si obtiene y asigna datos de un proyecto controlado
                        //foreach (DTO_prSolicitudResumen detSol in item.DetalleSolicitud)
                        //{
                        //    if (detSol.Detalle4ID.Value.HasValue)
                        //    {
                        //        DTO_pyProyectoMvto mvtoProyecto = this._dal_pyProyectoMvto.DAL_pyProyectoMvto_GetByConsecutivo(detSol.Detalle4ID.Value);
                        //        item.Movimiento.DocSoporte.Value = detSol.Detalle4ID.Value;
                        //    }
                        //}
                        #endregion
                ...
                if (documentID != AppDocuments.DistribucionCostos && documentID != AppDocuments.DeterioroInvAprob && documentID != AppDocuments.RevalorizacionInvAprob)
                {
                                    ...
                                    if (costos != null)
                                    {
                                        if (costeoOrigen.CosteoTipo.Value == (byte)TipoCosteoInv.Promedio)
                                        {
                                            ...
                                            mvtoDeta.Valor1EXT.Value = (costos.CtoExtSaldoIni.Value.Value + costos.CtoExtEntrada.Value.Value - costos.CtoExtSalida.Value.Value) /
                                                                        (cantidadExist) * mvtoDeta.CantidadUNI.Value.Value;
                    ...
                    {                       
                        foreach (var deta in listMvtoDeta)
                        {
                            ...
                            traslado.ProyectoID.Value = deta.ProyectoID.Value;
        ...
        public DTO_TxResult Proceso_TrasladoSaldosInventario(int documentID, DateTime periodoOld, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                List<int> consecCostos = new List<int>();
                #endregion
                ...
                foreach (DTO_inControlSaldosCostos f in all)
                {
                    ...
                    int regSaldos = this._dal_mvtoSaldosCostos.DAL_inSaldosExistencias_Add(saldosNew);
                    #endregion
                    ...
                    if (!consecCostos.Contains(f.RegistroCosto.Value.Value))
                    {
                        ...
                        regCostos = this._dal_mvtoSaldosCostos.DAL_inCostosExistencias_Add(costosNew);
                        consecCostos.Add(f.RegistroCosto.Value.Value);
                    }
                    #endregion
                    #region Agrega un Control de Saldos y Costos
        ...
        /// <returns>resultado de la transaccion</returns>
        public DTO_SerializedObject InventarioFisico_Add(int documentID, string bodega, DateTime periodo, out int numeroDoc, List<DTO_inFisicoInventario> fisicoInventario, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            DTO_TxResult result = new DTO_TxResult();
            result.Result = ResultValue.OK;
            DTO_TxResultDetail resultGLDC;
            ...
            try
            {
                ...
                {
                    bool _existInvFisico = this._dal_inFisicoInventario.DAL_inFisicoInventario_Exist(bodega, periodo);
                    ...
                    if (!_existInvFisico)
                    {
                        ...
                        _dtoControl.FechaDoc.Value = periodo;
                        _dtoControl.PeriodoDoc.Value = periodo;
                        ...
                        _dtoControl.PeriodoUltMov.Value = periodo;
        ...
        public DTO_TxResult InventarioFisico_Aprobar(int documentID,DTO_InvFisicoAprobacion invAprob, string actFlujoID, bool createDoc,Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                {
                    #region Guarda Doc Control Entradas
                    ...
                    _dtoControlEntradas.PrefijoID.Value = bodega.PrefijoID.Value;
                    _dtoControlEntradas.Fecha.Value = invAprob.PeriodoID.Value.Value;
                    _dtoControlEntradas.PeriodoDoc.Value = invAprob.PeriodoID.Value.Value;
                    _dtoControlEntradas.TasaCambioCONT.Value = this._moduloGlobal.TasaDeCambio_Get(monedaLocal,invAprob.PeriodoID.Value.Value);
                    _dtoControlEntradas.TasaCambioDOCU.Value = this._moduloGlobal.TasaDeCambio_Get(monedaLocal,invAprob.PeriodoID.Value.Value);
                    _dtoControlEntradas.DocumentoID.Value = documentID;
                    ...
                    _dtoControlEntradas.Observacion.Value = EntradaSalida.Entrada.ToString();

                    DTO_TxResultDetail resultGLDC = this._moduloGlobal.glDocumentoControl_Add(documentID, _dtoControlEntradas, true);
                    if (resultGLDC.Message != ResultValue.OK.ToString())
                    {
                        result.Result = ResultValue.NOK;
                        result.ResultMessage = "NOK";
                        ...
                        _dtoControlEntradas.NumeroDoc.Value = Convert.ToInt32(resultGLDC.Key);
                    #endregion
                    #region Guardar en inMovimientoDocu Entradas
                    ...
                    if (result.Result == ResultValue.NOK)
                        return result;
                    #endregion;
                    #region Guarda movimientos de Entradas
                    foreach (DTO_glMovimientoDeta mov in mvtoEntrada)
                        mov.NumeroDoc.Value = _dtoControlEntradas.NumeroDoc.Value;
                    if (mvtoEntrada.Any(x => string.IsNullOrEmpty(x.BodegaID.Value)))
                    {
                        result.Result = ResultValue.NOK;
                        result.ResultMessage = "La Bodega en el detalle esta vacía, revisar";
                        return result;
                    } 
                    result = this._moduloGlobal.glMovimientoDeta_Add(mvtoEntrada,true,true);
                    if (result.Result == ResultValue.NOK)
                        return result;
                    #endregion
                    #region Guarda en la bitacora
                    this._moduloAplicacion = (ModuloAplicacion)this.GetInstance(typeof(ModuloAplicacion), this._mySqlConnection, this._mySqlConnectionTx, this.Empresa, this.UserId, this.loggerConnectionStr);
                    this._moduloAplicacion.aplBitacora_Add(this.Empresa.ID.Value, documentID, (int)FormsActions.Add, DateTime.Now,
                        this.UserId, this.Empresa.ID.Value, resultGLDC.Key, string.Empty, string.Empty,
                        string.Empty, string.Empty, 0, 0, 0);
                    #endregion
                    
                    #region PROCESA MOVIMIENTO INVENTARIOS
                    this.Transaccion_Aprobar(documentID, _dtoControlEntradas.NumeroDoc.Value.Value,actFlujoID, true, batchProgress, true,false);                    
                    #endregion
                    porcTotal += porcParte;
                    batchProgress[tupProgress] = (int)porcTotal;
                }
                ...
                {
                    #region Guarda Doc Control Salidas
                    ...
                    _dtoControlSalidas.PrefijoID.Value = bodega.PrefijoID.Value;
                    _dtoControlSalidas.Fecha.Value = invAprob.PeriodoID.Value.Value;
                    _dtoControlSalidas.PeriodoDoc.Value = invAprob.PeriodoID.Value.Value;
                    _dtoControlSalidas.TasaCambioCONT.Value = this._moduloGlobal.TasaDeCambio_Get(monedaLocal, invAprob.PeriodoID.Value.Value);
                    _dtoControlSalidas.TasaCambioDOCU.Value = this._moduloGlobal.TasaDeCambio_Get(monedaLocal, invAprob.PeriodoID.Value.Value);
                    _dtoControlSalidas.DocumentoID.Value = documentID;
                    ...
                    _dtoControlSalidas.Observacion.Value = EntradaSalida.Salida.ToString();

                    DTO_TxResultDetail resultGLDC = this._moduloGlobal.glDocumentoControl_Add(documentID, _dtoControlSalidas, true);
                    if (resultGLDC.Message != ResultValue.OK.ToString())
                    {
                        result.Result = ResultValue.NOK;
                        result.ResultMessage = "NOK";
                        ...
                        _dtoControlSalidas.NumeroDoc.Value = Convert.ToInt32(resultGLDC.Key);
                    #endregion
                    #region Guardar en inMovimientoDocu Salidas
                    DTO_inMovimientoDocu header = new DTO_inMovimientoDocu();
                    ...
                    if (result.Result == ResultValue.NOK)
                        return result;
                    #endregion;
                    #region Guarda movimientos de Salidas
                    foreach (var mov in mvtoSalida)
                        mov.NumeroDoc.Value = _dtoControlSalidas.NumeroDoc.Value;
                    if (mvtoSalida.Any(x => string.IsNullOrEmpty(x.BodegaID.Value)))
                    {
                        result.Result = ResultValue.NOK;
                        result.ResultMessage = "La Bodega en el detalle esta vacía, revisar";
                        return result;
                    } 
                    result = this._moduloGlobal.glMovimientoDeta_Add(mvtoSalida,true,true);
                    if (result.Result == ResultValue.NOK)
                        return result;
                    ...
                    #endregion
                    #region PROCESA MOVIMIENTO INVENTARIOS
                    this.Transaccion_Aprobar(documentID, _dtoControlSalidas.NumeroDoc.Value.Value,actFlujoID, true, batchProgress, true,false);
                    #endregion
                    porcTotal += porcParte;
                    batchProgress[tupProgress] = (int)porcTotal;
                }
            ...
            finally
            {
                if (result.Result == ResultValue.OK)
                {
                    if (!insideAnotherTx)
                    {
                        ...
                        if (_dtoControlEntradas != null && _dtoControlEntradas.DocumentoNro.Value == 0)
                        {
                            _dtoControlEntradas.DocumentoNro.Value = this.GenerarDocumentoNro(documentID, _dtoControlEntradas.PrefijoID.Value);
                            this._moduloGlobal.ActualizaConsecutivos(_dtoControlEntradas, true, false, false);
                        }
                        if (_dtoControlSalidas != null && _dtoControlSalidas.DocumentoNro.Value == 0)
                        {
                            _dtoControlSalidas.DocumentoNro.Value = this.GenerarDocumentoNro(documentID, _dtoControlSalidas.PrefijoID.Value);
                            this._moduloGlobal.ActualizaConsecutivos(_dtoControlSalidas, true, false, false);
                        }
                    }
                }
                else if (base._mySqlConnectionTx != null && !insideAnotherTx)
                ...
                foreach (DTO_InvFisicoAprobacion itemAprob in list)
                {
                    ...
                    foreach (var itemFisico in fisicoInventario)
                    {
                        if (itemFisico.CantAjuste.Value > 0)
                        {
                            ...
                            _valEntrada += (itemFisico.CostoLocal.Value.Value / itemFisico.CantKardex.Value.Value) * itemFisico.CantAjuste.Value.Value;
                        }
                        else if (itemFisico.CantAjuste.Value < 0)
                        {
                            _unidAjSalida += itemFisico.CantAjuste.Value.Value;
                            _valSalida += (itemFisico.CostoLocal.Value.Value / itemFisico.CantKardex.Value.Value) * Math.Abs(itemFisico.CantAjuste.Value.Value);
                        }
                    }
        ...
        public DTO_SerializedObject OrdenSalida_ApproveMvtoInv(int documentID, DTO_OrdenSalida orden, Dictionary<Tuple<int, int>, int> batchProgress, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                foreach (DTO_inOrdenSalidaDeta det in orden.Footer)
                {
                    ...
                    var mvtosDeta = this._dal_glMovimientoDeta.DAL_glMovimientoDeta_GetByParameter(filter,false);
        ...
        public DTO_TxResult Inventarios_Revertir(int documentID, int numeroDoc, int? consecutivoPos, ref List<DTO_glDocumentoControl> ctrls, ref List<DTO_coComprobante> coComps, bool insideAnotherTx)
        {
            ...
            try
            {
                ...
                if (ctrls.Count > 0)
                {
                    ...
                    #region Guardar en inMovimientoDocu
                    if (mvtoRevertido != null && mvtoRevertido.Header.MvtoTipoInvID.Value == mvtoEntrada)
                        mvtoRevertido.Header.MvtoTipoInvID.Value = mvtoSalida;
                    else if (mvtoRevertido != null && mvtoRevertido.Header.MvtoTipoInvID.Value == mvtoSalida)
                        mvtoRevertido.Header.MvtoTipoInvID.Value = mvtoEntrada;
                    ...
                    foreach (var d in mvtoRevertido.Footer)
                    {
                        ...
                        d.Movimiento.EntradaSalida.Value = mvtoRevertido != null && mvtoRevertido.Header.MvtoTipoInvID.Value == mvtoEntrada ? (byte)1 : (byte)2;
                ...
                switch (tipoReoprte)
                {
                    ...
                    case InventarioFisicoReportType.Diferencia:
                        ...
                        foreach (var dtoInvFisicoDetail in _listFisicoInventario)
                        {
                            ...
                            detail.UnidadLoc = dtoInvFisicoDetail.CostoLocal.Value.Value / dtoInvFisicoDetail.CantAjuste.Value.Value;
                            detail.TotalLoc = dtoInvFisicoDetail.CostoLocal.Value.Value;
                            detail.UnidadExt = dtoInvFisicoDetail.CostoExtra.Value.Value / dtoInvFisicoDetail.CantAjuste.Value.Value;
                ...
                string referencia, string grupo, string clase, string tipo, string serie, string material, bool isSerial, string Libro=null)
        {
            ...
            inven.Detalles = _dal_ReportesInventarios.DAL_ReportesInventarios_Saldos(año, mesIni, bodega, tipoBodega, referencia, grupo, clase, tipo, serie, material, isSerial, Libro);
                                                       ...
                                                       string serie, string material, bool isSerial, string libro, object otroFilter, byte? agrup, byte? romp)
        {
            try
            {
                ...
                result = this._dal_ReportesInventarios.DAL_Reportes_In_InventarioToExcel(documentID, mesIni, mesFin, bodega, tipoBodega, referencia, grupo, clase, tipo, serie, material, isSerial, libro,otroFilter, agrup, romp);
