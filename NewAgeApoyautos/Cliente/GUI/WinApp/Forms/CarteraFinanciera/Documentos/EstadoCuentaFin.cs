    ...
    public partial class EstadoCuentaFin : FormWithToolbar
    {
            ...
            try
            {
                         ...
                         proposito == (byte)PropositoEstadoCuenta.RestructuracionAbono || proposito == (byte)PropositoEstadoCuenta.RestructuracionPlazo)
                {
                    ...
                    foreach (DTO_ccSaldosComponentes comp in this.infoCreditoTemp.SaldosComponentes)
                    {
                        ...
                        if (this.fechaCorte.Date >= pagosFecha[comp.CuotaID.Value.Value].Date && comp.CuotaSaldo.Value.Value > 0 && comp.CuotaID.Value >= primeraCuota)
                        {
                        ...
                        if (this.sector == SectorCartera.Financiero && this.lkpPoliza.Enabled && comp.CuotaSaldo.Value.Value > 0 && comp.CuotaID.Value >= primeraCuota)
                        {
                ...
                else if (proposito == (byte)PropositoEstadoCuenta.EnvioCobroJuridico)
                {
                    ...
                    foreach (DTO_ccSaldosComponentes comp in this.infoCreditoTemp.SaldosComponentes)
                    {
                        if (comp.CuotaSaldo.Value.Value > 0 && comp.CuotaID.Value >= primeraCuota)
                        {
                            //Pago del capital
                ...
                else if (proposito == (byte)PropositoEstadoCuenta.CancelaPoliza)
                {
                    ...
                    foreach (DTO_ccSaldosComponentes comp in this.infoCreditoTemp.SaldosComponentes)
                    {
                        ...
                        if (this.fechaCorte >= pagosFecha[comp.CuotaID.Value.Value] && comp.CuotaSaldo.Value.Value > 0 && comp.CuotaID.Value >= primeraCuota)
                        {
                            //Pago de componentes atrasados
                            comp.AbonoValor.Value = comp.CuotaSaldo.Value;
                        }
        ...
        private void lkp_Proposito_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.sector == SectorCartera.Financiero && (proposito == (byte)PropositoEstadoCuenta.Prepago || proposito == (byte)PropositoEstadoCuenta.RecogeSaldo ||
                    proposito == (byte)PropositoEstadoCuenta.RestructuracionPlazo))
                {
                    this.lkpPoliza.EditValue = "1";
                    this.lkpPoliza.Enabled = true;
                }
                else if (this.sector == SectorCartera.Financiero && proposito == (byte)PropositoEstadoCuenta.RestructuracionAbono)
                {
                    this.lkpPoliza.EditValue = "1";
                    this.lkpPoliza.Enabled = false;
                    this.txtVlrAbono.ReadOnly = false;
                }
                else if (this.sector == SectorCartera.Financiero && proposito == (byte)PropositoEstadoCuenta.CancelaPoliza)
                {
