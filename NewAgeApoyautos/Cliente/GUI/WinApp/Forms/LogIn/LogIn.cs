        ...
        private void btnLogIn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (userVal == UserResult.AlreadyMember)
                {
                    ...
                    IEnumerable<DTO_seGrupoDocumento> rsx = _bc.AdministrationModel.seGrupoDocumento_GetByUsuarioId(user.ReplicaID.Value.Value, user.EmpresaIDPref.Value);
                    //rsx = rsx.Where(x => x.ModuloID.Value == "cc" || x.ModuloID.Value == "cf" || x.ModuloID.Value == "gl" || x.ModuloID.Value == "se").ToList();
