...
namespace NewAge.Cliente.GUI.WinApp.Forms
{
    ...
    /// </summary>
    public partial class ModalComplex : Form
    {
        #region Variables
        ...
        private int _docFK = 0;
        private bool _jerarquicaFKInd = false;
        ...
        private IEnumerable<DTO_MasterComplex> _dtoList;
        ...
        public ModalComplex(ButtonEdit origin, string code, string fCount, string fData, Object[] fArgs, string colId, string colDesc, int docFK, bool jerarquicaFKInd, List<DTO_glConsultaFiltro> filtros = null)
        {
            //Variables
            ...
            this._docFK = docFK;
            this._jerarquicaFKInd = jerarquicaFKInd;
        ...
        private void LoadGridData(bool firstTime, bool firstPage, bool lastPage)
        {
            try
            {
                ...
                long count = (long)mCount.Invoke(_bc.AdministrationModel, new Object[] { this._documentID, this._filtrosComplejos, active });
                ...
                var data = mData.Invoke(_bc.AdministrationModel, new Object[] { this._documentID, this._pageSize, this.pgGrid.PageNumber, this._filtrosComplejos, active });

                //var test = mData.Invoke(_bc.AdministrationModel, new Object[] { AppMasters.seUsuarioGrupo, this._pageSize, this.pgGrid.PageNumber, null, active });
                

                this._dtoList = (IEnumerable<DTO_MasterComplex>)data;
                this.grlcontrolMasterComplex.DataSource = data;
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-ModalComplex.cs", "MasterComplex(" + this._documentID + ")-LoadGridData"));
            }
        }
        ...
        private void LoadGridStructure()
        {
            try
            {
                ...
                this.toolTipGrid.SetToolTip(this.grlcontrolMasterComplex, _bc.GetResource(LanguageTypes.Messages, DictionaryMessages.ToolTipGrid));

                #region Organiza la grilla datos

                foreach (GridColumn columna in this.gvMasterComplex.Columns)
                {
                ...
                this.gvMasterComplex.Columns.Add(code);
                ...
                this.gvMasterComplex.Columns.Add(desc);
            ...
            {
                MessageBox.Show(_bc.GetResourceForException(ex, "WinApp-MasterComplex.cs", "MasterComplex(" + this._documentID + ")-LoadGridStructure"));
            }           
        }
        ...
        {
            DTO_MasterComplex dto = (DTO_MasterComplex)this.gvMasterComplex.GetRow(row);
            this._currentText = dto.PKValues[this._colId];//dto.ID.Value;
        ...
        {
            Point pt = this.grlcontrolMasterComplex.PointToClient(Control.MousePosition);

            GridHitInfo hitInfo = this.gvMasterComplex.CalcHitInfo(pt);
            if (hitInfo.InColumn)
            {
        ...
        private void grlcontrolMasterModal_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                int row = this.gvMasterComplex.FocusedRowHandle;
        ...
        protected virtual void gvMasterModal_CustomUnboundColumnData(object sender, CustomColumnDataEventArgs e)
        {
            if (e.IsGetData)
            {
                DTO_MasterComplex dto = this._dtoList.ElementAt(e.ListSourceRowIndex);

                string fieldName = e.Column.FieldName.Substring(this.unboundPrefix.Length);

                DTO_MasterBasic dtoFK = (DTO_MasterBasic)_bc.GetMasterDTO((this._jerarquicaFKInd)?AppMasters.MasterType.Hierarchy: AppMasters.MasterType.Simple, this._docFK, false, dto.PKValues[this._colId], true);

                if (fieldName == this._colId)
                    e.Value = dtoFK.ID.Value;
                if (fieldName == this._colDesc)
                    e.Value = dtoFK.Descriptivo.Value;
            }
        }
        ...
        protected virtual void gvMasterModal_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            if (e.FocusedRowHandle>=0 && e.FocusedRowHandle<this._dtoList.Count())
                this._currentText = this._dtoList.ElementAt(e.FocusedRowHandle).PKValues[this._colId];
        }
