            ...
            try
            {
                ...
                if (dtoObj.GetType() == typeof(DTO_ccCliente))
                {
                    try
                    {
                        ...
                        #region Verifica el Estado Civil
                        if (dto.EstadoCivil.Value == 0)
                            if (string.IsNullOrEmpty(dto.EstadoOtro.Value))
                            {
                                res = false;
                                msg = _bc.GetResource(LanguageTypes.Messages, MasterMessages.Cc_Cliente_ServicioTipo_EstadoCivil);
                                result.Add("EstadoCivil", msg);
                            }
