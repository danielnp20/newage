        ...
        public string GenerateReport(DTO_SolicitudTrabajo solicitud, byte tipoReport)
        {
            try
            {
                ...
                if (tipoReport == 2) //Reporte con Valores Cliente
                {
                    ...
                    //if (claseproy != null && claseproy.TipoPresupuesto.Value == (byte)TipoPresupuestoProy.Construccion && solicitud.Header.PorMultiplicadorPresup.Value != 100)//calcula el valor del multiplicador si es Proy Cliente
                    //{
                    //    foreach (DTO_pyPreProyectoTarea tarea in solicitud.Detalle)
                    //    {
                    //        if (tarea.CostoLocalUnitCLI.Value != null)
                    //            tarea.CostoLocalUnitCLI.Value = Math.Round(tarea.CostoLocalUnitCLI.Value.Value * (solicitud.Header.PorMultiplicadorPresup.Value.Value / 100), 0);
                    //        tarea.CostoLocalCLI.Value = Math.Round(tarea.CostoLocalCLI.Value.Value * (solicitud.Header.PorMultiplicadorPresup.Value.Value / 100), 0);
                    //    }
                    ...
                    //    }
                    //    this.solicitud.Header.Valor.Value = this.solicitud.Detalle.FindAll(x => x.DetalleInd.Value.Value).Sum(x => x.CostoLocalCLI.Value) +
                    //                this.solicitud.Header.ValorIVA.Value + this.solicitud.Header.ValorOtros.Value;
                    //}

                    #endregion
                    foreach (var tarea in this.solicitud.Detalle)
                    {
                        tarea.CostoTotalML.Value = tarea.CostoLocalCLI.Value;
