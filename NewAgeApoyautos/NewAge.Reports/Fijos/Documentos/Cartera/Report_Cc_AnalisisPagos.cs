...
{
    public partial class Report_Cc_AnalisisPagos : ReportBaseLandScape
    {
        #region Variables
        ModuloCartera _modulocart;
        #endregion
        public Report_Cc_AnalisisPagos()
        {

        }

        public Report_Cc_AnalisisPagos(string loggerConn, SqlConnection c, SqlTransaction tx, DTO_glEmpresa empresa, int userId, ExportFormatType formatType)
        ...
        public string GenerateReport(byte? tipoReporte, DateTime? fechaIni, DateTime fechaFin, string cliente, int? libranza, string zonaID, string ciudad, string concesionario, string asesor, string lineaCredi, string compCartera)
        {
            try
            {
                this.lblReportName.Text = this._moduloGlobal.GetResource(LanguageTypes.Forms, AppReports.ccRepAnalisisPagos.ToString());
                ...
                List<DTO_QueryCarteraMvto> mvtos = this._modulocart.CarteraMvto_QueryByLibranza(libranza,cliente,false,tipoReporte == 2? true : false);

                //Asigna valores de pagos del Cliente
                if (tipoReporte == 2)
                {
                    this.lblReportName.Text = "Pagos Cliente";
